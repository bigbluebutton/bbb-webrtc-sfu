...
const HOSTNAME = "video--approval.rnp.br";
...
const getTransmissionUrl = (transmission) => {
  return "https://" + HOSTNAME + "/portal/transmission.action?idItem=" + transmission.id;
};
...
const getStreamKey = (auth, callback) => {
  createTransmission(auth, (err, transmission) => {
    ...
    return callback(null, transmission.rtmp, getTransmissionUrl(transmission));
  });
}
