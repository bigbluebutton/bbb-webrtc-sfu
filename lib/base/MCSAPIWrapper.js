...
module.exports = class MCSAPIWrapper extends EventEmitter {
  ...
  async start (address, port) {
    ...
    return new Promise((resolve) => {
      try {
        Logger.info('Connecting to MCS', { addr: this.addr });
        this._mcs = new MCS(this.addr);
        this._monitorConnectionState();
        this._connectionResolver = resolve;
      } catch(error) {
        Logger.error(`MCS startup connection failed: ${error.message}`,
  ...
  _onOpen () {
    Logger.info('Connected to MCS', { addr: this.addr });
  ...
  _onDisconnection () {
    ...
    if (this._reconnectionRoutine == null) {
      ...
      this._reconnectionRoutine = setInterval(async () => {
        try {
          Logger.info('Reconnecting to MCS');
          this._mcs = new MCS(this.addr);
          this._monitorConnectionState()
        } catch (error) {
          Logger.warn(`MCS reconnection failed: ${error.message}`,
            { errorMessage: error.message, errorCode: error.code });
          delete this._mcs;
        }
      }, 2000);
  ...
  _onClientConnectionError (error) {
    if (error) {
      Logger.error( `MCS connection ungracefully terminated: ${error.message}`,
        { errorMessage: error.message, errorCode: error.code });
    } else {
      Logger.error('MCS connection ungracefully terminated');
    }
  ...
  async onEvent (eventName, identifier, callback) {
    try {
      await this._mcs.onEvent(eventName, identifier, callback);
    }
    catch (error) {
      Logger.error('MCSAPIWrapper: onEvent failure', error);
    }
  }
  ...
  async restartIce (mediaId) {
    try {
      const { descriptor } = await this._mcs.restartIce(mediaId);
      return descriptor;
    } catch (error) {
      throw (this._handleError(error, 'mediaId', { mediaId }));
    }
  }
