...
const { RECORDING_DIR: BBB_WEBRTC_RECDIR } = require('../common/bbb-webrtc-recorder');
...
module.exports = class BaseProvider extends EventEmitter {
  ...
  getRecordingBaseDir (recordingAdapter) {
    // TODO deduplicate configs - get dir as answer from recorder
    if (recordingAdapter === 'bbb-webrtc-recorder') return BBB_WEBRTC_RECDIR;

    return `/var/${recordingAdapter.toLowerCase()}`;
  }

  getRecordingFilePathSuffix (room, subPath, recordingName, format) {
    const timestamp = (new Date()).getTime();
    return `${subPath}/${room}/${recordingName}-${timestamp}.${format}`;
  }

  getFullRecordingPath (baseDir, fileName) {
