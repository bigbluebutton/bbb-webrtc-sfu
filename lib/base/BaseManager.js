...
module.exports = class BaseManager {
  ...
  _fetchLifecycleQueue (id, concurrency = 1) {
    if (this._lifecycleQueue[id] == null) {
      const newQueue = QueueD({ concurrency, autostart: true });
      this._lifecycleQueue[id] = newQueue;
    }

    return this._lifecycleQueue[id];
  }

  _deleteLifecycleQueue (id) {
    if (this._lifecycleQueue[id]) {
      delete this._lifecycleQueue[id];
    }
  }
