...
const BASE_MODULE_SPAWN_DELAY = config.has('moduleSpawnDelayMs')
  ? config.get('moduleSpawnDelayMs')
  : 0;
const CORE_COUNT = os.cpus().length;
const MODULE_SPAWN_DELAY = Math.min(
  (4 * BASE_MODULE_SPAWN_DELAY) / CORE_COUNT,
  (12000 / MODULES.length),
);
...
class SFUModuleManager {
  ...
  async start () {
    Logger.info("Starting SFU Module Manager", {
      moduleCount: MODULES.length,
      moduleSpawnDelay: MODULE_SPAWN_DELAY,
    });
