...
module.exports = class WebsocketConnectionManager {
  static extractUserInfosFromWsReq (websocketReq) {
    ...
    if (typeof userId === 'string'
      ...
      return { userId, meetingId, voiceBridge, userName };
    } else {
      throw new Error('InvalidHeaders');
    }
  }

  constructor (host, port, path, wsServerOptions, options = {}) {
    this.emitter = this.setEventEmitter(options.eventEmitter);
  ...
  _closeSocket (ws) {
    try {
      ...
      Logger.error('WSManager: socket close failed', {
        connectionId: ws.id,
  ...
  _setupServerHeartbeat (ws) {
    ...
    ws.pingRoutine = setInterval(() => {
      if (ws.isAlive === false) {
        Logger.error("Terminating websocket: heartbeat failure", {
          connectionId: ws.id,
        });
      ...
      setTimeout(() => {
        try {
          ...
          Logger.error("WSManager: ping routine failed", {
  ...
  _onNewConnection (ws, req) {
    try {
      ...
      this._updateLastMsgTime(ws);
      Logger.debug("WSManager: connection opened", { connectionId: ws.id, userInfos: ws.userInfos });
    } catch (error) {
      if (WS_STRICT_HEADER_PARSING && error.message === 'InvalidHeaders') {
        Logger.error('WSManager: failure on WS connection startup', {
          errorMessage: error.message,
        });
        this._closeSocket(ws);
  ...
  _onMessage (ws, data) {
    ...
    try {
      ...
      Logger.error("WSManager: JSON message parse failed", {
  ...
  _onError (ws, error, reason = 'UnknownReason') {
    Logger.debug("WSManager: error event", {
  ...
  _onClose (ws) {
    Logger.debug("WSManager: connection closed", { connectionId: ws.id });
  ...
  sendMessage (ws, json) {
    if (ws._closeCode === 1000) {
      Logger.error("WSManager: socket is closed, won't send message", {
    ...
    return ws.send(JSON.stringify(json), (error) => {
      if (error) {
        Logger.error('WSManager: send failed', {
          connectionId: ws ? ws.id : 'unknown',
          errorMessage: error.message,
          requestId: json.id,
        });
