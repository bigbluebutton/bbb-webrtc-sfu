...
module.exports = class Screenshare extends BaseProvider {
  ...
  getConnectionIdAndRole (userId) {
    ...
    for (const connectionId in this._viewerUsers) {
      if (Object.prototype.hasOwnProperty.call(this._viewerUsers, connectionId)) {
        const user = this._viewerUsers[connectionId]
        if (Object.prototype.hasOwnProperty.call(user, 'userId') && user['userId'] === userId) {
  ...
  async startRecording () {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        this.recording = await proxiedStartRecording(
