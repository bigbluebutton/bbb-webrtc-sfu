...
module.exports = class Screenshare extends BaseProvider {
  ...
  async onIceCandidate (candidate, role, userId, connectionId) {
    ...
    switch (role) {
      ...
      case C.RECV_ROLE:
        ...
        if (endpoint) {
          try {
            await this.flushCandidatesQueue(endpoint, this._viewersCandidatesQueue[connectionId]);
  ...
  _mediaStateWebRTC (event, endpoint) {
    ...
    switch (name) {
      ...
      case "MediaFlowOutStateChange":
      case "MediaFlowInStateChange":
        // TODO this may have broken flash WebRTC screensharing :P
        Logger.info(LOG_PREFIX, "Session with media", mediaId, "received state", state);
        if (details === 'FLOWING') {
          this._onRtpMediaFlowing();
        }
  ...
  sendStartShareEvent () {
    ...
    this._BigBlueButtonGW.writeMeetingKey(this._meetingId, shareEvent, function(error) {
  ...
  _startPresenter (sdpOffer) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        Logger.error(LOG_PREFIX, "I'm sorry, I'm afraid I couldn't start the RTMP screenshare transposer :-)", e);
      }
    });
  }
  ...
  async _upstartRTPStream () {
    try {
      ...
      let mediaServerIp = answer.match(/(IP4\s)([0-9.]*)/g)[1];
  ...
  _startViewer(connectionId, voiceBridge, sdpOffer, userId, presenterEndpoint) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const { mediaId, answer } = await this.mcs.subscribe(this.mcsUserId,
          "content", C.WEBRTC, options);

        this._viewerEndpoints[connectionId] = mediaId;
        sdpAnswer = answer;
        await this.flushCandidatesQueue(this._viewerEndpoints[connectionId], this._viewersCandidatesQueue[connectionId]);
