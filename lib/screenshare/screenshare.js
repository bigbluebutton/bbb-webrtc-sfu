...
module.exports = class Screenshare extends BaseProvider {
  ...
  async _onRtpMediaFlowing () {
    if (!this._rtmpBroadcastStarted) {
      ...
      const strm = Messaging.generateStartTranscoderRequestMessage(this.meetingId, this.meetingId, this._rtpParams);

      // Checking if transcoder is avaiable; if so, transposes the stream to RTMP
      if (shouldHandleRtmpBridge) {
        // Interoperability: capturing 1.1 start_transcoder_reply messages
        this.bbbGW.once(C.START_TRANSCODER_REPLY+this.meetingId, (payload) => {
        ...
        this.bbbGW.once(C.START_TRANSCODER_RESP_2x+this.meetingId, (payload) => {
  ...
  async _upstartRTPStream () {
    try {
      ...
      this._rtpParams = MediaHandler.generateTranscoderParams(mediaServerIp, localIpAddress,
        sendVideoPort, recvVideoPort, this.meetingId, "stream_type_video",
  ...
  _notifyScreenshareEndToBBB () {
    return new Promise(async (resolve, reject) => {
      try {
        const shouldHandleRtmpBridge = await this.bbbGW.isChannelAvailable(C.TO_BBB_TRANSCODE_SYSTEM_CHAN) && ENABLE_SCREENSHARE_FLASH_BRIDGE;
        const strm = Messaging.generateStopTranscoderRequestMessage(this.meetingId, this.meetingId);
        // Either the transcoder is not available or screensharing couldn't be
        // correctly started
        if (this.status != C.MEDIA_STARTED || !shouldHandleRtmpBridge) {
          this._stopRtmpBroadcast(this.meetingId);
          return resolve();
        }
        if (shouldHandleRtmpBridge) {
          // Interoperability: capturing 1.1 stop_transcoder_reply messages
          this.bbbGW.once(C.STOP_TRANSCODER_REPLY+this.meetingId, async (payload) => {
          ...
          // Capturing stop transcoder responses from the 2x model
          this.bbbGW.once(C.STOP_TRANSCODER_RESP_2x+this.meetingId, async (payload) => {
  ...
  stop () {
    return new Promise(async (resolve, reject) => {
      ...
      switch (this.status) {
        ...
        default:
          Logger.info(LOG_PREFIX, `Stopping screensharing session`,
            this._getFullPresenterLogMetadata());
          this.stopPresenter().then(resolve).catch(error => {
