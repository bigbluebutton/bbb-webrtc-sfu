...
module.exports = class Screenshare extends BaseProvider {
  ...
  _getFullPresenterLogMetadata (connectionId) {
    return {
      ...
      connectionId: connectionId || this._connectionId,
      role: `presenter`,
    };
  }
  ...
  _getFullViewerLogMetadata (connectionId) {
    ...
    return {
      ...this._getPartialLogMetadata(),
      presenterUserId: this.userId,
      presenterMediaId: this._presenterEndpoint,
      presenterConnectionId: this._connectionId,
      userId,
      mediaId,
      connectionId,
      role: `viewer`,
  ...
  async onIceCandidate (candidate, role, connectionId) {
    switch (role) {
      ...
      default:
        Logger.warn("onIceCandidate: Unknown role", role);
      }
  }
  ...
  restartIce (role, userId, connectionId) {
    let endpoint;

    if (userId === this.userId) {
      // Presenter
      endpoint = this._presenterEndpoint;
    } else {
      // Viewer
      endpoint = this._viewerEndpoints[connectionId];
    }

    if (endpoint) return this.mcs.restartIce(endpoint);
  ...
  stopViewer (id) {
    ...
    const logMetadata = this._getFullViewerLogMetadata(id) || this._getPartialLogMetadata();
    ...
    if (viewerMediaId) {
      return this.mcs.unsubscribe(userId, viewerMediaId)
        ...
        .catch(error => {
          ...
          Logger.info('Viewer screensharing session stopped', logMetadata);
        });
    } else {
  ...
  async stopPresenter () {
    const logMetadata = this._getFullPresenterLogMetadata(this._connectionId);
    ...
    Logger.info('Presenter screensharing session stopped', logMetadata);
  }
