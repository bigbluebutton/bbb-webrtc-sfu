...
module.exports = class Screenshare extends BaseProvider {
  ...
  async onIceCandidate (candidate, role, userId) {
    ...
    switch (role) {
      ...
      case C.RECV_ROLE:
        let endpoint = this._viewerEndpoints[userId];
        if (endpoint) {
          try {
  ...
  _mediaStateRTP (event, endpoint) {
    ...
    switch (name) {
      ...
      case "MediaFlowOutStateChange":
        Logger.info("[screenshare] Session with media", mediaId, "received state", state);
        break;
  ...
  async _publishWebRTCStream (descriptor) {
    try {
      const { mediaId, answer } = await this.mcs.publish(this.mcsUserId, this._meetingId, 'WebRtcEndpoint', { descriptor });
  ...
  async _upstartRTPStream () {
    try {
      ...
      const { mediaId, answer } = await this.mcs.subscribe(this.mcsUserId,
        sharedScreens[this._voiceBridge], 'RtpEndpoint',
        { descriptor: rtpSdpOffer, keyframeInterval: KEYFRAME_INTERVAL});
  ...
  _startViewer(connectionId, voiceBridge, sdpOffer, userId, presenterEndpoint) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const { mediaId, answer } = await this.mcs.subscribe(this.mcsUserId, sharedScreens[voiceBridge], 'WebRtcEndpoint', {descriptor: sdpOffer});

        this._viewerEndpoints[userId] = mediaID;
        sdpAnswer = answer;
        await this.flushCandidatesQueue(this._viewerEndpoints[userId], this._viewersCandidatesQueue[userId]);

        this.mcs.on(C.MEDIA_STATE, (event) => {
          this._mediaStateWebRTC(event, this._viewerEndpoints[userId])
        });

        this.mcs.on(C.MEDIA_STATE_ICE, (event) => {
          this._onMCSIceCandidate(event, connectionId, this._viewerEndpoints[userId]);
        });

        Logger.info(LOG_PREFIX, "MCS subscribe returned for user", this.mcsUserId, "returned", this._viewerEndpoints[userId], "at userId", userId);
        return resolve(sdpAnswer);
      }
      catch (err) {
  ...
  stop () {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        Logger.info(LOG_PREFIX, "Left MCS room", this._meetingId);
