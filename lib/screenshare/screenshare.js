...
module.exports = class Screenshare extends BaseProvider {
      ...
      case "MediaFlowInStateChange":
        if (details === 'NOT_FLOWING' && this.status !== C.MEDIA_PAUSED) {
          ...
          if (this.recordFullDurationMedia === false && !this._startRecordingEventFired) {
            this.sendStartShareEvent(state?.timestampHR, rawEvent?.timestampMillis);
          }
        }
        break;
  ...
  async startRecording (options = {}) {
    try {
      ...
      if (this.recordFullDurationMedia || (!eventOnFlow && !this._startRecordingEventFired)) {
  ...
  async _stopRecording () {
    ...
    if (this.isSFURecording()) {
      ...
      try {
        const recordingId = this.recording.recordingId;
        this.recording = {};
        await this.mcs.stopRecording(this.presenterMCSUserId, recordingId);
