  ...
  async stopViewer(id) {
    ...
    if (viewerMediaId) {
      try {
        ...
        Logger.error(LOG_PREFIX, `Viewer unsubscribe failed for ${userId}`,
          this._getFullViewerLogMetadata(id));
      }
    }
    ...
    if (userId) {
      try {
        ...
        Logger.error(LOG_PREFIX, `Viewer leave failed for ${userId}`,
          this._getFullViewerLogMetadata(id));
      }
    }
      ...
      if (this.presenterMCSUserId) {
        if (this._presenterEndpoint) {
          ...
          try {
            ...
            Logger.error(LOG_PREFIX, `Unpublish failed for presenter ${this.presenterMCSUserId}`,
              { ...this._getFullPresenterLogMetadata(this._connectionId), error });
          }
        }
        ...
        if (this._ffmpegEndpoint) {
          try {
            ...
            Logger.error(LOG_PREFIX, `RTP relay unsubscribe failed for presenter's ${this.presenterMCSUserId}`,
              { ...this._getFullPresenterLogMetadata(this._connectionId), error });
            this._ffmpegEndpoint = null;
          }
        }
        ...
        try {
          ...
          Logger.warn(LOG_PREFIX, `Leave failed for presenter ${this.presenterMCSUserId}`,
            { ...this._getFullPresenterLogMetadata(this._connectionId), error });
        }
      }
