...
module.exports = class Screenshare extends BaseProvider {
        ...
        if (this.isRecorded) {
          this.startRecording().catch(error => {
            Logger.error('Recording start failed', {
              recordingAdapter: this.getRecordingAdapter(),
  ...
  async _stopWebRTCRecorder () {
    ...
    if (nativeSubMediaId) {
      try {
        ...
        Logger.error("bbb-webrtc-recorder: native recording subscriber cleanup failure",
          { ...this._getFullPresenterLogMetadata(), error, recordingAdapter: this.getRecordingAdapter() });
      } finally {
    ...
    if (recordingSessionId) {
      try {
        // TODO use reason and timestamps for better sync'ed events
        await Recorder.stopRecording(recordingSessionId);
      } catch(error) {
  ...
  async _recordViaWebRTCRecorder (sourceMediaId, _recordingPath, { filename }) {
    ...
    // Step 2
    // Noticed that we're ignoring the full file path (_recordingPath)
    // because the base directory is pre-set in bbb-webrtc-recorder
    const hgaAnswer = await Recorder.startRecording( recordingSessionId, filename, nativeDescriptor);
    ...
    return {
      ...
      filename,
      // TODO filename: needs to be full file path, and the base is defined in
      // bbb-webrtc-recorder...
  ...
  async _recordViaHGAdapter (sourceMediaId, recordingPath, recordingOptions) {
    ...
    // Step 4
    recordingOptions.adapter = this.getRecordingAdapter();
  ...
  async _stopRecording () {
    ...
    if (this.isRecorded && this.recording && this.recording.recordingId) {
      ...
      // TODO use recording stop timestamps from recorder
      if (!this._stopRecordingEventFired) this._sendStopShareEvent();

      // TODO do only what's necessary here - define proper stop method without
      // this sketchy if block
      if (this._getRecordingAdapter() === 'bbb-webrtc-recorder') {
        await this._stopWebRTCRecorder();
        return;
      }
