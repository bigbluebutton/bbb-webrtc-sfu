...
module.exports = class Screenshare extends BaseProvider {
  ...
  static buildSubscriberMCSOptions (descriptor, streamName, adapter) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const mcsOptions = Screenshare.buildSubscriberMCSOptions(
          descriptor, streamName, options.mediaServer
        );
  ...
  processAnswer (answer, role, userId, connectionId) {
    ...
    if (endpoint) {
      ...
      const mcsOptions = {
        mediaId: endpoint,
        ...Screenshare.buildSubscriberMCSOptions(answerWithCSlides, streamName),
      };
