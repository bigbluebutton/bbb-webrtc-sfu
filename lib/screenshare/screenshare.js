...
module.exports = class Screenshare extends BaseProvider {
  ...
  _startPresenter (sdpOffer) {
    return new Promise(async (resolve, reject) => {
      try {
        const { mediaId, answer } = await this.mcs.publish(this.mcsUserId, this._meetingId, 'WebRtcEndpoint', { descriptor: sdpOffer });

        this._presenterEndpoint = mediaId;
        sharedScreens[this._voiceBridge] = this._presenterEndpoint;
        let presenterSdpAnswer = answer;
        await this.flushCandidatesQueue(this._presenterEndpoint, this._presenterCandidatesQueue);
        ...
        const { rtpMediaId, rtpAnswer } = await this.mcs.subscribe(this.mcsUserId, sharedScreens[this._voiceBridge], 'RtpEndpoint', { descriptor: rtpSdpOffer, keyframeInterval: KEYFRAME_INTERVAL});

        this._ffmpegEndpoint = rtpMediaId;
        rtpEndpoints[this._voiceBridge] = this._ffmpegEndpoint;

        let recvVideoPort = rtpAnswer.match(/m=video\s(\d*)/)[1];
