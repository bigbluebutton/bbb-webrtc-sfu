...
module.exports = class Screenshare extends BaseProvider {
  ...
  async _publishWebRTCStream (descriptor) {
    try {
      const options = {
        descriptor,
        name: this._assembleStreamName('publish', this.userId, this._meetingId),
      };

      const { mediaId, answer } = await this.mcs.publish(this.mcsUserId, this._meetingId, C.WEBRTC, options);
  ...
  async _upstartRTPStream () {
    try {
      ...
      const { mediaId, answer } = await this.mcs.subscribe(this.mcsUserId,
        sharedScreens[this._voiceBridge], C.RTP, options);
  ...
  _startViewer(connectionId, voiceBridge, sdpOffer, userId, presenterEndpoint) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const options = {
          descriptor: sdpOffer,
          name: this._assembleStreamName('subscribe', userId, this._meetingId),
        }

        const { mediaId, answer } = await this.mcs.subscribe(this.mcsUserId,
          sharedScreens[voiceBridge], C.WEBRTC, options);
