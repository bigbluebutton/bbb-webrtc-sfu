...
module.exports = class ScreenshareManager extends BaseManager {
  ...
  static getMetadataFromMessage (message) {
    return {
      sfuMessageId: message.id || "none",
      connectionId: message.connectionId || "none",
      internalMeetingId: message.internalMeetingId || "none",
      roomId: message.voiceBridge || "none",
  ...
  handleIceCandidate (message) {
    ...
    if (session && session.constructor === Screenshare) {
      session.onIceCandidate(candidate, role, callerName, connectionId);
      Logger.debug(this._logPrefix, `Added ICE candidate for screensharing session`,
        ScreenshareManager.getMetadataFromMessage(message));
    } else {
      Logger.info(this._logPrefix, `ICE candidate queued for screensharing sessio`,
        ScreenshareManager.getMetadataFromMessage(message));
  ...
  handleStop (message) {
    ...
    // FIXME CHECK THIS
    //} else {
    //  Logger.warn(this._logPrefix, `Screensharing session not found on stop`,
    //    ScreenshareManager.getMetadataFromMessage(message));
    //}
  }
  ...
  handleClose (message) {
    ...
    Logger.info(this._logPrefix, 'Connection closed',
      ScreenshareManager.getMetadataFromMessage(message));
    return this.closeSession(session, connectionId, role, voiceBridge);
  }
  ...
  async _onMessage(message = {}) {
    Logger.info(this._logPrefix, `Received message from ${message.connectionId}: ${message.id}`);
    let queue;
  ...
  _handleRoomCreated (event) {
    ...
    this.mcs.onEvent('roomDestroyed', room, (event) => {
      Logger.debug(this._logPrefix, `Room destroyed`, { roomId: room });
      try {
        this._stopSession(room);
      } catch (error) {
        Logger.error(LOG_PREFIX, `Screensharing session stop failed at room destroyed handler`,
  ...
  async _handleContentFloorChanged (event) {
    ...
    try {
      ...
      Logger.error(LOG_PREFIX, `Screensharing session stop failed at content floor changed handler`,
        { error: this._handleError(this._logPrefix, null, null, null, error), roomId: room });
    }
  }
