...
module.exports = class ScreenshareManager extends BaseManager {
  ...
  _trackScreenBroadcastStopSysMsg () {
    this._bbbGW.on(C.SCREEN_BROADCAST_STOP_SYS_MSG, (payload) => {
      ...
      const userId = payload[C.USER_ID_2x] || 'SYSTEM';
      const sessionId = this._meetings[meetingId];
      const session = this.getSession(sessionId);
      ...
      if (session) {
      ...
      } else if (!streamId.startsWith('TR_')) {
        // If there is no session, we still need to notify the clients that
        // the stream has stopped, otherwise they will keep trying to
        // subscribe to a non-existing stream forever. We don't do this
        // if the streamId starts with 'TR_' because those are LiveKit and
        // should be handled in the `livekit` module.
        ...
        const dsrstom = Messaging.generateScreenshareRTMPBroadcastStoppedEvent2x(
          voiceBridge, voiceBridge, streamId, 0, 0, timestamp, { userId },
        );
  ...
  handleRestartIce (message) {
    ...
    if (session && session.constructor === Screenshare) {
      const sessionMetadata = ScreenshareManager.getMetadataFromMessage(message);
      return session.restartIce(role, userId, connectionId).then((sdp) => {
        this.sendToClient({
          type: C.SCREENSHARE_APP,
          connectionId,
        ...
        Logger.error('Screenshare manager: ICE restart failed', {
          errorMessage: error.message,
          errorCode: error.code,
          sessionMetadata,
        });
