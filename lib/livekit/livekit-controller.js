...
class LiveKitController extends BaseManager {
  ...
  async _handleUpdateLiveKitParticipantPermissions (payload) {
    ...
    // Not silent: akka counts every send as an enforcement attempt and escalates on
    // that count, so a write dropped here skews the tally it eventually acts on.
    ...
    try {
      ...
      // Absent participant is the desired end state for a downgrade, not a failure, and
      // akka re-sends that on a timer. A restore is sent once, so anything else here is
      // a permanent loss for that user and has to stay at error. Keyed on the twirp
      // status rather than the message text, which is not a contract.
      const notFound = error?.code === 'not_found' || error?.status === 404
        || error?.message?.includes('participant does not exist');
      Logger[notFound ? 'debug' : 'error']('LiveKitController: Error handling UpdateLiveKitParticipantPermissionsSysMsg', {
        meetingId,
        userId,
        identity,
        canPublish: !!grant?.canPublish,
  ...
  async _handleGenerateLiveKitTokenReq (payload) {
    try {
      ...
      const room = this._hasRoom(meetingId)
        ? this._getRoom(meetingId)
        : await this._waitForRoom(meetingId);
      const baseMetadata = {
        ...metadata,
        bbb_meetingId: meetingId,
        bbb_voiceConf: metadata?.bbb_voiceConf || metadata?.voiceConf,
        bbb_audioBridge: metadata?.bbb_audioBridge || 'livekit',
        bbb_cameraBridge: metadata?.bbb_cameraBridge,
        bbb_screenShareBridge: metadata?.bbb_screenShareBridge,
        bbb_domain: BBB_DOMAIN,
      };
      let extMetadata = {};
