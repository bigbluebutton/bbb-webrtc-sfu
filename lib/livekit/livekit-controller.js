...
class LiveKitController extends BaseManager {
  ...
  async _handleScreenBroadcastStop ({ meetingId, userId, streamId, voiceConf }) {
    ...
    try {
      const participant = await this.roomServiceClient.getParticipant(meetingId, identity);
      const permissions = participant?.permission || {};
      ...
      // Force screen share to be unpublished
      await this.roomServiceClient.updateParticipant(meetingId, identity, {
        permission: {
          ...permissions,
          canPublish: true,
          canPublishSources: [this.liveKitRTC.TrackSource.SOURCE_MICROPHONE, this.liveKitRTC.TrackSource.SOURCE_CAMERA],
        },
      });
      // Restore screen share permission for the participant
      await this.roomServiceClient.updateParticipant(meetingId, identity, {
        permission: {
          ...permissions,
          canPublish: true,
