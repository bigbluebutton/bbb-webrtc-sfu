...
class LiveKitController extends BaseManager {
  ...
  async stop () {
    ...
    if (this.webrtcRecorderManager) this.webrtcRecorderManager.stop();
  ...
  async _handleMuteUserInVoiceConf (payload) {
    ...
    const lkUserId = isWebUser(userId)
      ? userId
      : isLkBBBSipUser(userId)
        ? bbbIdToLkSipId(userId)
        : userId;
    ...
    try {
      ...
      const participant = await this.roomServiceClient.getParticipant(meetingId, lkUserId);
      ...
      await Promise.all(targetTracks.map((track) => {
        return this.roomServiceClient.mutePublishedTrack(meetingId, lkUserId, track.sid, mute);
      }));
  ...
  async _handleCamBroadcastStop ({ meetingId, userId, streamId }) {
    try {
      ...
      await this.roomServiceClient.mutePublishedTrack(meetingId, userId, targetTrack.sid, true);
