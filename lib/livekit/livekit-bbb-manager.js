...
// How long a web user's camera stop is held back after LiveKit unpublishes
// the track. A client that reconnects republishes the same camera automatically
// within a timeframe, and having the grace period allows that to be seamless.
// Removing it at once makes akka-apps eject any republished cameras unecessarily.
// Cameras are still cleaned up on the stop in other scenarios (voluntary unshare,
// user leave triggers etc)
const DEFAULT_CAM_STOP_GRACE_MS = 15000;
...
class LiveKitBbbManager {
  ...
  constructor(
    ...
    roomMap, {
      ...
      camStopGraceMs = DEFAULT_CAM_STOP_GRACE_MS,
    } = {},
  ) {
    ...
    this._camStopGraceMs = camStopGraceMs;
    this._pendingCamStops = new Map();
  }

  /**
   * @param {string} meetingId - The meeting ID (room name).
   * @param {string} streamId - The BBB camera stream ID.
   * @returns {string} The key a held camera stop is filed under.
   * @private
   */
  static _camStopKey(meetingId, streamId) {
    return `${meetingId}|${streamId}`;
  }

  /**
   * Drops a held camera stop, if any.
   * @param {string} meetingId - The meeting ID (room name).
   * @param {string} streamId - The BBB camera stream ID.
   * @returns {boolean} Whether a stop was being held.
   * @private
   */
  _cancelPendingCamStop(meetingId, streamId) {
    const key = LiveKitBbbManager._camStopKey(meetingId, streamId);
    const timer = this._pendingCamStops.get(key);

    if (!timer) return false;

    clearTimeout(timer);
    this._pendingCamStops.delete(key);

    return true;
  }

  /**
   * Holds a web user's camera stop for the grace period, replacing any
   * stop already held for the same stream.
   * @param {string} meetingId - The meeting ID (room name).
   * @param {string} userId - The user ID.
   * @param {string} streamId - The BBB camera stream ID.
   * @private
   */
  _deferCamBroadcastStopped(meetingId, userId, streamId) {
    const key = LiveKitBbbManager._camStopKey(meetingId, streamId);

    this._cancelPendingCamStop(meetingId, streamId);

    const timer = setTimeout(() => {
      this._pendingCamStops.delete(key);

      if (!this._getRoom(meetingId)) return;

      Logger.info('LiveKitBbbManager: camera not republished within grace, stop sent', {
        roomName: meetingId,
        userId,
        streamId,
        graceMs: this._camStopGraceMs,
      });
      this._sendCamBroadcastStoppedInSfuEvtMsg(meetingId, userId, streamId);
    }, this._camStopGraceMs);

    this._pendingCamStops.set(key, timer);
  }

  /**
   * Drops every camera stop still held back for a meeting. Nothing is sent:
   * the meeting is gone, and so are its streams.
   * @param {string} meetingId - The meeting ID (room name).
   */
  clearPendingCamStops(meetingId) {
    const prefix = `${meetingId}|`;

    this._pendingCamStops.forEach((timer, key) => {
  ...
  clearAllPendingCamStops() {
    this._pendingCamStops.forEach((timer) => clearTimeout(timer));
    this._pendingCamStops.clear();
  }
  ...
  async handleParticipantJoined(event) {
    try {
      ...
      // Non-primary memberships already hold a primary voice presence
      // elsewhere. Primary is the authoritative state, discard this.
      if (metadata?.bbb_roomPurpose && metadata.bbb_roomPurpose !== 'primary') {
        Logger.debug('LiveKitBbbManager: handleParticipantJoined non-primary, skipping voice-conf emit', {
          userId,
          kind,
          purpose: metadata.bbb_roomPurpose,
          roomName,
        });
        return;
      }
  ...
  async handleParticipantLeft(event) {
    try {
      ...
      // Non-primary participants have no voice presence in this room's
      // voiceConf, so the left emit would do no good.
      // LiveKitParticipantLeftEvtMsg still goes out (room-keyed) so akka can
      // react to the disconnect.
      const isNonPrimary = participantMetadata?.bbb_roomPurpose
        && participantMetadata.bbb_roomPurpose !== 'primary';

      if (!isNonPrimary && (usingAudioBridge || !webUser)) {
  ...
  _handleMicrophoneTrackPublished(event, participantMetadata) {
    // Non-primary memberships already hold a primary voice presence
    // elsewhere. Primary is the authoritative state, discard this.
    if (participantMetadata?.bbb_roomPurpose
      && participantMetadata.bbb_roomPurpose !== 'primary') {
      return;
    }
  ...
  async _handleCameraTrackPublished(event) {
    // No bbb_roomPurpose check here (unlike the mic/join handlers): non-primary
    // grants are mic-only (canPublishSources, LK-enforced), so cam/screen
    // events can only come from primary participants today. Check this too
    // like we do for audio before any purpose gains camera/screen publish rights.
    ...
    try {
      ...
      if (isWebUser(participantIdentity)) {
        // The web client announces its own camera to akka-apps. Just cleanup
        // any held stop requests in case this is reconnection re-publish.
        if (this._cancelPendingCamStop(event.room?.name, event.track?.name)) {
          Logger.info('LiveKitBbbManager: camera republished within grace, stop dropped', {
            roomName: event.room?.name,
            identity: participantIdentity,
            streamId: event.track?.name,
  ...
  async _handleScreenShareTrackPublished(event, participantMetadata) {
    // Room purpose check note: see _handleCameraTrackPublished.
  ...
  async handleTrackPublished(event) {
    ...
    try {
      ...
      if (trackSource === 'camera' && isWebUser(event.participant?.identity)) {
        this._handleCameraTrackPublished(event);
        return;
      }
  ...
  _handleCameraTrackUnpublished(event) {
    ...
    try {
      ...
      // A web client stops its own camera over GraphQL before it unpublishes,
      // so for it this event is either an echo of that or the participant
      // being torn down. The server does the latter on every reconnect it
      // forces, and the client republishes the same stream a moment later;
      // a stop sent now would end the camera the client is bringing back.
      if (isWebUser(event.participant?.identity) && this._camStopGraceMs > 0) {
        this._deferCamBroadcastStopped(event.room?.name, userId, streamId);
        return;
      }
  ...
  async handleTrackUnpublished(event) {
    try {
      ...
      if (trackSource === 'camera') {
        this._handleCameraTrackUnpublished(event);
        return;
      }
