...
const LiveKitRTC = require('@livekit/rtc-node');
...
const validBBBMetadata = (metadata = {}) => {
  return !isMetadataEmpty(metadata)
    && metadata?.bbb_meetingId
    && metadata?.bbb_voiceConf
};
...
const isUsingCameraBridge = (metadata) => {
  return metadata?.bbb_cameraBridge === 'livekit';
}

const isUsingScreenShareBridge = (metadata) => {
  return metadata?.bbb_screenShareBridge === 'livekit';
}
  ...
  return new Promise((resolve) => {
    const onRecordingStatusReply = (payload) => {
      if (payload.requestedBy === userId) {
        ...
        // Depending on the BBB version, recordUserCameras|Audio|ScreenShare may
        // be undefined. Parse it to a boolean with default true.
        payload.recordUserCameras = typeof payload?.recordUserCameras === 'boolean'
          ? payload.recordUserCameras
          : true;
        payload.recordUserAudio = typeof payload?.recordUserAudio === 'boolean'
          ? payload.recordUserAudio
          : true;
        payload.recordUserScreenShare = typeof payload?.recordUserScreenShare === 'boolean'
          ? payload.recordUserScreenShare
          : true;
        resolve(payload);
      }
    };
...
/*
 * Extracts the participant name from a LiveKit WebHooks event.
 *
 * If the participant's kind is SIP, it'll either return the phone number if it
 * exists in the participant's attributes, or the participant's name or
 * identity with the "Phone " prefix stripped.
 *
 * For any other participant kinds, it will return the participant's name or
 * identity as is.
 *
 * This function throws if no participant name can be found.
 */
const getParticipantNameFromEvent = (event = {}) => {
  const { participant = {} } = event;
  const { kind = 'unknown', identity, attributes = {} } = participant;

  let participantName = participant?.name || identity;

  if (isSipParticipant(kind)) {
    // If phoneNumber is present in attributes, masking is not applied. Use it.
    if (attributes['sip.phoneNumber']) {
      participantName = attributes['sip.phoneNumber'];
    } else {
      // Masking is active for SIP participants. Use participantName || identity,
      // but strip the "Phone " prefix if it exists. It's a hardcoded prefix from
      // LK which we do not want to show in BBB.
      participantName = participantName?.replace(/^Phone\s+/i, '');
    }
  }

  if (!participantName) throw new Error('No participant name found');
...
const trackSourceToString = (sdk, source) => {
  if (typeof source === 'string') return source;

  return sdk.trackSourceToString(source);
}

const isDisconnectReasonUnexpected = (reason) => {
  switch (reason) {
    case LiveKitRTC.DisconnectReason.CLIENT_INITIATED:
    case LiveKitRTC.DisconnectReason.DUPLICATE_IDENTITY:
    case LiveKitRTC.DisconnectReason.PARTICIPANT_REMOVED:
    case LiveKitRTC.DisconnectReason.ROOM_DELETED:
    case LiveKitRTC.DisconnectReason.ROOM_CLOSED:
      return false;
    default:
      return true;
  }
