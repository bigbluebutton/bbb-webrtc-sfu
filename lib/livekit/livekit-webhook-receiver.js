...
class LiveKitWebhookReceiver extends EventEmitter {
  ...
  async _handleParticipantJoined(event) {
    ...
    const { identity: participantIdentity, kind } = event.participant;

    if (!isFrontendParticipant(kind)) {
      Logger.trace('LiveKitWebhookReceiver: _handleParticipantJoined ignored due to kind', {
        kind,
        event,
      });
      return;
  ...
  async _handleParticipantLeft(event) {
    ...
    if (!isFrontendParticipant(kind)) return;
