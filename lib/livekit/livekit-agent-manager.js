...
class LiveKitAgentManager {
  ...
  async _handleRoomStarted(event) {
    try {
      ...
      Logger.debug('LiveKitAgentManager: Creating agent for room', {
        agent: identity,
        roomName,
      });
      ...
      if (this._autoSubscribe) {
        room
          ...
          .on(this._liveKitRTC.RoomEvent.LocalTrackPublished, (track) => {
            Logger.debug('LiveKitAgentManager: LocalTrackPublished', {
              agent: identity,
              track,
            });
          })
          .on(this._liveKitRTC.RoomEvent.LocalTrackUnpublished, (track) => {
            Logger.debug('LiveKitAgentManager: LocalTrackUnpublished', {
              agent: identity,
              track,
            });
          })
  ...
  async stop() {
    ...
    for (const [roomName, agent] of this._agentMap.entries()) {
      const identity = `bbb-webrtc-sfu/${roomName}`;
      Logger.debug('LiveKitAgentManager: Disconnecting agent during stop', {
        agent: identity,
        roomName,
      });
      disconnectPromises.push(agent.disconnect().catch(error => {
        Logger.warn('LiveKitAgentManager: Error disconnecting agent during stop', {
          agent: identity,
          roomName,
          error: error?.message,
          stack: error?.stack,
