...
class LiveKitAgentManager {
  ...
  constructor(
    ...
    // Set<roomName> — tracks rooms with an agent creation in progress
    // to prevent races when multiple SIP participants join concurrently.
  ...
  _validParticipant(remoteParticipant) {
    ...
    const frontendParticipant = isFrontendParticipant(
      ...
      remoteParticipant?.permissions?.hidden ?? remoteParticipant?.permission?.hidden,
  ...
  init() {
    this._eventBus.on('participant_joined', this._handleParticipantJoined);
    this._eventBus.on('room_finished', this._handleRoomEnded);

    Logger.info('LiveKitAgentManager initialized');
  }

  // Only creates an agent when a non-web participant (SIP, voice) joins.
  // Web users handle talking detection client-side; the server-side agent
  // is only needed for SIP/voice participants that lack browser-side detection.
  async _handleParticipantJoined(event) {
    try {
      const roomName = event.room?.name;
      // Only trigger agent creation for non-web frontend participants (SIP, voice)
      if (!this._validParticipant(event.participant)) return;
      ...
      if (this.hasAgent(roomName)) return;
      ...
      await this._startAgent(roomName);
    } catch (error) {
      Logger.error('LiveKitAgentManager: Error handling participant joined', {
        error: error.message,
        stack: error.stack,
        event,
      });
  ...
  async _startAgent(roomName) {
    if (this.hasAgent(roomName) || this._pendingAgents.has(roomName)) return;

    this._pendingAgents.add(roomName);
    ...
    try {
      ...
      // Subscribe to tracks already published before the agent joined.
      // Without this, tracks published before connect() would be missed
      // since TrackPublished only fires for NEW publications.
      if (!this._autoSubscribe) {
        for (const participant of room.remoteParticipants.values()) {
          for (const publication of participant.trackPublications.values()) {
            this._handleTrackPublished(publication, participant);
          }
        }
      }
      ...
      this._pendingAgents.delete(roomName);
    }
  }
  ...
  async _handleTrackPublished(remotePublication, remoteParticipant) {
    ...
    const participantMetadata = parseLiveKitMetadata(remoteParticipant);
  ...
  async stop() {
    this._eventBus.removeListener('participant_joined', this._handleParticipantJoined);
