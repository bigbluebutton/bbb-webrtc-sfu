...
class LiveKitWebRTCRecorderManager {
  ...
  async shouldRecord(roomName, participantId) {
    const {
      recorded,
      recording,
  ...
  async _startRecording(recordingInfo) {
    ...
    const { shouldRecord, recordFullDurationMedia } = await this.shouldRecord(
      meetingId,
      userId,
    );
  ...
  _scheduleRecordingRetry(recordingInfo) {
    ...
    const trackAndRoomActive = () => this.isTrackActive(trackId, { roomId: meetingId })
      && this._roomRecordingStatusMap.get(meetingId);
  ...
  _handleRecordingRtpStatusChanged(event) {
    ...
    switch (status) {
      case 'flowing': {
        const isRoomRecording = this._roomRecordingStatusMap.get(meetingId);
        const isTrackActive = this.isTrackActive(trackId, { roomId: meetingId });
        this._resetRetryCount(trackId);

        if (!isRoomRecording || !isTrackActive) {
        ...
        if (!recordingInfo.startEventSent && isRoomRecording && isTrackActive) {
          switch (source) {
            case 'microphone':
            case 'screen_share_audio': {
              writeAudioRecordingStartEvent(
