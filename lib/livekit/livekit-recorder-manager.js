...
class LiveKitWebRTCRecorderManager {
  ...
  async _startRecording(recordingInfo) {
    ...
    if (meetingId == null) {
      // _participantMap is Map<roomName, Map<identity, info>>, search all rooms
      let participant;
      for (const [, inner] of this._participantMap) {
        const found = inner.get(userId);
        if (found) { participant = found; break; }
      }
