    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        switch (type) {
          case "RtpEndpoint":
          case "WebRtcEndpoint":
            session = user.addSdp(params.descriptor, type, params, params.adapter, params.name);
            answer = await user.startSession(session.id);
            break;
          case "URI":
            session = user.addUri(params.descriptor, type, params, params.adapter, params.name);
            answer = await user.startSession(session.id);
            break;
