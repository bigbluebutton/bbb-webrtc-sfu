...
const MR = class MCSRouter extends EventEmitter {
  ...
  _dispatchEvents() {
    ...
    this.emitter.on(C.EVENT.ROOM_CREATED, event => {
      ...
      if (client) {
        Logger.error("PORRA DUM CARALHO", event, typeof client);
    ...
    this.emitter.on(C.EVENT.MEDIA_DISCONNECTED, event => {
      const { roomId } = event;
      const client = this._getClientToDispatch(roomId, C.EVENT.MEDIA_DISCONNECTED);
