...
const MR = class MCSRouter extends EventEmitter {
  ...
  async setConferenceFloor(args) {
    try {
      const { media_id, room_id } = args
      await this._mediaController.setConferenceFloor(room_id, media_id)
    }
    catch (error) {
  ...
  async setContentFloor(args) {
    try {
      const { room_id, media_id } = args
      await this._mediaController.setContentFloor(room_id, media_id)
    }
    catch (error) {
  ...
  async releaseConferenceFloor(args) {
    try {
      const { media_id, room_id } = args
      await this._mediaController.releaseConferenceFloor(room_id, media_id)
    }
    catch (error) {
  ...
  async releaseContentFloor(args) {
    try {
      const { room_id, media_id } = args
      await this._mediaController.releaseContentFloor(room_id, media_id)
    }
    catch (error) {
  ...
  setupClient(client) {
    ...
    client.on('setConferenceFloor', async (args) => {
      let transactionId, media_id, room_id;
      try {
        ({transactionId, media_id, room_id} = args);
        await this.setConferenceFloor(args)
        client.conferenceFloorChanged(room_id, media_id, {transactionId})
      }
      catch (e) {
        this._notifyMethodError(client, e, transactionId)
      }
    ...
    client.on('setContentFloor', async (args) =>{
      ...
      try {
        ...
        client.contentFloorChanged(room_id, media_id, {transactionId})
      }
      catch (e) {
        this._notifyMethodError(client, e, transactionId)
      }
    ...
    client.on('releaseConferenceFloor', async (args) => {
      ...
      try {
        ({ transactionId, media_id, room_id } = args);
        await this.releaseConferenceFloor(args)
        client.conferenceFloorChanged(room_id, media_id, {transactionId})
      }
      catch (e) {
        this._notifyMethodError(client, e, transactionId)
