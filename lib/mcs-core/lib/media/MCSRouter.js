...
const MR = class MCSRouter extends EventEmitter {
  ...
  async setContentFloor(args) {
    try {
      const {room_id, room_id} = args
      await this._mediaController.setContentFloor(room_id, room_id)
    }
    catch (error) {
  ...
  async releaseContentFloor(args) {
    try {
      const {room_id, room_id} = args
      await this._mediaController.releaseContentFloor(room_id, room_id)
    }
    catch (error) {
  ...
  setupClient(client) {
    ...
    client.on('stopRecording', async (args) => {
      ...
      client.on('setConferenceFloor', async (args) => {
        try {
          const {transactionId, media_id, room_id} = args;
