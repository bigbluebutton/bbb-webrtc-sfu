  ...
  setupClient(client) {
    ...
    try {
      ...
      client.on('startRecording', async (args) => {
        const { source_id, media_id, recording_path } = args;
        await this.startRecording(source_id, media_id, recording_path);
      });

      client.on('stopRecording', async (args) => {
        const { source_media_id } = args;
        await this.stopRecording(source_media_id);
      });
