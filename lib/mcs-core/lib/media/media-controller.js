...
module.exports = class MediaController {
  ...
  start () {
    ...
    GLOBAL_EVENT_EMITTER.on(C.EVENT.MEDIA_BBB_AUDIO_CONNECTED, this._handleBBBAudioMediaConnected.bind(this));
  }
  ...
  async _handleBBBAudioMediaConnected(event) {
    try {
      const { roomId, userId, userName, sdpOffer, sdpAnswer, media } = event;
      const room = await this.createRoom(roomId);
      const user = await this.createUser(roomId, 'SFU', {userId, name: userName});
      ...
      this.addMediaSession(session);
      GLOBAL_EVENT_EMITTER.emit(C.EVENT.MEDIA_CONNECTED, session.getMediaInfo());
    }
    catch (error) {
      reject(this._handleError(error));
    }
