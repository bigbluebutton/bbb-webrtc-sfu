...
module.exports = class MediaController {
  ...
  setVolume(mediaId, volume) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        Logger.info('[mcs-controller-setVolume] Fetched media', mediaSession.id);
  ...
  mute(mediaId) {
    return new Promise(async (resolve, reject) => {
      try {
        console.log("Mute ",mediaId);
        const mediaSession = this.getMediaSession(mediaId);
        Logger.info('[mcs-controller] Fetched media', mediaSession.id);
  ...
  unmute(mediaId) {
    return new Promise(async (resolve, reject) => {
      try {
        console.log("Unmute ",mediaId);
        const mediaSession = this.getMediaSession(mediaId);
        Logger.info('[mcs-controller] Fetched media', mediaSession.id);
        mediaSession.setVolume(mediaSession.volume);
        resolve();
      }
      catch (error) {
        reject(this._handleError(error));
