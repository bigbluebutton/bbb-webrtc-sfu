...
module.exports = class MediaController {
  ...
  removeRoom (roomId) {
    ...
    try {
      ...
      this.strategyManager.removeFromHandler(room.id, room.strategy);
      this.emitter.emit(C.EVENT.ROOM_DESTROYED, room.getInfo());
      room.destroy();
      removed = this.rooms.delete(roomId)
      Logger.debug(LOG_PREFIX, "Removed room", { roomId });
      ...
      if (!removed) this.rooms.delete(roomId);
      return false;
    }
  }
  ...
  removeUser (user) {
    ...
    if (this.users.delete(user.id) | this.users.delete(user.externalUserId)) {
      this.strategyManager.removeFromHandler(user.id, user.strategy);
  ...
  removeMedia (media) {
    const { id, strategy } = media;

    if (this.medias.delete(media.id)) {
      this.strategyManager.removeFromHandler(id, strategy);
    }
  }
