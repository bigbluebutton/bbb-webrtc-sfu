...
class MediaControllerC {
  ...
  _handleNewVideoFloor (event) {
    ...
    if (!mediaId) {
      try {
        ...
        Logger.error(`Error releasing conference floor at ${roomId}`,
      ...
      try {
        ...
        Logger.error(`Error setting conference floor at ${roomId}`,
          { errorMessage: error.message, errorCode: error.code });
      }
    }
  }
  ...
  join (roomId, type, params) {
    try {
      ...
      Logger.debug('User joined room', { userId: user.id, roomId, type });
      return user.id;
    } catch (e) {
  ...
  _ejectUser (userInfo) {
    ...
    try {
      ...
      Logger.info("User ejected", { userId, externalUserId, roomId });
    } catch (error) {
      if (error === C.ERROR.USER_NOT_FOUND) {
        Logger.debug('User with already left the room', {
          userId, externalUserId, roomId,
        });
      } else {
        Logger.error('Auto eject failed', {
  ...
  leave (roomId, userId) {
    ...
    if (user) {
      ...
      user.leave().then((killedMedias) => {
        Logger.info("User left", { userId, externalUserId });
        ...
        Logger.error("CRITICAL: exception on user leave cleanup", {
  ...
  isAboveGlobalMediaThreshold ({ mediaId, ignoreThresholds = false }) {
    if (GLOBAL_MEDIA_THRESHOLD > 0 && !ignoreThresholds) {
      ...
      if (!preExistantMedia && this.medias.size >= GLOBAL_MEDIA_THRESHOLD) {
        Logger.error(`Server has exceeded the media threshold`,
  ...
  async publishAndSubscribe (roomId, userId, sourceId, type, params = {}) {
    ...
    Logger.trace('Publish/Subscribe request', {
    ...
    try {
      ...
      Logger.error('PublishAndSubscribe failed: publish phase', {
    ...
    if (source) {
      try {
        ...
        Logger.warn('PublishAndSubscribe subscription failed', {
  ...
  async publish (userId, roomId, type, params = {}) {
    ...
    Logger.trace('Publish request', { userId, roomId, descriptor: params.descriptor });
  ...
  async startRecording (userId, sourceId, recordingPath, params = {}) {
    ...
    try {
      ...
      Logger.warn('startRecording failed', {
    ...
    try {
      ...
      switch (mappedEvent) {
        ...
        case C.EVENT.SUBSCRIBED_TO:
          // TODO refactor
          break;
        default: Logger.trace("Invalid event subscription", mappedEvent, identifier);
      }
    }
    catch (err) {
  ...
  createRoom (roomId)  {
    ...
    if (room == null) {
      ...
      Logger.info('New room created', { coreRoomInfo: room.getInfo() });
  ...
  removeRoom (roomId) {
    ...
    try {
      ...
      Logger.error("CRITICAL: Room deletion failed",
      ...
      if (!removed) {
        if (this.rooms.delete(roomId)) {
          MCSPrometheusAgent.set(METRIC_NAMES.ROOMS, this.getNumberOfRooms());
          Logger.info("CRITICAL: room destruction cleanup", { roomId });
        }
      }
      return false;
    }
  ...
  getUsers (roomId) {
    try {
      ...
      Logger.error(`getUsers failed for room ${roomId} due to ${error.message}`,
        { roomId, error });
      throw (this._handleError(error));
    }
  }
  ...
  setContentFloor (roomId, mediaId) {
    try {
      ...
      Logger.error(`setContentFloor for room ${roomId} as media ${mediaId} failed due to ${error.message}`,
  ...
  setConferenceFloor (roomId, mediaId) {
    try {
      ...
      Logger.error(`setConferenceFloor for room ${roomId} as media ${mediaId} failed due to ${error.message}`,
        { roomId, mediaId, error });
      throw (this._handleError(error));
    }
  }
  ...
  getContentFloor (roomId) {
    try {
      ...
      Logger.error(`getContentFloor for room ${roomId} failed due to ${error.message}`,
        { roomId, error });
      throw (this._handleError(error));
    }
  }
  ...
  getConferenceFloor (roomId) {
    try {
      ...
      Logger.error(`getConferenceFloor for room ${roomId} failed due to ${error.message}`,
        { roomId, error });
      throw (this._handleError(error));
    }
  }
  ...
  setVolume (mediaId, volume) {
    try {
      ...
      Logger.error(`setVolume for media$ ${mediaId} failed due to ${error.message}`,
  ...
  mute (mediaId) {
    try {
      ...
      Logger.error(`mute for media$ ${mediaId} failed due to ${error.message}`,
        { mediaId, error });
      throw (this._handleError(error));
    }
  }
  ...
  unmute (mediaId) {
    try {
      ...
      Logger.error(`unmute for media$ ${mediaId} failed due to ${error.message}`,
        { mediaId, error });
      throw (this._handleError(error));
    }
  }
  ...
  _getMemberByIdentifier (identifier) {
    try {
      ...
      Logger.debug("Media not found, falling back to user", e);
    }
    ...
    try {
      ...
      Logger.debug("User not found, falling back to room", e);
    }
    ...
    try {
      ...
      Logger.debug("Room not found, no valid member for", identifier);
  ...
  dtmf (mediaId, tone, options) {
    try {
      Logger.debug("Sending DTMF tone", { mediaId, tone });
      const mediaSession = this.getMediaSessionThrowable(mediaId);
  ...
  requestKeyframe (mediaId) {
    try {
      Logger.debug("Requesting keyframe from media", { mediaId });
      const mediaSession = this.getMediaSessionThrowable(mediaId);
  ...
  getMedias (memberType, identifier, options = {}) {
    try {
      Logger.info(`getMedias request for ${identifier}`, { memberType, identifier, options });
  ...
  async restartIce (mediaId) {
    try {
      Logger.debug(`Requesting an ICE restart for ${mediaId}`);
      const mediaSession = this.getMediaSessionThrowable(mediaId);
      return mediaSession.restartIce();
    } catch (error) {
      throw (this._handleError(error));
    }
  }
