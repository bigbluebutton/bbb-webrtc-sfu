...
const KMS_ARRAY = config.util.cloneDeep(config.get('kurento'));
...
class Balancer extends EventEmitter {
        ...
        if (retries < NOF_STARTUP_CONNECTION_RETRIES) {
          if (!this._hostStarted(url, ip)) {
            try {
              ...
              host.retries++;
              Logger.error('KMSBalancer: Failed to connect to media server',
                { url, ip, mediaType, retries });
              setTimeout(() => tryToConnect(host), HOST_RETRY_TIMER);
            }
          }
        } else {
          Logger.error('KMSBalancer: Maximum number of retries expired for media server',
            { url, ip, mediaType, retries });
        }
      };
  ...
  async getHost (mediaType = C.MEDIA_PROFILE.ALL) {
    ...
    Logger.info(`KMSBalancer: Got media server ${host.id}`, {
      hostId: host.id,
      url: host.url,
  ...
  addHost (host) {
    if (host) {
      ...
      Logger.info('KMSBalancer: New media server host added',
        { hostId: id, url: host.url, ip: host.ip, mediaType: host.mediaType });

      return;
    }

    Logger.warn('KMSBalancer: Undefined media server host on addHost, SHOULD NOT HAPPEN!');
  }
  ...
  decrementHostStreams (hostId, mediaType) {
    ...
    if (host) {
      ...
      Logger.info(`KMSBalancer: Media server ${mediaType} streams decremented`,
  ...
  incrementHostStreams (hostId, mediaType) {
    ...
    if (host) {
      host.medias[mediaType]++;
      Logger.info(`KMSBalancer: Media server ${mediaType} streams incremented`,
        { hostId: host.id, url: host.url, mediaType, [mediaType]: host.medias[mediaType] });
    }
  }
  ...
  _monitorConnectionState (host) {
    ...
    try {
      ...
      Logger.error('KMSBalancer: Error when trying to monitor media server',
  ...
  _onDisconnection (host) {
    try {
      ...
      Logger.error('KMSBalancer: Media server disconnected',
      ...
      Logger.error('KMSBalancer: Error trying to handle media server disconnection',
  ...
  _onReconnection (sameSession, host) {
    if (!sameSession) {
      Logger.warn('KMSBalancer: Media server reconnected, not same session',
  ...
  _reconnectToServer (host) {
    ...
    Logger.info('KMSBalancer: Reconnecting to media server',
      ...
      this._reconnectionRoutine[id] = setInterval(async () => {
        try {
          ...
          Promise.race([connect, failOver]).then(h => {
            ...
            this.addHost(h);
            Logger.warn('KMSBalancer: Reconnection to media server succeeded',
              { hostId: id, url, mediaType: host.mediaType });
          }).catch(error => {
            Logger.error('KMSBalancer: Failed to reconnect to media server',
