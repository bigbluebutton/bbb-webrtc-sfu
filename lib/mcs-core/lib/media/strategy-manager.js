...
class StrategyManager {
  ...
  _parseMemberInfo (memberInfo) {
    ...
    Logger.trace(LOG_PREFIX, "Parsing member", memberInfo, memberType);
  ...
  getHandler (roomId, strategy) {
    ...
    const handler = roomHandlers.find(sh => sh.name === strategy);
  ...
  addToHandler (memberInfo) {
    ...
    try {
      const member = this._parseMemberInfo(memberInfo);
    } catch (e) {
