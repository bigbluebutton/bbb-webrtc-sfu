  ...
  _reviewMeetingFloors (floor, previousFloors) {
    ...
    if (mediaSessions.length > 0) {
      mediaSessions.forEach(ms => {
        const sinkMediaSession = this.room.getMediaSession(ms.mediaSessionId);
        this._connectMediaFloors(sinkMediaSession, floor, previousFloors);
      });
    }
  }

  _connectMediaFloors (sinkMediaSession, floorMediaSession, previousFloors) {
    if (sinkMediaSession) {
      let sinkMedias = sinkMediaSession.medias;
      // Get only the video medias from the session  that are capable of receiving
      let availableSinkMedias = sinkMedias.filter(sm => {
        const { mediaTypes } = sm;
        if (mediaTypes.video && (mediaTypes.video === 'sendrecv' || mediaTypes.video === 'recvonly')) {
          return true;
        }
