    ...
    res.media.forEach(ml => {
      if(ml.type == 'video') {
        ml.fmtp.forEach(fmtp => {
          ...
          // TODO: in the future, add mbps, fs and br specs for content media too
          if (specCodec === 'H264' && type === 'main') {
            let configProfile = "profile-level-id=" + spec[specCodec].profile_level_id +
              "; max-mbps=" + spec[specCodec].max_mbps + "; max-fs=" + spec[specCodec].max_fs +
              "; max-br=" + spec[specCodec].max_br;
