...
module.exports = class SdpWrapper {
  ...
  getDirection (type) {
    ...
    const fetchDirection = (_media) => {
      if (_media == null || _media.direction == null) {
        return 'sendrecv';
      } else  {
        return _media.direction;
      }
    }
  ...
  submitToSpec (sdp, spec) {
    ...
    res = SdpWrapper.filterByAudioCodec(res, audioCodec, this.mediaProfile, codecPriorities);
  ...
  processSdp () {
    this._plainSdp = this.submitToSpec(this._plainSdp, this.mediaSpecs, this.mediaProfile);
  ...
  static filterByAudioCodec (sdp, audioCodec, mediaProfile, codecPriorities = {}) {
    let res = typeof sdp === 'string'? transform.parse(sdp) : sdp;
    let { audioPrioCodec } = codecPriorities;
    let validPayloads;
    ...
    res.media.forEach((ml, index) => {
      let codecToFilter = getCodecToFilter(audioCodec, audioPrioCodec, ml);

      if (ml.type === 'audio' && codecToFilter !== 'ANY') {
        // Audio: filter by @codec
