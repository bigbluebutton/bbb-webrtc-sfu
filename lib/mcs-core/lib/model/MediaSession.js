...
module.exports = class MediaSession {
  ...
  async stop () {
    if (this._status === C.STATUS.STARTED) {
      ...
      try {
        ...
        this.emitter.emit(C.EVENT.MEDIA_DISCONNECTED, { mediaId: this.id });
  ...
  async connect (sink, type = 'ALL') {
    try {
      ...
      if (type === 'AUDIO') {
        const adapter = this._audioAdapter;
        const sourceElement = this.audioMediaElement;
        const sinkElement = sink.audioMediaElement;
      } else {
        const adapter = this._videoAdapter;
        const sourceElement = this.videoMediaElement;
        const sinkElement = sink.videoMediaElement;
      }
