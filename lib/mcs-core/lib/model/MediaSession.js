...
module.exports = class MediaSession {
  ...
  async start () {
    ...
    try {
      ...
      if (this._isComposedAdapter) {
        if (this._adapter.video === C.STRING.KURENTO) {
          let { mediaElement, host } = await this._videoAdapter.createMediaElement(this.room, this._type, this._options);

          this.videoMediaElement = mediaElement;
          this.videoHost = host;
        } else {
          this.videoMediaElement = await this._videoAdapter.createMediaElement(this.roomId, this._type, this._options);
        }

        if (this._adapter.audio === C.STRING.KURENTO) {
          let { mediaElement, host } = await this._audioAdapter.createMediaElement(this.room, this._type, this._options);

          this.audioMediaElement = mediaElement;
          this.audioHost = host;
        } else {
          this.audioMediaElement = await this._audioAdapter.createMediaElement(this.roomId, this._type, this._options);
        }
        ...
        if (this._adapter === C.STRING.KURENTO) {
          let { mediaElement, host } = await this._videoAdapter.createMediaElement(this.room, this._type, this._options);
      ...
      try {
        ...
        if (this.hasVideo) {
          if (this._isComposedAdapter && this._adapter.video === C.STRING.KURENTO ||
            ...
            Balancer.decrementHostStreams(this.videoHost.id, 'video');
          }
        }

        if (this.hasAudio) {
          if (this._isComposedAdapter && this._adapter.audio === C.STRING.KURENTO) {
            Balancer.decrementHostStreams(this.audioHost.id, 'audio');
          } else {
            Balancer.decrementHostStreams(this.videoHost.id, 'audio');
          }
        }
