...
module.exports = class MediaSession {
  constructor (
    ...
    if (this._isComposedAdapter) {
      this._audioAdapter = MediaSession.getAdapter(this._adapter.audio, emitter);
      this._videoAdapter = MediaSession.getAdapter(this._adapter.video, emitter);
    } else {
      this._audioAdapter = MediaSession.getAdapter(this._adapter, emitter);
      this._videoAdapter = MediaSession.getAdapter(this._adapter, emitter);
    }
  ...
  static getAdapter (adapter, emitter) {
    ...
    switch (adapter) {
      case C.STRING.FREESWITCH:
        obj = new Freeswitch(Balancer, emitter);
        break;
      default:
        obj = new Kurento(Balancer, emitter);
        break;
    }
