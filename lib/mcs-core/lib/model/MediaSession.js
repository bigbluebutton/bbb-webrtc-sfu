...
module.exports = class MediaSession {
  ...
  _dispatchMediaStateEvent (event) {
    ...
    stateEvent.mediaId = this.id;

    if (!this._mediaStateSubscription) {
      Logger.debug("[mcs-media-session] Media session", this.id, "queuing event", stateEvent);
      this.eventQueue.push(stateEvent);
    }
    else {
      Logger.trace("[mcs-media-session] Dispatching", stateEvent);
      this.emitter.emit(C.EVENT.MEDIA_STATE.MEDIA_EVENT, stateEvent);
    }
  }

  _dispatchIceCandidate (event) {
    const iceEvent = event;
    iceEvent.mediaId = this.id;

    if (!this._iceSubscription) {
      Logger.debug("[mcs-media-session] Media session", this.id, "queuing event", iceEvent);
      this.outboundIceQueue.push(iceEvent);
    }
    else {
      Logger.trace("[mcs-media-session] Dispatching ICE", iceEvent);
      this.emitter.emit(C.EVENT.MEDIA_STATE.ICE, iceEvent);
    }
  }
