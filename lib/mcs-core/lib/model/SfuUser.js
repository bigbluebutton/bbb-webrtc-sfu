...
module.exports = class SfuUser extends User {
  ...
  addSdp (sdp, type, options, adapter = C.STRING.KURENTO, name = C.STRING.DEFAULT_NAME) {
    // TODO switch from type to children SdpSessions (WebRTC|SDP)
    let session = new SdpSession(this.emitter, sdp, this.roomId, type, adapter, name, options);
