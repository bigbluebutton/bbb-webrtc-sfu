...
module.exports = class SfuUser extends User {
  ...
  async addUri (uri, type) {
      ...
      session.emitter.once(C.EVENT.MEDIA_DISCONNECTED, (sessId) => {
  ...
  addSdp (sdp, type, options) {
    ...
    const session = new SdpSession(this.emitter, sdp, this.roomId, this.id, type, options);
    session.emitter.on(C.EVENT.MEDIA_DISCONNECTED, (sessId) => {
      if (sessId === session.id) {
