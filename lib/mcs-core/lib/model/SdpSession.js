...
module.exports = class SdpSession extends MediaSession {
  ...
  _updateHostLoad () {
    if (this._offer && this._answer.hasAvailableVideoCodec()) {
      Balancer.incrementHostStreams(this.videoHost.id, 'video');
      this.hasVideo = true;
    }

    if (this._offer && this._answer.hasAvailableAudioCodec()) {
      if (this._isComposedAdapter && this._adapter.audio === C.STRING.KURENTO) {
        Balancer.incrementHostStreams(this.audioHost.id, 'audio');
      } else {
        Balancer.incrementHostStreams(this.videoHost.id, 'audio');
      }
