...
module.exports = class RecordingSession extends MediaSession {
  constructor(room, user, recordingPath, options) {
    ...
    this.fillMediaTypes();
    Logger.info("Recording session: created", this.getMediaInfo());
  }
  ...
  async process () {
    try {
      ...
      Logger.debug(`Recording session: started ${this.id}`,
        { mediaInfo: this.getMediaInfo() });
      return this.id;
    } catch (error) {
      Logger.error(`Recording session: start failed for ${this.id}`,
