...
module.exports = class Media {
  ...
  async _connect (sink, sourceMediaId, mediaProfile, connectionType) {
    Logger.trace(LOG_PREFIX, "Adapter elements to be connected", this.adapterElementId, "=>", sink.adapterElementId);
    try {
      this.adapter.connect(
        this.adapterElementId,
        sink.adapterElementId,
        connectionType,
      );
    } catch (err) {
  ...
  async _disconnect (sink, sourceMediaId, mediaProfile, connectionType) {
    Logger.trace(LOG_PREFIX, "Adapter elements to be disconnected", this.adapterElementId, "=X>", sink.adapterElementId);
    try {
