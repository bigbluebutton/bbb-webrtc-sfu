...
module.exports = class Media extends EventEmitter {
  ...
  set subscribedTo (mediaId) {
    ...
    if (media || mediaId === '') {
      ...
      Logger.warn(LOG_PREFIX, `Source media was not found on subscribedTo change ${JSON.stringify({ subscriberId: this.id, sourceId: mediaId })}`);
    }
  }
  ...
  async trackMedia () {
    ...
    Logger.info(LOG_PREFIX, "Tracking media UNIT events", {
      mediaId: this.id, mediaSessionId: this.mediaSessionId, roomId: this.roomId, adapterElementId: this.adapterElementId
    });
  }
  ...
  async _connect (sink, connectionType) {
    ...
    connSinks.forEach(async sim => {
      ...
      Logger.info(LOG_PREFIX, `Adapter elements to be connected`, logObject);
  ...
  _sendBufferedEvents (eventName) {
    ...
    eventBuffer.forEach(event => {
      Logger.trace(LOG_PREFIX, `Send buffered ${eventName}`,
        { mediaSessionId: this.mediaSessionId, mediaId: this.id, eventName, event });
  ...
  _dispatchConferenceNewVideoFloor () {
    ...
    Logger.info(LOG_PREFIX, `Dispatching conference new video floor event for ${this.id}`,  { event });
