...
module.exports = class Media extends EventEmitter {
  static _getValidConnectionUnits(allUnits, connectionType, directionToFilter) {
    let connSinks = [];

    switch (connectionType) {
      case C.CONNECTION_TYPE.AUDIO:
        connSinks = allUnits.filter(target =>
          target.mediaTypes.audio && target.mediaTypes.audio !== directionToFilter
        );
        break;
      case C.CONNECTION_TYPE.VIDEO:
        connSinks = allUnits.filter(target =>
          target.mediaTypes.video && target.mediaTypes.video !== directionToFilter
        );
        break;
      case C.CONNECTION_TYPE.CONTENT:
        connSinks = allUnits.filter(target =>
          target.mediaTypes.content && target.mediaTypes.content !== directionToFilter
        );
        break;
  ...
  set subscribedTo (mediaId) {
    ...
    if (media || mediaId === '') {
      ...
      Logger.warn("Media: not found on subscribedTo change", {
  ...
  async trackMedia () {
    ...
    this.adapter.on(C.EVENT.MEDIA_SERVER_OFFLINE, this.onHostOffline);
  ...
  stop () {
    if (this.status === C.STATUS.STARTED || this.status === C.STATUS.STARTING) {
      ...
      try {
        ...
        Logger.info("Media: unit stopped", this.getMediaInfo());
        ...
        this.adapter.removeListener(C.EVENT.MEDIA_SERVER_OFFLINE, this.onHostOffline);
  ...
  async connect (sink, type = 'ALL') {
    try {
      Logger.debug("Media: connecting endpoints", {
  ...
  async consume (source, type = 'ALL') {
    try {
      Logger.debug("Media: consuming", {
        sinkId: this.id, sourceId: source.id, type
      });
  ...
  async _consume (source, connectionType) {
    const sources = source.medias? source.medias : [source];
    const connSources= Media._getValidConnectionUnits(sources, connectionType, 'recvonly');

    return Promise.all(connSources.map(async som => {
      try {
        this.localDescriptor = await this.adapter.consume(
          this.adapterElementId,
          som.adapterElementId,
          connectionType,
        );

        // Update the sink's source data
        this.subscribedTo = som.id;
      } catch (error) {
        Logger.error('Media: failed to run consume procedure', {
          sinkId: this.id, sourceId: som.id, errorMessage: error.message,
        });
      }
    }));
  }
  ...
  async _connect (sink, connectionType) {
    ...
    const connSinks = Media._getValidConnectionUnits(sinks, connectionType, 'sendonly');
    ...
    connSinks.forEach(async sim => {
      ...
      Logger.debug("Media: adapter elements to be connected", logObject);
      ...
      try {
        ...
        Logger.error(`Media: failed to run underlying _connect procedure due to ${error.message}`,
          { ...logObject, error });
      }
    });
  }
  ...
  async disconnect (sink, type = 'ALL') {
    try {
      Logger.debug("Media: disconnecting endpoints", {
        sourceId: this.id, sinkId: sink.id, type
      });
  ...
  async _disconnect (sink, connectionType) {
    ...
    sinkMediasToDisconnect.forEach(async sim => {
      Logger.debug("Media: adapter elements to be disconnected", {
        sourceId: this.id,
        sourceAdapterElementId: this.adapterElementId,
        sinkId: sim.id,
        sinkAdapterElementId: sim.adapterElementId,
  ...
  onEvent (eventName) {
    switch (eventName) {
      ...
      case C.EVENT.MEDIA_STATE.ICE:
        ...
        break;
      default: Logger.trace(`Media: unknown event subscription ${eventName}`);
    }
  }
  ...
  _dispatchMediaStateEvent (event) {
    ...
    Logger.trace(`Media: dispatched ${C.EVENT.MEDIA_STATE.MEDIA_EVENT}`,
  ...
  _dispatchIceCandidate (event) {
    ...
    Logger.trace(`Media: dispatched ${C.EVENT.MEDIA_STATE.ICE}`,
      { mediaSessionId: this.mediaSessionId, mediaId: this.id, event: normEvent });
  ...
  dtmf (tone, options) {
    return this.adapter.dtmf(this.adapterElementId, tone, options);
  }
  ...
  async restartIce () {
    try {
      this.localDescriptor = await this.adapter.restartIce(this.adapterElementId);
      return this.localDescriptor;
    } catch (error) {
      if (typeof this.adapter.restartIce !== 'function' ||
        error.code === C.ERROR.MEDIA_INVALID_OPERATION.code) {
        Logger.warn('restartIce not supported, repeating local offer', this.getMediaInfo());
        return this.localDescriptor;
      }
