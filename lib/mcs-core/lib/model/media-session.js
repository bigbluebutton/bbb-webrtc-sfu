...
module.exports = class MediaSession {
  ...
  async _disconnect (sink, sourceMediaId, mediaProfile, connectionType) {
    ...
    if (sourceMedia && sinkMedias.length > 0) {
      sinkMedias.forEach(sinkMedia => {
        Logger.trace(LOG_PREFIX, "Adapter elements to be disconnected", sourceMedia.adapterElementId, "=X>", sm.adapterElementId);
        try {
  ...
  createAndSetMediaNames() {
    this.medias.forEach(async (m, index) => {
      let name = this.name+"-"+(index+1).toString();
      Logger.debug("[mcs-media-session] Setting name", name, "for media", m.id);
      m.setName(this.name+"-"+(index+1).toString());
    });
  }
}
