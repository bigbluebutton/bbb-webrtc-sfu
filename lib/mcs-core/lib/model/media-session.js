...
module.exports = class MediaSession {
  ...
  async restartIce () {
    Logger.debug("Media session: restarting ICE", {
      mediaSessionId: this.id, type: this.type,
    });

    return Promise.all(this.medias.map(m => m.restartIce())).then(() => {
      return this.getAnswer();
    });
  }
  ...
  processOptionalParameters (options) {
    ...
    // Update options object with actual media specs
    this._options.mediaSpecs = this.mediaSpecs;
    ...
    // Update options object with actual profiles
    this._options.profiles = this.profiles;
