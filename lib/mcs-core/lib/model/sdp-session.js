  ...
  async _defileAndProcess () {
    ...
    try {
      ...
      videoMedias.forEach(m => {
        ...
        m.localDescriptor = SdpWrapper.getVideoSDP(partialLocalDescriptor);
      });
    } catch (e) {
      this._handleError(e);
    }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (this.remoteDescriptor.mainVideoSdp && this.shouldProcessRemoteDescriptorAsAnswerer) {
          try {
            Logger.info(LOG_PREFIX, "Renegotiating media streams for", this.id)
        ...
        if (this.remoteDescriptor.audioSdp && this.shouldProcessRemoteDescriptorAsAnswerer) {
          try {
            Logger.info(LOG_PREFIX, "Renegotiating audio streams for", this.id)
        ...
        if (this.remoteDescriptor.contentVideoSdp && (!this.localDescriptor || !this.localDescriptor.contentVideoSdp)) {
          try {
            Logger.info(LOG_PREFIX, "Renegotiating content streams for", this.id)
