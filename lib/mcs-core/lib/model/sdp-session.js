...
module.exports = class SDPSession extends MediaSession {
  ...
  async _defileAndProcess () {
    ...
    try {
      ...
      videoMedias.forEach(m => m.setAnswer(SdpWrapper.getVideoSDP(m.answer._plainSdp)));
    } catch (e) {
      this._handleError(e);
    }
    ...
    try {
      ...
      audioMedias.forEach(m => m.setAnswer(SdpWrapper.getAudioSDP(m.answer._plainSdp)));
    } catch (e) {
      this._handleError(e);
    }
      ...
      try {
        ...
        if (this._offer.audioSdp) {
          try {
            ...
            await audioAdapter.processAnswer(audioMedia.adapterElementId, this._offer.audioSdp);
        ...
        if (this._offer.mainVideoSdp) {
          try {
            ...
            await videoAdapter.processAnswer(videoMedia.adapterElementId, this._offer.mainVideoSdp);
