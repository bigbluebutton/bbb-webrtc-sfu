...
const METRIC_NAMES = {
  ...
  DISPATCHED_EVENTS_TOTAL: 'mcsDispatchedEventsTotal',
  REGISTERED_EVENT_CALLBACKS: 'mcsRegisteredEventCallbacks',
}
...
const buildMetrics = () => {
  if (METRICS == null) {
    METRICS = {
      ...
      [METRIC_NAMES.DISPATCHED_EVENTS_TOTAL]: new Counter({
        name: `${METRICS_PREFIX}dispatched_events_total`,
        help: 'Total number of dispatched events in mcs-core',
        labelNames: ['event'],
      }),

      [METRIC_NAMES.REGISTERED_EVENT_CALLBACKS]: new Gauge({
        name: `${METRICS_PREFIX}registered_event_callbacks`,
        help: 'Number of registered event callbacks in mcs-core',
        labelNames: ['event'],
      }),
