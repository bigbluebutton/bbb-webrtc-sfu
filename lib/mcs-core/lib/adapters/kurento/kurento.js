  ...
  _transposeAndConnect(source, sink) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const { sourcePipeline, sourceHost } = source;
        const { sinkPipeline, sinkHost } = sink;

        const sourceTransposer = await this._createElement(sourcePipeline, C.MEDIA_TYPE.RTP);
        const sinkTransposer = await this._createElement(sinkPipeline, C.MEDIA_TYPE.RTP);
        await this.connect(source.id, sourceTransposer.id, 'ALL');
        await this.connect(sinkTransposer.id, sink.id, 'ALL');
        const sourceOffer = await this.generateOffer(sourceTransposer);
        const sinkAnswer = await this.processOffer(sinkTransposer);
        await this.processAnswer(sourceTransposer, sinkAnswer);
        Balancer.incrementHostStreams(sourceHost.id, 'video');
        Balancer.incrementHostStreams(sinkHost.id, 'video');
