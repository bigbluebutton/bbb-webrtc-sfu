  ...
  _transposeAndConnect(source, sink) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const sourceOffer = await this.generateOffer(sourceTransposer.id);
        const sinkAnswer = await this.processOffer(sinkTransposer.id, SdpWrapper.nonPureReplaceServerIpv4(sourceOffer, source.host.ip));
