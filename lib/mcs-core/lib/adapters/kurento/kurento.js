        ...
        return new Promise(async (resolve, reject) => {
          try {
            ...
            if (d) {
              ...
              answer = await this.generateOffer(mediaElement);
            }
      ...
      try {
        ...
        if (mediaElement) {
          mediaElement.generateOffer((error, offer) => {
            ...
            offer = offer.replace(/a=mid:video0/, '');
            offer = offer.replace(/a=mid:audio0/, '');
            offer = offer.replace(/a=rtcp-fb:103 nack/ig, '');
            offer = offer.replace(/a=rtcp-fb:103 nack pli/ig, '');
            offer = offer.replace(/a=rtcp-fb:103 ccm fir/ig, '');
            offer = offer.replace(/a=extmap:3 http:\/\/www.webrtc.org\/experiments\/rtp-hdrext\/abs-send-time/ig, '');
            offer = offer.replace(/a=setup:actpass/ig, '');

            return resolve(offer.replace(/AVPF/ig, 'AVP'));
          });
        }
        else {
          return reject(this._handleError(ERRORS[40101].error));
