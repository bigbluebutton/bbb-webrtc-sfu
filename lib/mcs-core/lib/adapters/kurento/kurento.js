...
module.exports = class Kurento extends EventEmitter {
  ...
  _vanillaICEHandleGatheringDone = (event) => {
    if (event.state.name === C.EVENT.ICE_GATHERING_DONE) {
    }
  }
      ...
      try {
        ...
        if (mediaElement) {
          ...
          mediaElement.processOffer(sdpOffer, (error, answer) => {
            ...
            if (trickle) return resolve(answer);

            this.on(C.EVENT.MEDIA_STATE.MEDIA_EVENT+elementId, this._vanillaICEHandleGatheringDone);

            this.gatherCandidates(elementId)
              .then(error => {
                // TODO change this setTimeout for the gathering event callback
                setTimeout(() => {
                  mediaElement.getLocalSessionDescriptor((e, a) => {
                    return resolve(a);
                  });
                }, 5000);
            });
        ...
        mediaElement.on(eventTag, (e) => {
          ...
          switch (eventTag) {
            case C.EVENT.MEDIA_STATE.ICE:
              ...
              break;
            default:
              event.state = { name: eventTag, details: e.state || e.newState };
              event.elementId = elementId;
              event.timestampUTC = timestampUTC;
              event.timestampHR = timestampHR;
