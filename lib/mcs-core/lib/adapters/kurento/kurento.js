...
module.exports = class MediaServer extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (this._mediaPipelines[roomId] && this._mediaPipelines[roomId][host.id]) {
          ...
          return resolve(this._mediaPipelines[roomId][host.id]);
        }
  ...
  createMediaElement (roomId, type, options = {}) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const host = await Balancer.getHost();
        const pipeline = await this._getMediaPipeline(host, roomId);
        ...
        resolve({ mediaElement: mediaElement.id , host });
      }
      catch (err) {
  ...
  _transposeAndConnect(source, sink) {
    return new Promise(async (resolve, reject) => {
      try {
        Logger.info("[mcs-media] Transposing from", source.id, "to", sink.id);

        const sourceTransposer = await this._createElement(source.pipeline, C.MEDIA_TYPE.RTP);
        const sinkTransposer = await this._createElement(sink.pipeline, C.MEDIA_TYPE.RTP);
        source.connect(sourceTransposer, (error) => {});
        sinkTransposer.connect(sink, (error) => {});
        const sourceOffer = await this.generateOffer(sourceTransposer);
        const sinkAnswer = await this._processOffer(sinkTransposer, SdpWrapper.nonPureReplaceServerIpv4(sourceOffer, source.host.ip));
        await this.processAnswer(sourceTransposer, SdpWrapper.nonPureReplaceServerIpv4(sinkAnswer, sink.host.ip));
        Balancer.incrementHostStreams(source.host.id, 'video');
        Balancer.incrementHostStreams(sink.host.id, 'video');
  ...
  async connect (sourceId, sinkId, type) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (shouldTranspose) {
          this._transposeAndConnect(source, sink);
          return resolve();
        }
  ...
  _processOffer (mediaElement, sdpOffer) {
  ...
  processAnswer (mediaElement, answer) {
    return new Promise((resolve, reject) => {
      try {
        if (mediaElement) {
