...
module.exports = class MediaServer extends EventEmitter {
  constructor(balancer, globalEmitter) {
    if (!instance){
      ...
      this._jobs = new Queue('kurento-adapter-jobs');
      this._jobs.process('transposeAndConnect', (job, done) => {
        this._transposeAndConnect(job.data.sourceId, job.data.sinkId, done);
      });
      this._jobs.process('getMediaPipeline', async (job) => {
        return this._getMediaPipeline(job.data.hostId, job.data.roomId);
      });
  ...
  createMediaElement (roomId, type, options = {}) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const job = await this._jobs.add('getMediaPipeline', {
          hostId: host.id,
          roomId
        });
  ...
  async _transposeAndConnect(sourceId, sinkId, done) {
    try {
      const source = this._mediaElements[sourceId];
      ...
      if (sourceTransposer == null && !this._transposingQueue.includes(source.id+sink.host.id)) {
      ...
      return done(this._handleError(err));
    }
  }
  ...
  connect (sourceId, sinkId, type) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        if (shouldTranspose) {
          const job = await this._jobs.add('transposeAndConnect', {
            sourceId,
            sinkId
          })
          await job.finished();
