  ...
  _transposeAndConnect(source, sink) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (sinkTransposer == null) {
          Logger.info("[mcs-media] Sink transposer for", sink.id, "to host", source.host.id, "not found");
          sinkTransposer = await this._createElement(sink.pipeline, C.MEDIA_TYPE.RTP);
          sink.pipeline.transposers[source.host.id+source.id] = sinkTransposer;
