...
module.exports = class MediaServer extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (this._mediaPipelines[roomId] && this._mediaPipelines[roomId][host.id]) {
          Logger.warn('[mcs-media] Pipeline for', roomId, 'at host', host.id, ' already exists.');
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (sourceTransposer == null) {
          ...
          this._transposingQueue.push(source.id+sink.host.id);
          ...
          Logger.info("[mcs-media] Sink transposer for", sink.id, "to host", source.host.id, "not found");
          ...
          this._transposingQueue = this._transposingQueue.filter(sm => sm !== source.id + sink.host.id);
          this.emit(C.ELEMENT_TRANSPOSED + source.id + sink.host.id);
          return resolve();

        } else {
          if (this._transposingQueue.includes(source.id + sink.host.id)) {
            this.once(C.ELEMENT_TRANSPOSED + source.id + sink.host.id, () => {
  ...
  connect (sourceId, sinkId, type) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        if (shouldTranspose) {
          await this._transposeAndConnect(source, sink);
          return resolve();
        }
        ...
        if (mediaElement && typeof mediaElement.release === 'function') {
          ...
          if (mediaElement.transposers) {
            Object.keys(mediaElement.transposers).forEach(t => {
              ...
              this.balancer.decrementHostStreams(mediaElement.host.id, 'video');
            });
          }
  ...
  processOffer (elementId, sdpOffer, replaceIp = false) {
    const mediaElement = this._mediaElements[elementId];
