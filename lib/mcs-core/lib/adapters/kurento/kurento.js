...
module.exports = class Kurento extends EventEmitter {
  ...
  async _getMediaPipeline (hostId, roomId) {
    try {
      ...
      if (this._mediaPipelines[roomId] && this._mediaPipelines[roomId][host.id]) {
        ...
        Logger.debug(`KMS: Created pipeline at room ${roomId}`,
  ...
  _releasePipeline (room, hostId) {
    return new Promise((resolve, reject) => {
      try {
        Logger.debug(`KMS: Releasing pipeline of room ${room}`,
          { hostId, roomId: room });
  ...
  _negotiateSDPEndpoint (roomId, userId, mediaSessionId, descriptor, type, options) {
    Logger.debug(`KMS: Negotiating SDP endpoint`, { userId, roomId });
    try {
        ...
        medias.forEach(m => {
          if (m.type === C.MEDIA_TYPE.WEBRTC) {
            this.gatherCandidates(m.adapterElementId).catch(error => {
              Logger.error(`KMS: Candidate gathering for media ${m.id} failed due to ${error.message}`,
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (type === C.MEDIA_TYPE.RTP || type === C.MEDIA_TYPE.WEBRTC) {
          this.setOutputBandwidth(mediaElement, 30, 1500);
          this.setInputBandwidth(mediaElement, 30, 1500);
  ...
  async _transposeAndConnect(sourceId, sinkId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        Logger.info(`KMS: Transposing elements`, {
        ...
        if (sourceTransposer == null) {
          ...
          Logger.debug("KMS: Source transposer not found",
          ...
          Logger.debug("KMS: Sink transposer not found", {
  ...
  async _connect (source, sink, type = 'ALL') {
    return new Promise((resolve, reject) => {
      try {
        ...
        Logger.info("KMS: Adapter elements to be connected", {
          sourceId: this.getMediaElementId(source),
          sinkId: this.getMediaElementId(sink),
          connectionType: type,
        });
  ...
  stop (room, type, elementId) {
    return new Promise(async (resolve, reject) => {
      try {
        Logger.info(`KMS: Releasing endpoint`, { elementId, roomId: room });
        ...
        if (mediaElement) {
          ...
          if (mediaElement.transposers) {
            Object.keys(mediaElement.transposers).forEach(t => {
              setTimeout(() => {
                ...
                Logger.debug(`KMS: Releasing transposer`,
          ...
          if (typeof mediaElement.release === 'function') {
            mediaElement.release(async (error) => {
              ...
              if (pipeline) {
                ...
                Logger.info(`KMS: Pipeline elements decreased for room ${room}`,
          ...
          Logger.warn(`KMS: Media element not found on stop`, { elementId });
          return resolve();
        }
      }
      catch (err) {
      ...
      try {
        if (mediaElement  && candidate) {
          if (this._checkForMDNSCandidate(candidate.candidate) &&
            !KURENTO_ALLOW_MDNS) {
            Logger.trace("KMS: Ignoring a mDNS obfuscated candidate", candidate.candidate);
            return resolve();
          }
          ...
          mediaElement.addIceCandidate(parsedCandidate, (error) => {
            ...
            Logger.trace("KMS: Added ICE candidate for => ", elementId, candidate);
            return resolve();
          });
        }
        else {
  ...
  async _vanillaGatherCandidates (elementId) {
    ...
    try {
      ...
      Logger.error(`KMS: Vanilla candidate gathering failed`,
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        mediaElement.gatherCandidates((error) => {
          ...
          Logger.debug(`KMS: Triggered ICE gathering for ${elementId}`);
          return resolve();
        });
      }
      catch (err) {
      ...
      try {
        ...
        if (mediaElement) {
          if (mediaElement.negotiated) {
            Logger.warn(`KMS: Element ${elementId} was already negotiated, ignoring processOffer`);
            return resolve();
          }
          ...
          Logger.trace(`KMS: Processing ${elementId} offer`, { offer: sdpOffer });
          ...
          mediaElement.processOffer(sdpOffer, (error, answer) => {
            ...
            this._vanillaGatherCandidates(elementId)
              .then((localDescriptor) => {
                Logger.info(`KMS: Vanilla candidate gathering succeeded`,
  ...
  processAnswer (elementId, answer) {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (mediaElement) {
          if (mediaElement.negotiated) {
            Logger.warn(`KMS: Element ${elementId} was already negotiated, ignoring processAnswer`);
            return resolve();
          }

          Logger.trace(`KMS: Processing ${elementId} answer`, { answer });
  ...
  generateOffer (elementId, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        Logger.error("KMS: Failed to digest media offer options", error);
      }
  ...
  addMediaEventListener (eventTag, elementId) {
    ...
    try {
      if (mediaElement) {
        Logger.trace(`KMS: Adding media state listener ${eventTag}`, { eventTag, elementId });
      ...
      Logger.error('KMS: Failure in addMediaEventListener', {
  ...
  _removeElementEventListeners (elementId) {
    ...
    Logger.trace(`KMS: Removing all event listeners for ${elementId}`);
  ...
  _destroyElementsFromHost (hostId) {
    try {
      ...
      Logger.error(`KMS: Error destroying elements from host ${hostId}`,
        { error, hostId });
    }
  }
  ...
  _handleError(err) {
    ...
    if (stack && !err.stackWasLogged)  {
      Logger.error(`KMS: Stack trace for error ${err.code} | ${err.message} ->`,
