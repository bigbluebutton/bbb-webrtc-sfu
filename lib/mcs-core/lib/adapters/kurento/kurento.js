        ...
        mediaElement.on(eventTag, (e) => {
          switch (eventTag) {
            case C.EVENT.MEDIA_STATE.ICE:
              event.candidate = mediaServerClient.getComplexType('IceCandidate')(e.candidate);
              event.elementId = elementId;
              this.emit(C.EVENT.MEDIA_STATE.ICE+elementId, event);
              break;
            default:
              event.state = eventTag;
              event.elementId = elementId;
              Logger.info("EMITTING", event, C.EVENT.MEDIA_STATE.MEDIA_EVENT+elementId);
