...
module.exports = class TaintedLoopbackAdapter extends EventEmitter {
  ...
  async _getMediaPipeline (hostId, roomId) {
    try {
      ...
      if (this._mediaPipelines[roomId] && this._mediaPipelines[roomId][host.id]) {
        ...
        Logger.debug(`LOA: Created pipeline at room ${roomId}`,
  ...
  _releasePipeline (room, hostId) {
    return new Promise((resolve, reject) => {
      try {
        Logger.debug(`LOA: Releasing pipeline of room ${room}`,
          { hostId, roomId: room });
  ...
  _negotiateSDPEndpoint (roomId, userId, mediaSessionId, descriptor, type, options) {
    Logger.debug(`LOA: Negotiating SDP endpoint`, { userId, roomId });
    try {
        ...
        medias.forEach(m => {
          if (m.type === C.MEDIA_TYPE.WEBRTC) {
            this.gatherCandidates(m.adapterElementId).catch(error => {
              Logger.error(`LOA: Candidate gathering for media ${m.id} failed due to ${error.message}`,
  ...
  async _connect (source, sink, type = 'ALL') {
    return new Promise((resolve, reject) => {
      try {
        ...
        Logger.info("LOA: Adapter elements to be connected", JSON.stringify({
  ...
  async stop (room, type, elementId) {
    try {
      Logger.info(`LOA: Releasing endpoint`, { elementId, roomId: room });
      ...
      if (mediaElement) {
        ...
        if (pipeline) {
          ...
          Logger.info(`LOA: Pipeline elements decreased for room ${room}`,
        ...
        Logger.warn(`LOA: Media element not found on stop`, { elementId });
      }
    } catch (error) {
      Logger.error('LOA: Element stop failed', {
      ...
      try {
        if (mediaElement  && candidate) {
          if (this._checkForMDNSCandidate(candidate.candidate)) {
            Logger.trace("LOA: Ignoring a mDNS obfuscated candidate", candidate.candidate);
            return resolve();
          }
          ...
          Logger.trace("LOA: Added ICE candidate for => ", elementId, candidate);
          return resolve();
        }
        else {
          return reject(this._handleError(ERRORS[40101].error));
  ...
  gatherCandidates (elementId) {
    return new Promise((resolve, reject) => {
      try {
        ...
        Logger.debug(`LOA: Triggered ICE gathering for ${elementId}`);
        return resolve();
      } catch (error) {
        return reject(error);
      }
      ...
      try {
        ...
        if (mediaElement) {
          if (mediaElement.negotiated) {
            Logger.warn(`LOA: Element ${elementId} was already negotiated, ignoring processOffer`);
            return resolve();
          }

          Logger.trace(`LOA: Processing ${elementId} offer`, { offer: sdpOffer });
  ...
  processAnswer (elementId, answer) {
    return new Promise((resolve, reject) => {
      try {
        ...
        if (mediaElement) {
          if (mediaElement.negotiated) {
            Logger.warn(`LOA: Element ${elementId} was already negotiated, ignoring processAnswer`);
            return resolve();
          }
          Logger.trace(`LOA: Processing ${elementId} answer`, { answer });
          mediaElement.negotiated = true;
          return resolve();
        }
        else {
      ...
      try {
        ...
        if (mediaElement) {
          mediaElement.generateOffer((error, offer) => {
            ...
            Logger.trace(`LOA: Generated offer for ${elementId}`, { offer });
  ...
  addMediaEventListener (eventTag, elementId) {
    ...
    try {
      if (mediaElement) {
        Logger.trace(`LOA: Adding media state listener ${eventTag}`, { eventTag, elementId });
      ...
      Logger.error('LOA: Failure in addMediaEventListener', {
  ...
  _removeElementEventListeners (elementId) {
    ...
    Logger.trace(`LOA: Removing all event listeners for ${elementId}`);
  ...
  _destroyElementsFromHost (hostId) {
    try {
      ...
      Logger.error(`LOA: Error destroying elements from host ${hostId}`,
        { error, hostId });
    }
  }
  ...
  _handleError(err) {
    ...
    if (stack && !err.stackWasLogged)  {
      Logger.error(`LOA: Stack trace for error ${err.code} | ${err.message} ->`,
