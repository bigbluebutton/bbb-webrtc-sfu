      ...
      try {
        if (userAgent) {
          if (sdpOffer == null || hackProxyViaKurento) {
            ...
            if (hackProxyViaKurento && sdpOffer) {
          ...
          const handleOffer = async (offer) => {
            ...
            const procAns = await this._Kurento.processOffer(rtp, procOffer, { replaceIp: true });
            session.mediaHandler.setRemoteOffer(procAns);
            session.mediaHandler.removeListener('REMOTE_SDP_RECEIVED', handleOffer);
          }

          if (hackProxyViaKurento) {
            if (sdpOffer) {
              session.on('REMOTE_SDP_RECEIVED', handleOffer.bind(this));
            } else {
              session.on('REMOTE_SDP_RECEIVED', offer => {
...
/**
  ...
  sipCall (userAgent, username, voiceBridge, host, port, rtp, descriptor) {
    const inviteWithoutSdp = descriptor? false : true;
    //call options
    var options = {
      media: {
        constraints: {
    ...
    //mediaHandler.SIPMediaHandler.setup(descriptor, rtp, this._Kurento);
