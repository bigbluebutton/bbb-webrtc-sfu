...
module.exports = class Freeswitch extends EventEmitter {
  constructor(balancer) {
    if(!instance){
      ...
      this._rtpProxies = {};
      this._channelIds = {};
      this._channelIdsToCallId = {};
      //map channelId to UserAgent
      this._channelIdsToUa = {};
      this._memberIds = {};
      this._memberIdsToUa = {};
      this._channelIdsToSdps = {};
  ...
  _handleChannelAnswer (channelId, callId, sdpOffer, sdpAnswer) {
    ...
    this._channelIdsToCallId[channelId] = callId;
    ...
    this._channelIdsToSdps[channelId] = { sdpOffer, sdpAnswer };
  }

  _handleChannelHangup (channelId, callId) {
    const elementId = this._channelIdsToUa[channelId];
  ...
  async _handleConferenceMember (channelId, memberId, callerIdNumber,roomId) {
    ...
    if (!elementId) {
      // element does not exists, the media joined externally (bbb), need to create 
      const splitted = callerIdNumber.split("-");
      // userId-bbbID-userName
      const userId = splitted[0];
      const userName = splitted[2];
      Logger.info(LOG_PREFIX,'BBB user microphone joined!!',callerIdNumber, roomId);
      const sdps = this._channelIdsToSdps[channelId];

      let mediaElement, host;
      ({ mediaElement, host } = await this.createMediaElement(roomId, C.MEDIA_TYPE.WEBRTC, {}));
      let media = new SDPMedia(roomId, userId, "", sdps.sdpOffer, sdps.sdpAnswer, C.MEDIA_TYPE.WEBRTC, this, mediaElement, host, {});
      media.trackMedia();

      let userAgent = this._userAgents[mediaElement];
      userAgent.callId = this._channelIdsToCallId[channelId];
      userAgent.channelId = channelId;
      this._channelIdsToUa[channelId] = mediaElement;
      this._memberIdsToUa[memberId] = mediaElement;

      const event = {
        roomId,
        userId,
        userName,
        sdpOffer: sdps.sdpOffer,
        sdpAnswer: sdps.sdpAnswer,
        media
      }

      GLOBAL_EVENT_EMITTER.emit(C.EVENT.MEDIA_BBB_AUDIO_CONNECTED,event);
    }
  ...
  _deleteChannelMappings(elementId) {
    if (elementId in this._channelIds) {
      ...
      delete this._channelIdsToUa[channelId];
      let memberId = this._memberIds[channelId];
      delete this._memberIds[channelId];
      delete this._memberIdsToUa[memberId];
      delete this._channelIdsToCallId[channelId];
      delete this._channelIdsToSdps[channelId];
    };
  }
