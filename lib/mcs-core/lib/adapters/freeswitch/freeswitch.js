...
module.exports = class Freeswitch extends EventEmitter {
    ...
    if (channelInfo) {
      ...
      if (userAgent) {
        ...
        if (!session || session.isOfferer) {
          ...
          Logger.warn(LOG_PREFIX, `Received CHANNEL_HANGUP for ${elementId}`,
            { elementId, channelId, callId });
  ...
  _handleExternalMediaConnected (channelId, callerIdNumber, roomId) {
    let elementId, mediaElement, host;
    const channelInfo = this._channelIdInfos[channelId];
    // callerIdNumber format: userId-bbbID-userName
    const tokenizedCallerIdNumber = callerIdNumber.split("-");
    const userId = tokenizedCallerIdNumber[0];
    const userName = tokenizedCallerIdNumber[2];
    const { sdpOffer, sdpAnswer } = this._channelIdInfos[channelId];
    this.createMediaElement(roomId, C.MEDIA_TYPE.WEBRTC, {})
  ...
  async _handleConferenceMember (channelId, memberId, callerIdNumber, roomId) {
    Logger.debug(LOG_PREFIX, `New conference member, associating channelUUID ${channelId} to memberId ${memberId}`, { roomId, channelId, memberId, callerIdNumber });
    const channelInfo = this._channelIdInfos[channelId];
  ...
  _handleVolumeChanged (channelId, volume) {
    ...
    Logger.debug(LOG_PREFIX, `Emitting volume changed for adapter element ${elementId}`,
      { adapterElementId: elementId, channelId, volume: convertedVolume });
    this.emit(C.EVENT.MEDIA_VOLUME_CHANGED+elementId, convertedVolume);
  }

  _handleMuted (channelId) {
    const elementId = this._channelIdInfos[channelId].ua;
    Logger.debug(LOG_PREFIX, `Emitting muted event for adapter element ${elementId}`,
      { adapterElementId: elementId, channelId });
    this.emit(C.EVENT.MEDIA_MUTED+elementId);
  }

  _handleUnmuted (channelId) {
    const elementId = this._channelIdInfos[channelId].ua;
    Logger.debug(LOG_PREFIX, `Emitting unmuted event for adapter element ${elementId}`,
      { adapterElementId: elementId, channelId });
  ...
  _handleFloorChanged (roomId, newFloorMemberId) {
    ...
    Logger.debug(LOG_PREFIX, `Emitting conference video floor changed to ${newFloorElementId} at ${roomId}`,
  ...
  setVolume (mediaElementId, volume) {
    ...
    try {
      ...
      Logger.info(LOG_PREFIX, `Request to setVolume ${volume} for adapter element ${mediaElementId}`,
  ...
  mute (mediaElementId) {
    ...
    try {
      const channelId = userAgent.channelId;
      const memberId = this._channelIdInfos[channelId].memberId;
      Logger.info(LOG_PREFIX, `Request to mute for adapter element ${mediaElementId}`,
        { adapterElementId: mediaElementId, channelId, memberId });
  ...
  unmute (mediaElementId) {
    ...
    try {
      const channelId = userAgent.channelId;
      const memberId = this._channelIdInfos[channelId].memberId;
      Logger.info(LOG_PREFIX, `Request to unmute for adapter element ${mediaElementId}`,
        { adapterElementId: mediaElementId, channelId, memberId });
  ...
  dtmf (elementId, tone) {
    ...
    try {
      const channelId = userAgent.channelId;
      Logger.info(LOG_PREFIX, `Request to send DTMF tone ${tone} for adapter element ${elementId}`,
        { adapterElementId: elementId, channelId, tone });
