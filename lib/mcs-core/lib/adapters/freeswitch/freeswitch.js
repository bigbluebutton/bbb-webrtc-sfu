...
module.exports = class Freeswitch extends EventEmitter {
  ...
  async createMediaElement (roomId, type, params) {
    try {
      ...
      return Promise.resolve(userAgentId);
    }
    catch (err) {
        ...
        if (userAgent) {
          ...
          if (sdpOffer == null) {
            if (this._rtpConverters[voiceBridge]) {
              rtpEndpoint = this._rtpConverters[voiceBridge];
            }
            else {
              rtpEndpoint = await this._Kurento.createMediaElement(voiceBridge, 'RtpEndpoint');
              this._rtpConverters[voiceBridge] = rtpEndpoint;
            }
            Logger.info("[mcs-media-freeswitch] RTP endpoint equivalent to SIP instance is", rtpEndpoint, "indexed at", voiceBridge);
          }
  ...
  _createUserAgent (type, displayName, roomId) {
    ...
    var newUA = new SIPJS.UA({
      uri: 'sip:' + uriUser + '@' + LOCAL_IP_ADDRESS,
