...
module.exports = class Freeswitch extends EventEmitter {
  static escapeDisplayName(displayName, maxLength = 128) {
    if (!displayName) return '';

    // Not for BBB, but for external endpoints
    if (displayName.length > maxLength) displayName = displayName.substring(0, maxLength);

    // Token only display names are safe to use as is - see SIP RFC
    const tokenOnly = /^[-A-Za-z0-9!%*_+`'~.]+$/.test(displayName);
    if (tokenOnly) return displayName;
    ...
    for (let i = 0; i < displayName.length; i++) {
      ...
      if (char === '"') {
        // FS shenanigans - double quotes bork some of the event headers we depend
        // on even escaped, so we replace them with single quotes.
        escaped += "'";
      } else {
        escaped += char;
      }
  ...
  _trackESLEvents() {
    ...
    this._eslWrapper.on(EslWrapper.EVENTS.FREESWITCH_RESTARTED, this._handleFreeswitchRestarted.bind(this));
  }

  _handleFreeswitchRestarted() {
    Logger.warn('FS: Freeswitch restarted, notifying mcs-core');
    this.emit(C.EVENT.MEDIA_SERVER_OFFLINE);
    PrometheusAgent.increment(FS_METRIC_NAMES.FREESWITCH_CRASHES);
  }

  _handleChannelAnswer (channelId, callId) {
    Logger.debug('FS: Associating channel', { channelId, callId });
    ...
    if (channelInfo) {
      ...
      if (userAgent) {
        ...
        if (!session || session.isOfferer) {
          ...
          Logger.debug('FS: Received CHANNEL_HANGUP', { elementId, channelId, callId });
  ...
  _handleStartTalking (channelId) {
    ...
    if (elementId) this.emit(C.EVENT.MEDIA_START_TALKING+elementId);
  }

  _handleStopTalking (channelId) {
    const elementId = this._channelIdInfos[channelId]?.ua;
    if (elementId) this.emit(C.EVENT.MEDIA_STOP_TALKING+elementId);
  }

  _handleConferenceMember (channelId, memberId, callerIdNumber, roomId) {
    Logger.debug("FS: New conference member, associating UUID to mID",
  ...
  _handleMuted (channelId) {
    const elementId = this._channelIdInfos[channelId]?.ua;
    if (elementId) this.emit(C.EVENT.MEDIA_MUTED+elementId);
  }

  _handleUnmuted (channelId) {
    const elementId = this._channelIdInfos[channelId]?.ua;
    if (elementId) this.emit(C.EVENT.MEDIA_UNMUTED+elementId);
  }
  ...
  _handleFloorChanged (roomId, newFloorMemberId) {
    ...
    if (newFloorMemberId === 'none') {
      ...
      Logger.debug('FS: Video floor released', { event });
    ...
    Logger.debug('FS: Video floor changed',
  ...
  async createMediaElement (roomId, type, params) {
    try {
      ...
      const userAgent = await this._createUserAgent(type, params.name, roomId, params);
  ...
  async stop (roomId, type = {}, elementId) {
    ...
    try {
      ...
      Logger.error(`FS: Error when stopping userAgent for ${elementId} at room ${roomId}`,
        { type, error: this._handleError(error) });
    }

    Logger.info("FS: Endpoint released", { elementId, roomId });
  }
  ...
  _deleteChannelMappings(elementId) {
    if (elementId in this._channelIds) {
      ...
      if (memberId) delete this._memberIdsToUa[memberId];
    ...
    if (userAgent) {
      ...
      const uaWaitForDisconnection = () => {
        userAgent.once('disconnected', () => {
          stopped = true;
          Logger.warn("FS: SIP.js UA disconnected", { elementId });
      ...
      const uaStopFailover = () => {
        setTimeout(() => {
          if (!stopped) {
            Logger.warn("FS: UA hit failover timeout, socket leak?", { elementId });
          }
          delete this._userAgents[elementId];
          this._deleteChannelMappings(elementId);
          return Promise.resolve();
  ...
  async processOffer (elementId, sdpOffer) {
    const userAgent = this._userAgents[elementId];
      ...
      try {
        if (userAgent) {
          ...
          const handleReinvite = (reinviteSDP) => {
            Logger.debug("FS: re-INVITE received", { elementId, reinviteSDP });
          ...
          session.once('rejected', (response, cause) => {
            Logger.error("FS: SIP dialog rejected", { elementId, cause });
            this.emit(C.EVENT.MEDIA_DISCONNECTED+elementId);
            handleNegotiationError(cause);
          });

          session.once('failed', (response, cause) => {
            Logger.error("FS: SIP dialog failure", { elementId, cause });
            this.emit(C.EVENT.MEDIA_DISCONNECTED+elementId);
            handleNegotiationError(cause);
          });

          session.once('terminated', () => {
            Logger.debug("FS: SIP dialog terminated", { elementId });
            this.emit(C.EVENT.MEDIA_DISCONNECTED+elementId);
  ...
  setVolume (mediaElementId, volume) {
    ...
    try {
      ...
      Logger.error(`FS: setVolume failed due to ${error.message}`,
  ...
  mute (mediaElementId) {
    ...
    try {
      ...
      return this._eslWrapper.mute(voiceBridge, memberId).then(() => {
        Logger.debug("FS: Audio muted",
          { adapterElementId: mediaElementId, channelId, memberId });
        userAgent.muted = true;
      });
    } catch (error) {
      Logger.error(`FS: mute failed due to ${error.message}`,
  ...
  unmute (mediaElementId) {
    ...
    try {
      ...
      Logger.error(`FS: unmute failed due to ${error.message}`,
        { adapterElementId: mediaElementId, error });
      throw (this._handleError(error));
    }
  }
  ...
  trackMediaState (elementId) {
    ...
    if (userAgent) {
      userAgent.on('invite', function() {
        Logger.trace("FS: UA recv'd invite");
      });

      userAgent.on('message', function(message) {
        Logger.trace("FS: UA recv'd message", { message });
      });

      userAgent.on('connected', function() {
        Logger.trace("FS: UA connected");
      });

      userAgent.on('disconnected', function (){
        Logger.debug("FS: UA disconnected");
      });
  ...
  _createUserAgent (type, displayName, roomId, options = {}) {
    const escapedDisplayName = Freeswitch.escapeDisplayName(displayName);
    const uriUser = escapedDisplayName ? escapedDisplayName : roomId;
    const userId = uriUser.split('-bbbID')[0];
    const userAgentString = options?.adapterOptions?.userAgentString || SIP_USER_AGENT;

    const newUA = new SIPJS.UA({
      uri: `sip:${encodeURIComponent(userId)}@${FREESWITCH_SIP_IP}`,
   ...
   */
  sipCall (userAgent, voiceBridge, host, port, descriptor, extension) {
  ...
  dtmf (elementId, tones, options) {
    ...
    try {
      ...
      Logger.error(`FS: dtmf failed due to ${error.message}`,
  ...
  _removeElementEventListeners (elementId) {
    ...
    Logger.trace("FS: Removing all event listeners for", elementId);
