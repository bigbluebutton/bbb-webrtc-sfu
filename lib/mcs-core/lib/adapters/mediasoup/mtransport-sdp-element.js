...
module.exports = class MultiTransportSDPElement extends BaseMediasoupElement {
  ...
  getProducerOfKind (kind) {
    let targetProducer;
    // Find first producer of the provided kind in either of the elements in this._internalSDPElements
    // TODO refactor this inefficient aberration
    this._internalSDPElements.forEach((internalElement) => {
      internalElement.producers.forEach((producer) => {
  ...
  getNumberOfProducers () {
    ...
    this._internalSDPElements.forEach((internalElement) => {
      value += internalElement.producers.size;
    });
