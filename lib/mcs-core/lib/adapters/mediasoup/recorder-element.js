...
const { RTP_TRANSPORT_SETTINGS, RECORDER_FFMPEG } = require('./configs.js');
...
module.exports = class RecorderElement extends BaseMediasoupElement {
  ...
  _handleRecorderEnded (reason) {
    Logger.info('mediasoup: recorder stopped', {
  ...
  _handleRecorderFailure (error) {
    Logger.error('mediasoup: recording failure', {
  ...
  _handleRecorderProgress (progress) {
    Logger.trace('mediasoup: recording progress', {
    ...
    if (this.recordingMarkEventFired === true) {
      ...
      try {
        ...
        finalUTCInMs = this.recorder.startedUTC;
        finalHRInMs = this.recorder.startedHR;
        Logger.warn('mediasoup: failure in recording timemark offset check', {
  ...
  _handleRecorderStarted () {
    Logger.info('mediasoup: recording started', { elementId: this.id });
  ...
  async _recordStream (codecParameters) {
    try {
      ...
      Logger.debug('mediasoup: internal stream recording failure', {
        errorMessage: error.message, elementId: this.id, type: this.type,
  ...
  async _stop () {
    Logger.trace('mediasoup: stopping recorder', {
      elementId: this.id,
    });
