...
const buildDefaultMetrics = () => {
  if (MS_METRICS == null) {
    MS_METRICS = {
      ...
      [MS_METRIC_NAMES.MEDIASOUP_ICE_TRANSPORT_PROTO]: new Counter({
        ...
        Object.entries(metrics).forEach(([metricName, metricValue]) => {
          ...
          if (metric) {
            ...
            const type = metric.set
              ...
              : 'Counter';
            if (type === 'Gauge') {
              metric.set({ pool: mediaType, workerUID }, metricValue);
            } else {
              ...
              if (metricValDiff > 0) metric.inc({ pool: mediaType, workerUID }, metricValDiff);
            }
          }
        });
      });
