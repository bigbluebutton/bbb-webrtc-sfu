...
const _pipeLog = (level, namespace, log, error) => {
  switch (level) {
    case 'warn':
      Logger.warn(`[SFU_MEDIASOUP] ${namespace} ${log}`);
      break;
    case 'error':
      Logger.error(`[SFU_MEDIASOUP] ${namespace} ${log}`, error);
      break;
    case 'debug':
    default:
      Logger.debug(`[SFU_MEDIASOUP] ${namespace} ${log}`);
  }
}

const observe = () => {
  if (PIPE_LOGS) {
    const workerLogLevel = WORKER_SETTINGS?.logLevel || 'warn';

    mediasoup.setLogEventListeners({
      ondebug: workerLogLevel !== 'debug' ? undefined : (namespace, log) => _pipeLog('debug', namespace, log),
      onwarn: (namespace, log) => _pipeLog('warn', namespace, log),
      onerror: (namespace, log, error) => _pipeLog('error', namespace, log, error),
    });
  }
      ...
      router.observer.on('newtransport', async (transport) => {
        ...
        transport.observer.on('close', async () => {
          ...
          if (transports.delete(transport.id)) {
            worker.appData.load.transports--;
        ...
        // If type is null, it's a PipeTransport
        if (type == null) type = 'PipeTransport';
