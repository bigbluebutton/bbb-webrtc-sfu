...
module.exports = class MediasoupSDPElement extends BaseMediasoupElement {
  ...
  async _processConsumerSDPOffer (kind, options) {
    ...
    // FIXME yeah... we really don't want to rely on single negotiation
    this.negotiated = true;
    let offerRtpParameters = consumer.rtpParameters;

    if (!this.remoteDescriptor && this.type === C.MEDIA_TYPE.WEBRTC) {
      // We're the offerer and we need to add all the RTCP-FB entries we can
      // that aren't generated by default by the consumer (i.e. transport-cc, remb)
      offerRtpParameters.codecs = enrichCodecsWithRtcpFb(offerRtpParameters.codecs, rtpParams.codecs);
      offerRtpParameters = enrichRtpParamsWithHdrExt(
        offerRtpParameters,
        rtpParams.headerExtensions
      );
      // Filter out unsupported header extensions *again* because we have added
      // new potential ones from the consumer's rtpParameters. Keep in mind
      // this is a post-processing step of sdp-element (and the original filtering
      // happens in base-element)
      offerRtpParameters.headerExtensions = this._filterUnsupportedExts(
        offerRtpParameters.headerExtensions
      );
    }
  ...
  async _connect (sourceElement, connectionType) {
    ...
    const kmapPatch = await Promise.all(kinds.map(kind => {
      ...
      return this._consume(sourceElement, producer, kind, rtpParams).then((consumer) => {
        return {
          kind,
          direction: 'recvonly',
