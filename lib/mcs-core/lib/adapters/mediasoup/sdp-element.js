...
module.exports = class MediasoupSDPElement extends BaseMediasoupElement {
  constructor(type, routerId, options) {
    ...
    /*
     * kindParametersMap: Array<KindParameterObj>
     * KindParameterObj: {
     *   kind: oneof<'audio' | 'video'>,
     *   direction: oneof<'sendonly' | 'recvonly' | 'sendrecv'>,
     *   offerRtpParameters: {
     *     mid: String,
     *     codecs: Array<RtpCodecCapability> (mediasoup),
     *     headerExtensions: Array<RtpHeaderExtension> (mediasoup),
     *     encodings: Array<RtpEncodingParameters> (mediasoup),
     *     rtcp: RtcpParameters (mediasoup),
     *   },
     *   streamId: String,
     *   trackId: String,
     *   setup: oneof<'active'|'passive'|'actpass'>
     * }
     */
  ...
  async _processProducerSDPOffer (kind, options) {
    ...
    const producer = await this.produce(kind, rtpParams, {
      appData: {
        mediaTypeHint: options.mediaTypeHint,
      },
    });
  ...
  async _processConsumerSDPOffer (kind, options) {
    ...
    const consumer = await this.consume(kind, rtpParams, {
      ...options,
      appData: {
        mediaTypeHint: options.mediaTypeHint,
      },
  ...
  async _generateOffererProducerCaps (targetKind) {
    const baseRouterSettings = this.transportSet?.routerSettings?.codecs || ROUTER_SETTINGS.mediaCodecs;
    const codecs = config.util.cloneDeep(baseRouterSettings)
  ...
  _getRTPParameters (kind, mode, { remoteDescriptor, sourceAdapterElementIds }) {
    ...
    if (mode === 'consumer') {
      ...
      if (!producer) throw new Error('No producer of kind ' + kind + ' found');
  ...
  async processSDPOffer (mediaTypes, options) {
    ...
    if (this.remoteDescriptor == null && options.remoteDescriptor) {
      ...
      Logger.trace("mediasoup: remote descriptor set", {
        elementId: this.id, type: this.type, router: this.routerId,
        transport: this.transportSet.id,
      });
    }

    const transducingRoutines = kindDirectionMap.map(({ kind, direction, mediaType }) => {
      return this._processMediaOffer(kind, direction, {
  ...
  _connectWRTCTransport (description) {
    ...
    return this.transportSet.connect({ dtlsParameters }).then(() => {
      Logger.debug("mediasoup: transport connected", {
        elementId: this.id, type: this.type, routerId: this.routerId, dtlsParameters,
      });
    }).catch(error => {
      Logger.error("mediasoup: transport connect failure", {
  ...
  _restartIce () {
    return this.transportSet.restartIce().then((iceParameters) => {
      ...
      Logger.error("mediasoup: ICE restart failed", {
        errorMessage: error.message, elementId: this.id, type: this.type,
