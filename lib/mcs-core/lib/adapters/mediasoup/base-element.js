...
module.exports = class BaseMediasoupElement extends EventEmitter {
  ...
  record (uri) {
      ...
      const recRoutine = new Promise(async (resolve, reject) => {
        ...
        try {
          ...
          if (producer.recorderConsumer == null) {
            const recorderConsumer = await producer.recorderTransport.consume({
              producerId: producer.id,
              rtpCapabilities: {
            ...
            producer.recorderConsumer = recorderConsumer;

            // Annotate correct payload type based on what the consumer merged
            // between input, producer and router
            codecParameters.codecId = recorderConsumer.rtpParameters.codecs[0].payloadType
    ...
    return Promise.all(recordingRoutines).then(() => {
      ...
      this.recorder.rtpPortsInUse = recCodecParameters.map(({ rtpPort }) => { return rtpPort });

      // Error stands for process end as well...
      ...
      this.recorder.on('error', (error) => {
        ...
        this.recorder.rtpPortsInUse.forEach((rtp) => {
          releasePortPair(rtp);
        });
        this.recorder = null;
      });
