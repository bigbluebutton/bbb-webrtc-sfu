...
module.exports = class TransportSet {
  ...
  get routerSettings () {
    ...
    return router?.filteredRouterCaps;
  }
  ...
  async _createPRTPTransportSet ({
    ...
    try {
      ...
      this.transport.appData.mappedType = this._mappedType;
  ...
  async _createWebRTCTransportSet ({
    ...
    try {
      ...
      if (tSettings.initialAvailableOutgoingBitrate == null) {
        tSettings = {...tSettings, initialAvailableOutgoingBitrate: DEFAULT_INITIAL_IN_BW};
      }
  ...
  setMaxOutgoingBitrate(max = 0) {
    if (this.transport
      && typeof this.transport.setMaxOutgoingBitrate === 'function'
      && max > 0
    ) {
  ...
  // @async
  restartIce () {
    if (this.transport == null) throw new Error('transport::not-found');
    if (typeof this.transport.restartIce !== 'function')
      throw new Error('transport::restartIce::not-implemented');

    return this.transport.restartIce().then((iceParameters) => {
      // Update transport options - restartIce generates a new IceParameters set
      this.transportOptions = TransportSet.getWebRTCTransportOpts(this.transport);
