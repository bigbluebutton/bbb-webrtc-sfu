    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (mediaElement) {
          ...
          try {
            const router = Routers.getRouter(routerId);

            if (router) {
              router.activeElements--;
              Logger.info(LOG_PREFIX, 'Router elements decreased',
            ...
            Logger.error(LOG_PREFIX, 'Router cleanup routine failed', {
              errorMessage: error.message, routerId, roomId: room, elementId,
            });
          }
