...
module.exports = class MediasoupAdapter extends EventEmitter {
 ...
 negotiate (roomId, userId, mediaSessionId, descriptor, type, options) {
  ...
  async startRecording (roomId, userId, mediaSessionId, descriptor, type, options) {
    try {
      ...
      const element = MediaElements.getElement(sourceId);
      ...
      recorderMediaModel.adapterElementId = element.id;
      recorderMediaModel.host = element.transportSet.host;


      await element.record(uri);
  ...
  stopRecording (sourceId) {
    ...
    return element ? element.stopRecording() : Promise.resolve();
  }
        ...
        if (mediaElement) {
          ...
          try {
            if (type === 'RecorderEndpoint') {
              return this.stopRecording(elementId);
            }
