...
module.exports = class MediasoupAdapter extends EventEmitter {
  constructor(name, balancer) {
    if (!instance){
      ...
      Workers.createWorkers(NOF_WORKERS, WORKER_SETTINGS);
      instance = this;
    }
  ...
  processOffer (elementId, sdpOffer, parsedSdp, mediaTypes, options)  {
    return new Promise(async (resolve, reject) => {
      try {
        const mediaElement = MediaElements.getElement(elementId);
        if (mediaElement) {
          Logger.trace(LOG_PREFIX, `Direct processing of ${elementId} offer`, { offer: sdpOffer });
          answer = await mediaElement.processSDPOffer(sdpOffer, parsedSdp, mediaTypes, options);
