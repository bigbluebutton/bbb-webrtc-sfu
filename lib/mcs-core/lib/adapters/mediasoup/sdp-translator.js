...
const _getMappedDirectionFromMType = (mTDirection) => {
  switch (mTDirection) {
    case 'recvonly':
      return 'sendonly';
    case 'sendonly':
...
const assembleSDP = (kindParametersMap, {
  ...
  kindParametersMap.forEach((kMap, i) => {
    ...
    if (targetMediaSection) {
      targetMediaSection._mediaObject.direction = _getMappedDirectionFromMType(kMap.direction);
      if (kMap.setup) {
