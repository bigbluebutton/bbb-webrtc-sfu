...
const getMid = (jsonSdp, kind) => {
  const mediaSections = jsonSdp.media || [];
  // Presuming kinds are unitary...
  const mediaLine = mediaSections.find((m) => m.type === kind);
  if (mediaLine && !(mediaLine.mid == null)) {
    if (typeof mediaLine.mid === 'string') return mediaLine.mid;
    if (typeof mediaLine.mid.toString() === 'function') return mediaLine.mid.toString();
  }
...
const extractRTPParamsForProducer = (jsonSdp, kind, msExtendedRtpCaps) => {
  ...
  params.mid = getMid(jsonSdp, kind);
...
const kMapTokMidMap = (kMap) => {
  const kMidMap = {};
  return kMap.reduce((pkMidMap, { kind, offerRtpParameters }, i) => {
    if (typeof pkMidMap[kind] === 'string') return pkMidMap;
    return {
      ...pkMidMap,
      [kind]: offerRtpParameters.mid || i.toString(),
    };
  }, kMidMap);
};
  ...
  kindParametersMap.forEach((kMap) => {
    ...
    if (targetMediaSection) {
      targetMediaSection._mediaObject.direction = _getMappedDirectionFromMType(
        ...
        targetMediaSection._mediaObject?.tainted,
      );
      targetMediaSection._mediaObject.tainted = true;
