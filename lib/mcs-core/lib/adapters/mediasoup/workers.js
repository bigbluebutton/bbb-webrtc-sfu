...
const {
  WORKER_SETTINGS, LOG_PREFIX, WORKER_EXPORT_RESOURCE_USAGE,
} = require('./configs.js');
const {
...
const _fetchWorkerStorage = (mediaType) => {
  ...
  return WORKER_STORAGE.sharedPool;
}
...
const createSharedPoolWorkers = (
  ...
  Logger.info(LOG_PREFIX, `Spawning shared pool workers: ${sharedPoolWorkers}`);

  for (let i = 0; i < sharedPoolWorkers ; i++) {
    createWorker({ workerSettings }).then(worker => {
      storeWorker(worker);
    }).catch(error => {
      Logger.error(LOG_PREFIX, 'Worker creation failed: shared pool',
...
const createDedicatedMediaTypeWorkers = (
  ...
  for (const [mediaType, numberOfWorkers] of Object.entries(dedicatedMediaTypeWorkers)) {
    ...
    Logger.info(LOG_PREFIX, `Spawning ${mediaType} workers: ${numberOfWorkers}`);

    for (let i = 0; i < numberOfWorkers; i++) {
      createWorker({ workerSettings, mediaType }).then(worker => {
        storeWorker(worker);
      }).catch(error => {
        Logger.error(LOG_PREFIX, 'Worker creation failed: dedicated media type',
