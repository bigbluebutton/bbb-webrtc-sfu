    ...
    if (instance == null) {
      ...
      CONFIGURED_ADAPTERS.forEach(a => {
        try {
          ...
          Logger.error('Could not add media server adapter', {
            adapter: a, errorMessage: error.message
          });
        }
      });

      Logger.info('Configured media server adapters', { adapters: this.adapters.map(a => a.name)});
    }
    return instance;
  }
