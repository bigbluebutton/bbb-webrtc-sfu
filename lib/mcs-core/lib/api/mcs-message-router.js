...
module.exports = class MCSRouter {
  ...
  leave (roomId, userId) {
    try {
      return this._mediaController.leave(roomId, userId);
    }
    catch (error) {
  ...
  async restartIce ({ mediaId }) {
    try {
      const descriptor = await this._mediaController.restartIce(mediaId);
      return descriptor;
    } catch (error) {
      throw (this._handleError(error, 'restartIce', { mediaId }));
    }
  }
  ...
  async onEvent (args) {
    ...
    try {
      await this._mediaController.onEvent(eventName, identifier);
    } catch (error) {
  ...
  _handleError (error, operation) {
    ...
    Logger.error(`Rejecting operation ${operation}`, {
      operation, errorCode: code, errorMessage: message,
      details, stack,
    });
  ...
  setupClient(client) {
    ...
    client.on('onEvent', async (args) => {
      try {
        ...
        Logger.error('OnEvent failure', { error });
      }
    });
    ...
    client.on('restartIce', async (args) => {
      let transactionId, mediaId;
      try {
        ({ transactionId, mediaId } = args);
        const descriptor = await this.restartIce(args);
        client.iceRestarted(mediaId, descriptor, { transactionId });
      } catch (error) {
        this._notifyMethodError(client, error, 'restartIce', transactionId);
      }
    });
  }
  ...
  _removeClientFromTracking (client) {
    ...
    if (trackingId && this.clients[trackingId]) {
      Logger.trace(`Removing client from tracking: ${trackingId}`);
      delete this.clients[trackingId];
    }
  }

  _clientEventMapSize () {
    return Object.keys(this.clientEventMap).length;
  }

  _getClientToDispatch (identifier, eventName) {
    const index = `${eventName}:${identifier}`
    const clients = this.clientEventMap[index] || [];

    MCSPrometheusAgent.incrementBy(METRIC_NAMES.DISPATCHED_EVENTS_TOTAL,
      clients.length,
      { event: eventName },
    );
  ...
  _dispatchEvents() {
    MCSPrometheusAgent.setCollectorWithGenerator(
      METRIC_NAMES.REGISTERED_EVENT_CALLBACKS,
      this._clientEventMapSize.bind(this),
    );
