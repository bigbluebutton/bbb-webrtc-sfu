...
module.exports = class MCSRouter {
  ...
  _handleError (error, operation) {
    ...
    Logger.error(LOG_PREFIX, `Rejecting operation ${operation}`, {
  ...
  setupClient(client) {
    ...
    client.on('onEvent', async (args) => {
      try {
        ...
        Logger.error(LOG_PREFIX, 'OnEvent failure', { error  });
      }
    });
  ...
  _removeClientFromTracking (client) {
    ...
    if (trackingId && this.clients[trackingId]) {
      Logger.trace(LOG_PREFIX, `Removing client from tracking: ${trackingId}`);
