...
class BBBWebRTCRecorder extends EventEmitter {
  ...
  constructor(pubChannel, subChannel, {
    notifyRecordingStopOnCrash = true,
  } = {}) {
    super();
    this.started = false;
    ...
    this.notifyRecordingStopOnCrash = notifyRecordingStopOnCrash;
  ...
  _notifyCrash() {
    if (this.notifyRecordingStopOnCrash) {
  ...
  incomingMessage (message) {
    ...
    if (id) {
      switch (id) {
        ...
        case 'getRecordingsResponse': {
          const { requestId } = payload;
          this.emit(`getRecordingsResponse:${requestId}`, payload);
          break;
        }
        case 'recorderStatus': {
          // {
  ...
  _waitForConnection () {
    ...
    return new Promise((resolve) => {
      ...
      const timeout = setTimeout(() => {
        this.removeListener('recorderInstanceStarted', onStarted);
        resolve(false);
      }, REC_CONNECTION_TIMEOUT);

      this.once('recorderInstanceStarted', onStarted);
    });
  }
    ...
    return this._waitForConnection().then((connected) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        try {
          ...
          // _stopHdlr serves two purposes:
          // 1. Pre-resolve: recording stopped before startRecordingResponse arrived
          //    == _cleanup() removes orphaned listeners, reject() settles the promise
          // 2. Post-resolve: recording started OK then stopped unexpectedly
          //    == _cleanup() removes now-unnecessary listeners, reject() is a no-op
  ...
  getRecordings() {
    return this._waitForConnection().then((connected) => {
      ...
      return new Promise((resolve, reject) => {
        const requestTimeout = setTimeout(() => {
          this.removeAllListeners(`getRecordingsResponse:${requestId}`);
          reject(new Error('Get recordings request timeout'));
        }, BBBWebRTCRecorder.REQUEST_TIMEOUT);

        this.once(`getRecordingsResponse:${requestId}`, (payload) => {
          clearTimeout(requestTimeout);
          resolve(payload);
        });
