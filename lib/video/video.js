...
module.exports = class Video extends BaseProvider {
    ...
    if (nativeSubMediaId) {
      try {
        ...
        Logger.error("bbb-webrtc-recorder: native recording subscriber cleanup failure",
          { ...this._getLogMetadata(), error, recordingAdapter: this.getRecordingAdapter() });
      } finally {
  ...
  async stopRecording () {
    // TODO do only what's necessary here
    // TODO use recording stop timestamps from recorder
    if (!this._stopRecordingEventFired) this.sendStopShareEvent();

    if (this._getRecordingAdapter() === 'bbb-webrtc-recorder') return this._stopWebRTCRecorder();
