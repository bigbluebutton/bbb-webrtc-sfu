...
module.exports = class Video extends BaseProvider {
  ...
  async _handleCamUnsubscribeSysMsg () {
    try {
      Logger.info(LOG_PREFIX, 'Disconnecting a subscriber session on CamStreamUnsubscribeSysMsg',
        this._getLogMetadata());
      await this.stop();
    } catch (error) {
      Logger.warn(LOG_PREFIX, 'Failed to disconnect subscriber session on CamStreamUnsubscribeSysMsg',
  ...
  async _handleCamBroadcastStopSysMsg () {
    try {
      Logger.info(LOG_PREFIX, 'Disconnecting a publisher session on CamBroadcastStopSysMsg',
        this._getLogMetadata());
      await this.stop();
    } catch (error) {
      Logger.warn(LOG_PREFIX, 'Failed to disconnect publisher session on CamBroadcastStopSysMsg',
        { ...this._getLogMetadata(), error });
    }
  }
  ...
  _handleIceComponentStateChange (state) {
    ...
    Logger.debug(LOG_PREFIX, `Video ICE component state changed`, {
      ...this._getLogMetadata(),
      elementId,
      iceComponentId,
      iceComponentState
  ...
  _handleIceGatheringDone (state) {
    ...
    Logger.debug(LOG_PREFIX, `Video ICE gathering done`, {
      ...this._getLogMetadata(),
      elementId,
    });
  }
    ...
    switch (name) {
      ...
      case "MediaFlowInStateChange":
        if (details === 'NOT_FLOWING' && this.status !== C.MEDIA_PAUSED) {
          ...
          if (this.notFlowingTimeout) {
            Logger.debug(LOG_PREFIX, "Media FLOWING received while timeout was set, clearing it",
              this._getLogMetadata());
            clearTimeout(this.notFlowingTimeout);
            delete this.notFlowingTimeout;
          }
    ...
    return new Promise((resolve) => {
      ...
      switch (this.status) {
        case C.MEDIA_STOPPED:
          Logger.debug(LOG_PREFIX, 'Video session already stopped',
            this._getLogMetadata());
          return resolve();

        case C.MEDIA_STOPPING:
          Logger.warn(LOG_PREFIX, 'Video session already stopping',
            this._getLogMetadata());
        ...
        case C.MEDIA_STARTING:
          Logger.warn(LOG_PREFIX, 'Video session is still starting. Let it finish then stop it',
            this._getLogMetadata());
          if (!this._stopActionQueued) {
            this._stopActionQueued = true;
            this.once(C.MEDIA_NEGOTIATED, () => {
              Logger.info(LOG_PREFIX, 'Video session: running delayed MEDIA_STARTING stop action',
                this._getLogMetadata());
              this._finishVideoSession(resolve);
            });
          } else {
            this.once(C.MEDIA_STOPPED, () => {
              Logger.info(LOG_PREFIX, 'Video session: delayed stop action executed',
                this._getLogMetadata());
              return resolve();
            });
          }
        ...
        default:
          Logger.info(LOG_PREFIX, 'Stopping video session',
            this._getLogMetadata());
