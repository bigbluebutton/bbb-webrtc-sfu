...
module.exports = class Video extends BaseProvider {
  constructor(_bbbGW, _meetingId, _id, _shared, _connectionId, mcs) {
    super();
  ...
  static setSharedWebcam(id, mediaId) {
    sharedWebcams[id] = mediaId;
  }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (this.shared) {
          const options = {
            descriptor,
            name: this._assembleStreamName('publish', this.id, this.meetingId),
          }

          const { mediaId, answer } = await this.mcs.publish(this.userId, this.meetingId, type, options);
          ...
          const options = {
            descriptor,
            name: this._assembleStreamName('subscribe', this.id, this.meetingId),
          }
          const { mediaId, answer } = await this.mcs.subscribe(this.userId, sharedWebcams[this.id], C.WEBRTC, options);
          this.mediaId = mediaId;
          return resolve(answer);
        }
      }
