...
module.exports = class Video extends BaseProvider {
  ...
  _handleBBBRecordingStatusChanged ({ recording }) {
    ...
    if (!this.isRecording && this.shouldRecord()) {
      Logger.info('BBB recording status changed to true, starting recording', {
        ...
        recordUserCameras: this.recordUserCameras,
  ...
  restartIce () {
    if (this.mediaId) return this.mcs.restartIce(this.mediaId);
      ...
      case "MediaFlowInStateChange":
        if (details === 'NOT_FLOWING' && this.status !== C.MEDIA_PAUSED) {
          ...
          if (this.status !== C.MEDIA_STARTED) {
            ...
            this.sendPlayStart();
            Logger.info("Video session media is FLOWING", this._getLogMetadata());
  ...
  async _stopWebRTCRecorder () {
    ...
    let timestampHR = hrTime();

    if (recordingSessionId) {
      try {
        // TODO use reason
        ({ timestampUTC, timestampHR } = await Recorder.stopRecording(recordingSessionId));
      } catch(error) {
    ...
    return { timestampHR, timestampUTC };
  }
...
this._getLogMetadata());
  ...
  async start (sdpOffer, options = {}) {
    try {
      Logger.info("Starting video session", this._getLogMetadata());
      ...
      if (this.status === C.MEDIA_STOPPED) {
        ...
        if (SHOULD_RECORD && this.shared) {
          const {
            ...
            recordUserCameras,
          } = await this.probeForRecordingStatus(this.meetingId, this.bbbUserId);

          this.isMeetingRecorded = recorded;
          this.isMeetingRecording = recording;
          this.recordUserCameras = typeof recordUserCameras === 'boolean'
            ? recordUserCameras
            : true;
  ...
  async _finishVideoSession (resolver) {
    ...
    const logMetadata = this._getLogMetadata();

    // Send the notification early to BBB to guarantee that any mishap in subsequent
    // stop procedures do not leave stale cameras in BBB
    if (this.shared) {
    ...
    Logger.info('Stopped video session', logMetadata);
    return resolver();
  }
    ...
    return new Promise((resolve) => {
      ...
      switch (this.status) {
        case C.MEDIA_STOPPED: {
          Logger.debug('Video session already stopped', logMetadata);
          return resolve();
        }

        case C.MEDIA_STOPPING: {
          Logger.warn('Video session already stopping', logMetadata);

          this.once(C.MEDIA_STOPPED, () => {
            Logger.info('Calling delayed stop resolution for queued stop', logMetadata);
        ...
        default:
          Logger.info('Stopping video session', logMetadata);
