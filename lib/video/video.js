...
module.exports = class Video extends BaseProvider {
  ...
  start (sdpOffer) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        this.mcs.on(C.MEDIA_STATE, (event) => {
          this._mediaStateWebRTC(event, this.mediaId);
        });

        this.mcs.on(C.MEDIA_STATE_ICE, (event) => {
  ...
  _addMCSMedia (type, descriptor) {
    return new Promise(async (resolve, reject) => {
      try {
        if (this.shared) {
          const { mediaId, answer } = await this.mcs.publish(this.userId, this.meetingId, type, { descriptor });
          this.mediaId = mediaId;
          sharedWebcams[this.id] = this.mediaId;
          return resolve(answer);
        }
        else if (sharedWebcams[this.id]) {
          const { mediaId, answer } = await this.mcs.subscribe(this.userId, sharedWebcams[this.id], C.WEBRTC, { descriptor });
          this.mediaId = mediaId;
          return resolve(answer);
        }
      }
