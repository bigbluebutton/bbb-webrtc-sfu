...
module.exports = class VideoManager extends BaseManager {
  ...
  _killConnectionSessions (connectionId) {
    Object.keys(this._sessions).forEach((sessionId) => {
      ...
      if (session && session.connectionId === connectionId) {
        ...
        queue.push(() => { return this._stopSession(sessionId); });
      }
    });
  }
  ...
  async handleStart (message) {
    ...
    // FIXME review this
    const shared = role === 'share' ? true : false;
    let video, iceQueue;
    ...
    if (bitrate != null) {
      VideoManager.addBwToSpec(mediaSpecs, bitrate);
    }

    return video.start(sdpOffer, mediaSpecs)
      .then(sdpAnswer => {
        Logger.debug(this._logPrefix, `Video session started`,
        ...
        video.once(C.MEDIA_SERVER_OFFLINE, async (event) => {
          const errorMessage = this._handleError(this._logPrefix, connectionId, cameraId, role, errors.MEDIA_SERVER_OFFLINE);
