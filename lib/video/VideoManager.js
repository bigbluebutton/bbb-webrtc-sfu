...
module.exports = class VideoManager extends BaseManager {
  ...
  _registerQueueFailure (reason) {
    PrometheusAgent.increment(SFUV_NAMES.QUEUE_FAILURES, { reason });
  }
  ...
  _trackExternalWebcamSources() {
    this._bbbGW.on(C.USER_CAM_BROADCAST_STARTED_2x, payload => {
      ...
      if (userId.match(/^v_*/ig)) {
        ...
        const source = { mediaId: normalizedStreamName, mediaSpecs: DEFAULT_MEDIA_SPECS };
        Video.setSource(stream, source);
        Video.setSource(userId, source);
      }
    });
  }
  ...
  async handleStart (request) {
    ...
    try {
      ...
      const sdpAnswer = await video.start(sdpOffer, { bitrate: request?.bitrate });
      ...
      Logger.error("Video manager: session start failed", {
  ...
  handleRestartIce (message) {
    ...
    if (session) {
      const sessionMetadata = session._getLogMetadata();
      return session.restartIce().then((sdp) => {
        ...
        Logger.error('Video manager: ICE restart failed', {
          errorMessage: error.message,
          errorCode: error.code,
          sessionMetadata,
        });
