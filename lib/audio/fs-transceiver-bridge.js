...
const BRIDGE_MODE = config.has('listenOnlyGlobalAudioMode') ? config.get('listenOnlyGlobalAudioMode')
  : 'RTP';
...
module.exports = class FSTransceiverBridge extends EventEmitter {
  ...
  _onBridgeMediaStateChange (event, endpoint) {
    ...
    switch (name) {
      case "MediaFlowOutStateChange":
        Logger.info(LOG_PREFIX, `RTP static transceiver relay session ${mediaId} received MediaFlowOut`,
          { ...this._getFullLogMetadata(), userId: this.userId, mediaId, state });
        break;
      case "MediaFlowInStateChange":
        Logger.info(LOG_PREFIX, `RTP static transceiver relay session ${mediaId} received MediaFlowIn`,
          { ...this._getFullLogMetadata(), userId: this.userId, mediaId, state });
        break;
      default:
        break;
  ...
  async _startTransceiverBridge (transceiverSourceId) {
    ...
    Logger.info(LOG_PREFIX, 'Starting FS static transceiver bridge', this._getFullLogMetadata());
    ...
    try {
      ...
      if (!this._negotiated && this.bridgeMediaStatus === C.MEDIA_STOPPED) {
        ...
        Logger.info(LOG_PREFIX, 'FS transceiver bridge relay created',
          { ...this._getFullLogMetadata(), mediaId: this.bridgeMediaId, userId: this.userId });
      }
    } catch (error) {
      Logger.error(LOG_PREFIX, 'Error starting FS transceiver bridge relay',
