...
module.exports = class FSTransceiverBridge extends EventEmitter {
  ...
  heldOnStart () {
    return this._getUAString() === 'bbb-webrtc-sfu-mhos';
  }

  _getUAString () {
    if (this.muteOnStart || this.role === 'passive-sendrecv') {
      // mute-on-start - see bbb_sfu_muos.xml dialplan
      if (!this.transparentListenOnly) return 'bbb-webrtc-sfu-muos';

      // mute-hold-on-start - see bbb_sfu_muos.xml + bbb_conference.xml
      // dialplans
      return 'bbb-webrtc-sfu-mhos';
    }

    // default (no extra dialplan magic)
    return 'bbb-webrtc-sfu';
  }
  ...
  _onBridgeMediaStateChange (event, endpoint) {
    ...
    switch (name) {
      ...
      case "MediaFlowInStateChange":
        Logger.debug('FS transceiver bridge received MediaFlow state',
          { ...this._getFullLogMetadata(), state });
        break;

      case C.MEDIA_SERVER_OFFLINE:
        Logger.error('CRITICAL: FS transceiver bridge received MEDIA_SERVER_OFFLINE',
      ...
      if (!this._negotiated && this.bridgeMediaStatus === C.MEDIA_STOPPED) {
        ...
        const fsOptions = {
          ...
          adapterOptions: {
            ...
            userAgentString: this._getUAString(),
          }
        }
        ...
        this.mcs.onEvent(C.MEDIA_STATE, fsMediaId, (event) => {
          this._onBridgeMediaStateChange(event, fsMediaId);
        });
        ...
        Logger.info('FS transceiver bridge started', this._getFullLogMetadata());
      }
    } catch (error) {
      Logger.error('FS transceiver bridge: start failure',
  ...
  async dtmf (tones) {
    ...
    if (this.fsMediaId) {
      try {
        ...
        Logger.debug('Sent DTMF tones',
          { ...this._getFullLogMetadata(), tones }
        );
      } catch (error) {
        Logger.error('DTMF failed',
  ...
  stop () {
    ...
    return this.mcs.leave(this.voiceBridge, this.userId).catch(error => {
      Logger.warn('Failed to stop transceiver bridge; this may cause a leak', {
        ...this._getFullLogMetadata(), errorMessage: error.message,
      });
    }).finally(() => {
      this._negotiated = false;
