...
module.exports = class ClientAudioStTransceiver extends BaseProvider {
  ...
  getMicrophonePermission (meetingId, voiceBridge, userId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.bbbGW.once(C.GET_MIC_PERM_RESP_MSG+this.connectionId, onResp);
  ...
  async start (sdpOffer) {
    ...
    try {
      await this.getMicrophonePermission(this.meetingId, this.voiceBridge, this.userId);
    } catch (error) {
      const normalizedError = this._handleError(LOG_PREFIX, error, "sendrecv", this.userId);
      Logger.error('Client audio transceiver failed: unauthorized',
