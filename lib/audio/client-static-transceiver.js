...
module.exports = class ClientAudioStTransceiver extends BaseProvider {
  constructor(
    ...
    this._getConsumerBridge = options.getConsumerBridge;
    this.muteOnStart = options.muteOnStart;
    this.transparentListenOnly = options.transparentListenOnly;
    ...
    this.mediaId = null;
    this.transceiverMode = C.SEND_RECV_ROLE;
    ...
    this._onConsumerBridgeRestarting = this.onConsumerBridgeRestarting.bind(this);
    this._onConsumerBridgeRestarted = this.onConsumerBridgeRestarted.bind(this);
    this._onConsumerBridgeStarted = this.onConsumerBridgeStarted.bind(this);
    // Delayed assignment of consumerBridge to avoid losing scope on the above methods
    this.consumerBridge = options.consumerBridge;

    this.mcs.on(C.MCS_DISCONNECTED, this.handleMCSCoreDisconnection);
    this.transceiverBridge.on(C.MEDIA_SERVER_OFFLINE, this.handleMCSCoreDisconnection);
  }
  ...
  set consumerBridge (bridge) {
    if (bridge == null && this._consumerBridge) {
      this._consumerBridge.removeSubscriber(this.sessionId);
      this._consumerBridge.removeListener(C.MEDIA_RESTARTING, this._onConsumerBridgeRestarting);
      this._consumerBridge.removeListener(C.MEDIA_STARTED, this._onConsumerBridgeStarted);
      this._consumerBridge.removeListener(C.MEDIA_RESTARTED, this._onConsumerBridgeRestarted);
    }

    this._consumerBridge = bridge;

    if (this._consumerBridge) {
      this._consumerBridge.on(C.MEDIA_RESTARTING, this._onConsumerBridgeRestarting);
      this._consumerBridge.on(C.MEDIA_RESTARTED, this._onConsumerBridgeRestarted);
      this._consumerBridge.on(C.MEDIA_STARTED, this._onConsumerBridgeStarted);
    }
  }

  get consumerBridge () {
    return this._consumerBridge;
  }

  onConsumerBridgeRestarting () {
    this.emit(C.MEDIA_CONSUMER_BRIDGE_RESTARTING);
  }

  onConsumerBridgeRestarted () {
    this.emit(C.MEDIA_CONSUMER_BRIDGE_RESTARTED);
  }

  onConsumerBridgeStarted () {
    this.emit(C.MEDIA_CONSUMER_BRIDGE_STARTED);
  }
  ...
  _getFullLogMetadata () {
    return {
      ...
      transceiverMode: this.transceiverMode,
      listenOnly: false,
    };
  }
  ...
  _onSubscriberMediaFlowing () {
    Logger.info('Client audio is FLOWING',
  ...
  async toggleListenOnlyMode (enabled) {
    if (enabled && this.transceiverMode !== C.RECV_ROLE) {
      Logger.info("Transparent listen only ON", { ...this._getFullLogMetadata() });

      if (!this._hasConsumerBridge()) {
        try {
          this.consumerBridge = await this._getConsumerBridge();
        } catch (error) {
          Logger.error('Client audio transceiver failure: consumer bridge', {
            ...this._getFullLogMetadata(),
            error,
          });
          throw this._handleError(LOG_PREFIX, error, "sendrecv", this.userId);
        }
      }

      return this.mcs.connect(
        this.consumerBridge.bridgeMediaId,
        [this.mediaId],
        'AUDIO'
      ).then(() => {
        this.transceiverMode = C.RECV_ROLE;
        this.consumerBridge.addSubscriber(this.sessionId);
        Logger.debug('Client audio transceiver: transceiver is now in RECV_ROLE', {
          ...this._getFullLogMetadata(),
          sourceId: this.consumerBridge?.bridgeMediaId,
          sinkId: this.mediaId,
        });

        return true;
      });
    } else if (!enabled && this.transceiverMode !== C.SEND_RECV_ROLE) {
      Logger.info("Transparent listen only OFF", { ...this._getFullLogMetadata() });
      return this.mcs.connect(
        ...
        this.transceiverMode = C.SEND_RECV_ROLE;
        this.consumerBridge.removeSubscriber(this.sessionId);
        Logger.debug('Client audio transceiver: transceiver is now in SEND_RECV_ROLE', {
          ...this._getFullLogMetadata(),
          sourceId: this.transceiverBridge?.bridgeMediaId,
          sinkId: this.mediaId,
        });
      ...
      Logger.debug("Transparent listen only mode already in desired state", {
        ...this._getFullLogMetadata(),
        enabled,
      });
    }
  ...
  _hasConsumerBridge () {
    return this.consumerBridge
      && this.consumerBridge.bridgeMediaId
      && !this.consumerBridge.isExpired();
  }

  async _negotiateTransceiver(sdpOffer) {
    const hasConsumerBridge = this._hasConsumerBridge();
    let mediaId, answer;
    ...
    const options = {
      ...
      mediaProfile: 'audio',
      adapterOptions: {
        overrideRouterCodecs: !hasConsumerBridge,
        dedicatedRouter:  !hasConsumerBridge,
        rtpHeaderExtensions: AUDIO_RTP_HDR_EXTS,
      }
    }

    if (hasConsumerBridge) {
    ...
    try {
      ...
      if (!this._hasConsumerBridge()) {
        ...
        if (!this.transceiverBridge.heldOnStart()) {
          await this.mcs.connect(this.transceiverBridge.bridgeMediaId, [this.mediaId], 'AUDIO');
        } else {
          this.transceiverMode = C.RECV_ROLE;
          this.consumerBridge.addSubscriber(this.sessionId);
          Logger.info('Client audio transceiver: HELD ON START, listen only mode enabled',
            this._getFullLogMetadata());
        }
      }
    } catch (error) {
  ...
  async stop () {
    ...
    if (this.consumerBridge) this.consumerBridge = null;
