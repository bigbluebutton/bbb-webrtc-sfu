...
module.exports = class ClientAudioStTransceiver extends BaseProvider {
  constructor(
    ...
    this.bbbGW.on(C.USER_JOINED_VOICE_CONF_EVT_MSG+this.userId, (event) => {
      // Associate voiceUserId and channel uuid to this session
      this.voiceUserId = event.voiceUserId;
    ...
    this._switchToRecvOnlyTimer = null;
  ...
  async _negotiateTransceiver(sdpOffer) {
    ...
    // Listen for UserMutedVoiceEvtMsg to switch between sendrecv and recvonly
    // based on whether the user is muted or not
    //
    // If muted, set a timer at SWITCH_INTERVAL to connect the consumerBridge to mediaId
    // If unmuted, cancel the timer if one exists. Otherwise, connect mediaId back to
    // transceiverBridge
    this.bbbGW.on(C.USER_MUTED+this.userId, async (event) => {
      Logger.trace("User muted event received", event);
      if (event.muted) {
        Logger.trace("User muted, setting timer", this.userId);
        this._switchToRecvOnlyTimer = setTimeout(async () => {
          Logger.trace("Switching to recvonly", this.userId);
          this.mcs.connect(this.consumerBridge.bridgeMediaId, [this.mediaId], 'AUDIO');
          this.sendHoldCmdToAkka(this.uuid, true);
          this._switchToRecvOnlyTimer = null;
        }, SWITCH_INTERVAL);
      } else {
        Logger.trace("User unmuted", this.userId);
        if (this._switchToRecvOnlyTimer) {
          Logger.trace("Cancelling timer", this.userId);
          clearTimeout(this._switchToRecvOnlyTimer);
          this._switchToRecvOnlyTimer = null;
        } else {
          Logger.trace("Switching to sendrecv", this.userId);
          this.mcs.connect(this.transceiverBridge.bridgeMediaId, [this.mediaId], 'AUDIO');
          this.sendHoldCmdToAkka(this.uuid, false);
        }
      }
    });
  ...
  sendHoldCmdToAkka (voiceUserId, hold) {
    const msg = Messaging.generateHold(
      this.meetingId, this.voiceBridge, voiceUserId, hold
    );
    this.bbbGW.publish(msg, 'to-voice-conf-redis-channel');
    Logger.trace("Sent hold command to akka", msg);
  }
