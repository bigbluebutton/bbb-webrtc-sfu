...
module.exports = class ClientAudioStTransceiver extends BaseProvider {
  ...
  _getPartialLogMetadata () {
    return {
      ...
      status: this.transceiverBridge.bridgeMediaStatus,
  ...
  async start (sdpOffer) {
    ...
    Logger.info(LOG_PREFIX, 'Starting new client audio transceiver',
      this._getFullLogMetadata());

    try {
      const sdpAnswer = await this._publishFullAudio(sdpOffer);
      return sdpAnswer;
    } catch (error) {
      const normalizedError = this._handleError(LOG_PREFIX, error, "recv", this.userId);
  ...
  processAnswer (answer) {
    Logger.debug(LOG_PREFIX, 'Processing client audio transceiver answer',
      this._getFullLogMetadata());
  ...
  async stop () {
    Logger.info(LOG_PREFIX, 'Stopping client audio transceiver', this._getPartialLogMetadata());

    try {
      await this._stop();
    } catch (error) {
      Logger.error(LOG_PREFIX, 'Client audio transceiver failure: stop',
        { ...this._getFullLogMetadata(), error });
    }
