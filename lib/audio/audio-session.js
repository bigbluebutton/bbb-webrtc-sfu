...
const { isSendRecvSession } = require('./utils.js');
...
module.exports = class AudioSession extends BaseProvider {
  constructor(
    ...
    this.muteOnStart = options?.muteOnStart ?? false;
    ...
    this._onConsumerBridgeRestarting = this._onConsumerBridgeRestarting.bind(this);
    this._onConsumerBridgeRestarted = this._onConsumerBridgeRestarted.bind(this);
  ...
  set blockTransparentListenOnly (block) {
    Logger.debug('Audio session: blocking transparent listen only mode', {
      ...this._getFullLogMetadata(),
      block,
    });
    this._blockTransparentListenOnly = block;
  }

  get blockTransparentListenOnly () {
    return this._blockTransparentListenOnly;
  }
  ...
  _onConsumerBridgeRestarting () {
    this.blockTransparentListenOnly = true;
    // Ensure the user is not in listen only mode when the consumer bridge
    // is restarting or they'll be deaf.
    this.toggleListenOnlyMode({
      meetingId: this.meetingId,
      voiceConf: this.voiceBridge,
      callerNum: this.callerId,
      enabled: false,
      force: true,
  ...
  _onConsumerBridgeRestarted () {
    // Unblock transparent listen only mode after the consumer bridge has been restarted
    // Only possible if dynamicGlobalAudio is enabled - otherwise, the consumer bridge
    // may end up in a different router and re-connecting to it won't be possible.
    if (DYNAMIC_GLOBAL_AUDIO) {
      this.blockTransparentListenOnly = false;
      // Transparent listen only will pick itself up in the next cycle
    }
  }
  ...
  _getFullLogMetadata () {
    return {
      ...
      isTransparentListenOnlyAllowed: this.transparentListenOnlyAllowed,
      blockTransparentListenOnly: this.blockTransparentListenOnly,
    };
  }
  ...
  _trackClientEndpointEvents () {
    if (this.clientEndpoint) {
      ...
      this.clientEndpoint.on(C.MEDIA_CONSUMER_BRIDGE_RESTARTING, this._onConsumerBridgeRestarting);
      this.clientEndpoint.on(C.MEDIA_CONSUMER_BRIDGE_RESTARTED, this._onConsumerBridgeRestarted);
    }
  }
    ...
    if (this.userId === userId && callState === 'IN_CONFERENCE') {
      if (voiceConf === this.voiceBridge) {
        ...
        if (origCalledDest === this.voiceBridge) {
          Logger.info('Audio session: user in main conference', {
            ...this._getFullLogMetadata(),
            voiceConf,
            origCalledDest,
          });
        ...
        if (origCalledDest === this.voiceBridge) {
          ...
          // Ensure the user is not in listen only mode when transferred
          // (audio-only) to a breakout room.
          this.toggleListenOnlyMode({
            meetingId: this.meetingId,
            voiceConf: this.voiceBridge,
            callerNum: this.callerId,
            enabled: false,
            force: true,
            reason: 'user_transferred_to_breakout',
          });

          if (this.muteOnStart) {
            // Ensure the user is UNMUTED when transferred (audio-only) to
            // a breakout room. Since muteOnStart is applied on the dialplan
            // extension and we can't really differentiate it on transfer,
            // we need to unmute the user here since they won't be able to
            // themselves.
            Logger.info('Audio session: ensure user is unmuted in breakout room', {
              ...this._getFullLogMetadata(),
              voiceConf,
              origCalledDest,
              voiceUserId,
            });
            this._muteUserInVoiceConf(voiceConf, voiceUserId, false);
          }
        }
      }
    }
  ...
  async transceive(sdpOffer) {
    ...
    this.clientEndpoint = new ClientAudioStTransceiver(
      ...
      this.role, {
        ...
        transparentListenOnly: this.isTransparentListenOnlyCapable(),
      }
    );
  ...
  async start (sdpOffer) {
    ...
    try {
      const sdpAnswer = isSendRecvSession(this.role)
  ...
  async stop (reason = 'normal_clearing') {
    ...
    if (this.clientEndpoint && typeof this.clientEndpoint.stop) {
      ...
      if (isSendRecvSession(this.role)
  ...
  _muteUserInVoiceConf (voiceBridge, voiceUserId, mute) {
    const msg = Messaging.generateMuteUserInVoiceConfSysMsg(
      this.meetingId,
      voiceBridge,
      voiceUserId,
      mute,
    ...
    this.bbbGW.publish(msg, C.TO_VOICE_CONF);
  }
  ...
  async toggleListenOnlyMode ({
    ...
    const callerIdPrefix = `${this.userId}_${this.clientSessionNumber}`;

    if (!callerNum.startsWith(callerIdPrefix)) {
      Logger.warn('Audio session: toggle recv-only mode ignored, callerId mismatch', {
        ...this._getFullLogMetadata(this.connectionId),
        callerIdPrefix,
        callerNum,
        enabled,
    ...
    Logger.debug('Audio session: toggling recv-only mode', {
      ...this._getFullLogMetadata(this.connectionId),
      meetingId,
      voiceConf,
      enabled,
