...
module.exports = class ClientAudioConsumer extends BaseProvider {
  constructor(
    ...
    this.consumerBridge.on(C.MEDIA_SERVER_OFFLINE, this.handleMCSCoreDisconnection);
  }
  ...
  set consumerBridge (bridge) {
    ...
    if (bridge) bridge.addSubscriber(this.sessionId);
  ...
  _onSubscriberMediaFlowing () {
    Logger.debug('Client audio consumer is FLOWING',
  ...
  _onSubscriberMediaNotFlowing () {
    Logger.debug('Client audio consumer is NOT_FLOWING',
      this._getFullLogMetadata());
    this.setMediaFlowingTimeout();
  }

  _onSubscriberMediaNotFlowingTimeout () {
    Logger.error('Client audio consumer NOT_FLOWING timeout reached',
      this._getFullLogMetadata());
    this.sendToClient({
      type: 'audio',
      connectionId: this.connectionId,
  ...
  setMediaFlowingTimeout () {
    if (!this._mediaFlowingTimeout) {
      Logger.debug('Client audio consumer NOT_FLOWING timeout set',
        this._getFullLogMetadata());
  ...
  _onSubscriberMediaConnected () {
    Logger.info('Client audio consumer is CONNECTED',
      this._getFullLogMetadata());
    this.clearMediaStateTimeout();
  }

  _onSubscriberMediaDisconnected () {
    Logger.warn('Client audio consumer is DISCONNECTED',
      this._getFullLogMetadata());
    this.setMediaStateTimeout();
  }

  _onSubscriberMediaDisconnectedTimeout () {
    Logger.error('Client audio consumer DISCONNECTED timeout reached',
      this._getFullLogMetadata());
  ...
  setMediaStateTimeout () {
    if (!this._mediaStateTimeout) {
      Logger.warn('Client audio consumer media state timeout set',
        this._getFullLogMetadata());
  ...
  _handleMediaStateChanged (state, logMetadata) {
    ...
    Logger.trace('Client audio consumer media state changed', {
      ...logMetadata,
      elementId,
      mediaState: details,
    });
  ...
  _mediaStateWebRTC (event, targetMediaId) {
    ...
    switch (name) {
      ...
      case "MediaFlowInStateChange":
        Logger.trace('Client audio consumer received MediaFlow state',
          { ...logMetadata, state });
      ...
      case C.MEDIA_SERVER_OFFLINE:
        Logger.error('CRITICAL: Client audio consumer received MEDIA_SERVER_OFFLINE',
          { ...logMetadata, event });
  ...
  async start (sdpOffer) {
    ...
    try {
      ...
      Logger.error('Client audio consumer failure: mcs-core join', {
        ...this._getFullLogMetadata(),
    ...
    try {
      ...
      const normalizedError = this._handleError(LOG_PREFIX, error, "recv", this.userId);
      Logger.error('GLOBAL_AUDIO subscribe failed', {
        ...this._getFullLogMetadata(),
        error: normalizedError
      });
  ...
  async _subscribeToGlobalAudio (sdpOffer) {
    ...
    try {
      ...
      Logger.error('Client audio consumer failure: GLOBAL_AUDIO subscription', {
        ...this._getFullLogMetadata(),
        subscribeError,
      });
    ...
    this._flushCandidatesQueue();
    Logger.info('Client consuming from global audio',
      this._getFullLogMetadata());
    return answer;
  }
  ...
  async stop () {
    ...
    if (this.mediaId && this.mcsUserId) {
      try {
        ...
        Logger.warn('Client audio consumer: stop failure',
          { ...this._getFullLogMetadata(), errorMessage: error.message });
      }
