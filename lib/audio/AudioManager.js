...
module.exports = class AudioManager extends BaseManager {
  ...
  _handleStartError (error, sessionId) {
  ...
  async handleStart (message) {
    ...
    // FIXME straighten this source audio stuff. Put it in start lifecycle queue.
    if (session == null) {
      ...
      try {
        await session.upstartSourceAudio(sdpOffer, caleeName);
      } catch (error) {
        const errorMessage = this._handleError(this._logPrefix, connectionId, caleeName, C.RECV_ROLE, error);
        this._handleStartError(errorMessage, sessionId);
    ...
    return session.start(sessionId, connectionId, sdpOffer, caleeName, userId, userName)
      .then(sdpAnswer => {
        ...
        session.once(C.MEDIA_SERVER_OFFLINE, async (event) => {
          ...
          return this._handleStartError(errorMessage, sessionId);
        });
