...
module.exports = class AudioReqHdlr extends BaseManager {
  ...
  async handleStart (message) {
    ...
    return session.start(sdpOffer).then(sdpAnswer => {
      ...
      Logger.info(this._logPrefix, 'Audio session started',
    ...
    if (session) {
      return this._stopSession(connectionId)
        .then(() => {
          ...
          Logger.info(this._logPrefix, 'Audio session destroyed',
            metadata);
        })
        .catch((error) => {
          this._deleteIceQueue(this._getReqIdentifier(sessionId, connectionId));
          Logger.error(this._logPrefix, 'CRITICAL: Audio session destroy failure', {
            errorMessage: error.message,
            errorCode: error.code,
            ...metadata,
          });
  ...
  handleSubscriberAnswer (message) {
    ...
    if (session) {
      ...
      session.processAnswer(answer).catch(error => {
        Logger.error(this._logPrefix,  'Audio session: remote description processing failed', {
          errorMessage: error.message,
          errorCode: error.code,
          metadata,
        });
  ...
  _handleInvalidRequest (message = {}) {
    ...
    Logger.debug(this._logPrefix, 'Invalid request', {
      ...AudioReqHdlr.getMetadataFromMessage(message),
      errorMessage: error.message,
    });
