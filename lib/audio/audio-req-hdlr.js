...
module.exports = class AudioReqHdlr extends BaseManager {
  ...
  _trackHoldUnholdEvents () {
    this._bbbGW.addSubscribeChannel(C.FROM_VOICE_CONF);
    this._bbbGW.addSubscribeChannel(C.TO_VOICE_CONF);
    ...
    this._bbbGW.on(C.TOGGLE_LISTEN_ONLY_MODE_SYS_MSG, (payload) => {
    ...
    this._bbbGW.on(C.CHANNEL_HOLD_CHANGED_VOICE_CONF_EVT_MSG, ({ header, body }) => {
  ...
  _trackMeetingEvents () {
    ...
    if (SFUA_NAMES.TIME_TO_HOLD || SFUA_NAMES.TIME_TO_UNHOLD) {
