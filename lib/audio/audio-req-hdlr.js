...
const AudioMetricsObserver = require('./metrics/observer.js');

const AUDIO_MEDIA_SERVER = config.get('audioMediaServer');
const ICE_RESTART = config.has('allowIceRestart')
  ? config.get('allowIceRestart')
  : false;
...
module.exports = class AudioReqHdlr extends BaseManager {
  ...
  static genCallerIdNum (request = {}) {
    ...
    const suffix = isSendRecvSession(role) ? userName : 'GLOBAL_AUDIO';
    let decodedSuffix = suffix.replace(/\+/g, ' ');

    try {
      decodedSuffix = decodeURIComponent(decodedSuffix);
    } catch (error) {
      Logger.error('Failed to decode callerIdNum', {
        userId,
        clientSessionNumber,
        role,
        userName,
    ...
    return `${userId}_${clientSessionNumber}-bbbID-${decodedSuffix}`;
}
  ...
  _registerQueueFailure (reason) {
    PrometheusAgent.increment(SFUA_NAMES.QUEUE_FAILURES, { reason });
  }

  _setMetrics() {
    if (this._metricsObserver == null) {
      this._metricsObserver = new AudioMetricsObserver(this._bbbGW, {
        sessions: this.sessions,
      });
      this._metricsObserver.start();
    }
  ...
  _disconnectAllUsers (meetingId) {
    if (!meetingId) {
      Logger.warn('Disconnecting all audio sessions failed: invalid meetingId', {
        meetingId
      });
      return;
    }
  ...
  _handleFatalFailure (sessionId, connectionId, role, rawError, reqId = 'event') {
    ...
    this._closeSession(sessionId, {
      sessionMetadata,
      reason: `fatal_failure_${rawError.message || error.message || error.code}`,
    });
  ...
  async handleStart (message) {
    ...
    try {
      ...
      const { muteOnStart = false } = await this._getPermission({
        userId,
        meetingId,
        voiceBridge,
        role,
      ...
      this._metricsObserver.onSessionStart(userId, sessionId);
  ...
  _closeSession (sessionId, options = {}) {
    ...
    if (session) {
      return this._stopSession(sessionId, { reason })
        .then(() => {
          this._deleteIceQueue(sessionId);
          Logger.info('Audio session destroyed', { ...sessionMetadata, reason });
        })
        .catch((error) => {
          this._deleteIceQueue(sessionId);
        ...
        .finally(() => {
          this._metricsObserver.onSessionEnd(session?.userId);
        });
    }
  ...
  handleRestartIce (message) {
    ...
    if (session && ICE_RESTART) {
      const sessionMetadata = AudioReqHdlr.getMetadataFromMessage(message);
      return session.restartIce().then((sdp) => {
        this.sendToClient({
          type: 'audio',
          connectionId,
