 ...
 */
  ...
  */
  ...
  async stopListener(id) {
    ...
    Logger.info(LOG_PREFIX, `Stopping listener ${userId}`, { userId, mcsUserId, connectionId: id });
    ...
    if (listener) {
      try {
        ...
        Logger.warn(LOG_PREFIX, `Error on unsubscribing listener media ${listener} of user ${mcsUserId} at room ${this.voiceBridge}`, { error });
      }
    }
    ...
    if (mcsUserId) {
      try {
        ...
        Logger.warn(LOG_PREFIX, `Error on leaving listener ${mcsUserId} MCS user of room ${this.voiceBridge}`, { error });
      }
    }
  ...
  async stop () {
    Logger.info(LOG_PREFIX, `Session-wide stop for room ${this.voiceBridge}, releasing everything`);
    try {
  ...
  async _stopSourceAudio () {
    try {
      ...
      Logger.warn(LOG_PREFIX, `Error on stopping source audio ${this.voiceBridge} with MCS user ${this.userId}`, { error });
    }
  ...
  sendUserDisconnectedFromGlobalAudioMessage(connectionId) {
    ...
    if (user) {
      ...
      Logger.info(LOG_PREFIX, `Sending global audio disconnection for user ${userId}`, { userId, userName, connectionId });
  ...
  sendUserConnectedToGlobalAudioMessage(connectionId) {
    ...
    if (user) {
      ...
      Logger.info(LOG_PREFIX, `Sending global audio connection for user ${userId}`, { userId, userName, connectionId });
