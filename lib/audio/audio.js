...
module.exports = class Audio extends BaseProvider {
  constructor(_bbbGW, _id, voiceBridge, mcs) {
    super();
 ...
 */
  ...
  async start (sessionId, connectionId, sdpOffer, caleeName, userId, userName, callback) {
    ...
    try {
      ...
      const sdpAnswer = await this._subscribeToGlobalAudio();
  ...
  _upstartGlobalAudio (sdpOffer, caleeName) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const { mediaId, answer } = await this.mcs.publish(this.userId,
          this.voiceBridge,
          'RtpEndpoint',
  ...
  _subscribeToGlobalAudio (sdpOffer) {
    return new Promise(async (resolve, reject) => {
      try {
