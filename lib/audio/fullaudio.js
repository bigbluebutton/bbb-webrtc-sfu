...
module.exports = class Audio extends BaseProvider {
  ...
  _startFullAudioBridge(transportType, connectionId) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        const { mediaId: clientMediaId } = this.audioEndpoints[connectionId];
        ...
        const { mediaId: fsMediaId, answer: fullAudioAnswer } = await this.mcs.publish(
          this.userId,
          this.voiceBridge,
          transportType,
          fullAudioOptions
  ...
  async start (sessionId, connectionId, sdpOffer, userId, role, caleeName) {
    ...
    try {
      let sdpAnswer = (role === 'sendrecv')
        ? await this._publishFullAudio(sdpOffer, connectionId, role)
        : await this._subscribeToGlobalAudio(sdpOffer, connectionId)

      await this._waitForFullAudio(connectionId);
      const { mediaId: clientMediaId } = this.audioEndpoints[connectionId];
      sdpAnswer = await this.mcs.consume(this.sourceAudio, clientMediaId, 'AUDIO');
      this.mcs.connect(clientMediaId, [this.sourceAudio], 'AUDIO');
      this.mcs.connect(this.sourceAudio, [clientMediaId], 'AUDIO');
  ...
  async _publishFullAudio(sdpOffer, connectionId, role) {
    ...
    if (this.fsMediaId) {
      try {
        ...
        Logger.debug(LOG_PREFIX, 'Sent DTMF tones',
          { ...this._getFullLogMetadata(connectionId), tones }
        );
      } catch (error) {
