...
module.exports = class FSConsumerBridge extends EventEmitter {
  ...
  constructor(mcs, voiceBridge, consumingAdapter) {
    super();
    this.mcs = mcs;
    this.voiceBridge = voiceBridge;
    this.consumingAdapter = consumingAdapter;
  ...
  async _startConsumerBridge (transportType) {
    ...
    Logger.info(LOG_PREFIX, 'Starting FS consumer bridge', this._getFullLogMetadata());
    ...
    try {
      ...
      if (!this._negotiated && this.bridgeMediaStatus === C.MEDIA_STOPPED) {
        ...
        // subscribers
        const proxyOptions = {
          adapter: this.consumingAdapter,
          name: `PROXY_${this.bridgeMediaName}|subscribe`,
        ...
        Logger.info(LOG_PREFIX, 'FS consumer bridge relay created',
          { ...this._getFullLogMetadata(), mediaId: this.bridgeMediaId, userId: this.userId });
      }
    } catch (error) {
      Logger.error(LOG_PREFIX, 'Error starting FS consumer bridge relay',
