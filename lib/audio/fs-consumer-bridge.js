  ...
  async _startConsumerBridge (transportType) {
    ...
    try {
      ...
      if (!this._negotiated && this.bridgeMediaStatus === C.MEDIA_STOPPED) {
        ...
        this.mcs.onEvent(C.MEDIA_STATE, gaMediaId, (event) => {
          this._onBridgeMediaStateChange(event, gaMediaId);
        });
        ...
        Logger.info('FS consumer bridge started', this._getFullLogMetadata());
      }
    } catch (error) {
