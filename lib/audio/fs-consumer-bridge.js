...
const AUTO_RESTART_GLOBAL_AUDIO = config.has('autoRestartGlobalAudio')
  ? config.get('autoRestartGlobalAudio')
  : false;
...
module.exports = class FSConsumerBridge extends EventEmitter {
  ...
  async _onGADisconnection (event) {
    const { mediaId } = event;
    const ignoreEvent = this.ignoreMCSMediaEvents || mediaId !== this.fsMediaId;

    Logger.warn('FS consumer bridge: global audio disconnected',
      { ...this.getFullLogMetadata(), event, ignoreEvent });
    ...
    if (AUTO_RESTART_GLOBAL_AUDIO) this._restartRoutine = this._setRestartRoutine();
  }
