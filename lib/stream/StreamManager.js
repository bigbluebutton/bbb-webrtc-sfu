...
module.exports = class StreamManager extends BaseManager {
  ...
  _onStart(meetingId, streamUrl, streamType, streamId) {
    return () => {
      Logger.info(this._logPrefix, 'Stream is starting for', meetingId, streamUrl, streamType, streamId);
      this._bbbGW.publish(
      Messaging.generateStreamEventMessage(meetingId, C.STREAM_STARTED, streamUrl, streamType, streamId)
    , C.TO_HTML5);
    };
  }
  ...
  _onMessage(message) {
    let session, meetingId, userId, name, streamUrl, streamType, streamId, confname;
    ...
    switch (name) {
      case 'StartStream':
        ...
        if (!session) {
          session = new Stream(this._bbbGW, meetingId, confname, streamUrl, streamType);
        } else {
        ...
        session.start((error) => {
          ...
          session.onStart(this._onStart(meetingId, streamUrl, streamType, streamId));
