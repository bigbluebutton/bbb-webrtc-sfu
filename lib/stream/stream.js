...
const getContainerSpawner = (imageName, streamType, meetingId, process) => {
  const containerType = config.get('bbb-stream.container_type');

  if (!containerType || containerType === 'docker') {
      return new DockerSpawner(imageName, streamType, meetingId, process);
  }

  if (containerType === 'kubernetes') {
    return new KubernetesSpawner(imageName, streamType, meetingId, process);
  }
...
module.exports = class Stream extends BaseProvider {
  constructor(_bbbGW, _id, _meetingId, _streamUrl, _streamType) {
    ...
    this.containerSpawner = getContainerSpawner(this.imageName, this.streamType, this.meetingId, this);
  }
