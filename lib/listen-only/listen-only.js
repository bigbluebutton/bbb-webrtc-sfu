...
module.exports = class ListenOnly extends BaseProvider {
  ...
  _getFullLogMetadata (connectionId) {
    const endpoint  = this.clientEndpoints[connectionId] || {};
    return {
      roomId: this.voiceBridge,
      internalMeetingId: this.meetingId,
  ...
  getConnectionIdsFromUser(userId) {
    // FIXME inefficient
    return Object.keys(this.clientEndpoints).filter(connectionId => {
      const endpoint = this.clientEndpoints[connectionId];
  ...
  async start (sessionId, connectionId, sdpOffer, userId) {
    ...
    this.clientEndpoints[connectionId] = clientConsumer;
  ...
  async stopListener(connectionId) {
    const listener = this.clientEndpoints[connectionId];
    ...
    if (listener) {
      ...
      if (this.clientEndpoints && Object.keys(this.clientEndpoints).length === 1) {
        this._stopSourceAudio();
      }

      delete this.clientEndpoints[connectionId];
    }
  }
  ...
  async stop () {
    ...
    try {
      const nofConnectedUsers = Object.keys(this.clientEndpoints).length;
      if (nofConnectedUsers <= 0) {
        return this._stopSourceAudio();
      }

      // TODO refactor
      for (var connectionId in this.clientEndpoints) {
        try {
