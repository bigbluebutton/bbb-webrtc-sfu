const ScreenshareRTMPBroadcastStartedEventMessage2x =
  require('./screenshare/ScreenshareRTMPBroadcastStartedEventMessage2x.js');
const ScreenshareRTMPBroadcastStoppedEventMessage2x =
  require('./screenshare/ScreenshareRTMPBroadcastStoppedEventMessage2x.js');
const UserCamBroadcastStoppedEventMessage2x =
  require('./video/UserCamBroadcastStoppedEventMessage2x.js');
const WebRTCShareEvent = require('./recording/WebRTCShareEvent.js');
const AudioTrackPublishedEvent = require('./recording/AudioTrackPublishedEvent.js');
const AudioTrackUnpublishedEvent = require('./recording/AudioTrackUnpublishedEvent.js');
const RecordingStatusRequestMessage2x =
  require('./recording/RecordingStatusRequestMessage2x.js');
const UserConnectedToGlobalAudio2x =
  require('./audio/UserConnectedToGlobalAudio2x.js');
const UserDisconnectedFromGlobalAudio2x =
  require('./audio/UserDisconnectedFromGlobalAudio2x.js');
const GetGlobalAudioPermissionReqMsg =
  require('./audio/GetGlobalAudioPermissionReqMsg.js');
const GetScreenBroadcastPermissionReqMsg =
  require('./screenshare/GetScreenBroadcastPermissionReqMsg.js');
const GetScreenSubscribePermissionReqMsg=
  require('./screenshare/GetScreenSubscribePermissionReqMsg.js');
const GetCamBroadcastPermissionReqMsg =
  require('./video/GetCamBroadcastPermissionReqMsg.js');
const GetCamSubscribePermissionReqMsg =
  require('./video/GetCamSubscribePermissionReqMsg.js');
const CamStreamSubscribedInSfuEvtMsg =
  require('./video/CamStreamSubscribedInSfuEvtMsg.js');
const CamStreamUnsubscribedInSfuEvtMsg =
  require('./video/CamStreamUnsubscribedInSfuEvtMsg.js');
const CamBroadcastStoppedInSfuEvtMsg =
  require('./video/CamBroadcastStoppedInSfuEvtMsg.js');
const GetMicrophonePermissionReqMsg =
  require('./audio/GetMicrophonePermission.js');
const ListenOnlyModeToggledEvtMsg = require('./audio/ListenOnlyModeToggledEvtMsg.js');
const GenerateLiveKitTokenRespMsg = require('./livekit/GenerateLiveKitTokenRespMsg.js');
const UpdateLiveKitParticipantPermissionsRespMsg =
  require('./livekit/UpdateLiveKitParticipantPermissionsRespMsg.js');
const EjectUserFromVoiceConfRespMsg = require('./livekit/EjectUserFromVoiceConfRespMsg.js');
const MuteUserInVoiceConfSysMsg = require('./audio/MuteUserInVoiceConfSysMsg.js');
const UserJoinedVoiceConfEvtMsg = require('./audio/UserJoinedVoiceConfEvtMsg.js');
const UserLeftVoiceConfEvtMsg = require('./audio/UserLeftVoiceConfEvtMsg.js');
const UserMutedInVoiceConfEvtMsg = require('./audio/UserMutedInVoiceConfEvtMsg.js');
const UserBroadcastCamStartMsg = require('./video/UserBroadcastCamStartMsg.js');
const UserBroadcastCamStopMsg = require('./video/UserBroadcastCamStopMsg.js');
const LiveKitParticipantLeftEvtMsg = require('./livekit/LiveKitParticipantLeftEvtMsg.js');
const GenerateBreakoutRoomLiveKitTokenRespMsg = require('./livekit/GenerateBreakoutRoomLiveKitTokenRespMsg.js');
const SetUserTalkingReqMsg = require('./audio/SetUserTalkingReqMsg.js');

module.exports = {
  generateScreenshareRTMPBroadcastStartedEvent2x: (
    conferenceName, screenshareConf, streamUrl, vw, vh, timestamp, options = {},
  ) => {
    return (new ScreenshareRTMPBroadcastStartedEventMessage2x(
      conferenceName, screenshareConf, streamUrl, vw, vh, timestamp, options,
    )).toJson();
  },

  generateScreenshareRTMPBroadcastStoppedEvent2x: (
    conferenceName, screenshareConf, streamUrl, vw, vh, timestamp, options = {},
  ) => {
    return (new ScreenshareRTMPBroadcastStoppedEventMessage2x(
      conferenceName, screenshareConf, streamUrl, vw, vh, timestamp, options,
    )).toJson();
  },

  generateUserCamBroadcastStoppedEventMessage2x: (meetingId, userId, streamUrl) => {
    return (new UserCamBroadcastStoppedEventMessage2x(meetingId, userId, streamUrl))
      .toJson();
  },

  generateWebRTCShareEvent: (
    name, meetingId, streamUrl, timestampHR, timestampUTC, userId,
  ) => {
    return (new WebRTCShareEvent(
      name,
      meetingId,
      streamUrl,
      timestampHR,
      timestampUTC,
      userId,
    )).payload;
  },

  generateAudioTrackPublishedEvent: (
    meetingId, userId, filename, source, timestampHR, timestampUTC,
  ) => {
    return (new AudioTrackPublishedEvent(
      meetingId, userId, filename, source, timestampHR, timestampUTC,
    )).payload;
  },

  generateAudioTrackUnpublishedEvent: (
    meetingId, userId, filename, source, timestampHR, timestampUTC,
  ) => {
    return (new AudioTrackUnpublishedEvent(
      meetingId, userId, filename, source, timestampHR, timestampUTC,
    )).payload;
  },

  // FIXME what the asjdkasjdl is that default userId parameter
  generateRecordingStatusRequestMessage: (meetingId, userId = '') => {
    return (new RecordingStatusRequestMessage2x(meetingId, userId)).toJson();
  },

  generateUserConnectedToGlobalAudioMessage: (voiceConf, userId, name) => {
    return (new UserConnectedToGlobalAudio2x(voiceConf, userId, name)).toJson();
  },

  generateUserDisconnectedFromGlobalAudioMessage: (voiceConf, userId, name) => {
    return (new UserDisconnectedFromGlobalAudio2x(voiceConf, userId, name)).toJson();
  },

  generateGetGlobalAudioPermissionReqMsg: (
    meetingId, voiceConf, userId, sfuSessionId
  ) => {
    return (new GetGlobalAudioPermissionReqMsg(meetingId, voiceConf, userId, sfuSessionId)).toJson();
  },

  generateGetScreenBroadcastPermissionReqMsg: (
    meetingId, voiceConf, userId, sfuSessionId
  ) => {
    return (new GetScreenBroadcastPermissionReqMsg(meetingId, voiceConf, userId, sfuSessionId)).toJson();
  },

  generateGetScreenSubscribePermissionReqMsg: (
    meetingId, voiceConf, userId, streamId, sfuSessionId
  ) => {
    return (new GetScreenSubscribePermissionReqMsg(
      meetingId, voiceConf, userId, streamId, sfuSessionId
    )).toJson();
  },

  generateGetCamBroadcastPermissionReqMsg: (
    meetingId, userId, streamId, sfuSessionId
  ) => {
    return (new GetCamBroadcastPermissionReqMsg(meetingId, userId, streamId, sfuSessionId)).toJson();
  },

  generateGetCamSubscribePermissionReqMsg: (
    meetingId, userId, streamId, sfuSessionId
  ) => {
    return (new GetCamSubscribePermissionReqMsg(
      meetingId, userId, streamId, sfuSessionId
    )).toJson();
  },

  generateCamStreamSubscribedInSfuEvtMsg: (
    meetingId, userId, streamId, subscriberStreamId, sfuSessionId
  ) => {
    return (new CamStreamSubscribedInSfuEvtMsg(
      meetingId, userId, streamId, subscriberStreamId, sfuSessionId
    )).toJson();
  },

  generateCamStreamUnsubscribedInSfuEvtMsg: (
    meetingId, userId, streamId, subscriberStreamId, sfuSessionId
  ) => {
    return (new CamStreamUnsubscribedInSfuEvtMsg(
      meetingId, userId, streamId, subscriberStreamId, sfuSessionId
    )).toJson();
  },

  generateCamBroadcastStoppedInSfuEvtMsg: (
    meetingId, userId, streamId
  ) => {
    return (new CamBroadcastStoppedInSfuEvtMsg(
      meetingId, userId, streamId
    )).toJson();
  },

  generateGetMicrophonePermissionReqMsg: (
    meetingId, voiceConf, userId, callerIdNum, sfuSessionId
  ) => {
    return (new GetMicrophonePermissionReqMsg(
      meetingId, voiceConf, userId, callerIdNum, sfuSessionId,
    )).toJson();
  },

  generateListenOnlyModeToggledEvtMsg: (
    meetingId, voiceConf, userId, callerNum, enabled,
  ) => {
    return (new ListenOnlyModeToggledEvtMsg(
      meetingId, voiceConf, userId, callerNum, enabled,
    )).toJson();
  },

  generateGenerateLiveKitTokenRespMsg: (
    meetingId, userId, token, grant,
  ) => {
    return (new GenerateLiveKitTokenRespMsg(
      meetingId, userId, token, grant,
    )).toJson();
  },

  generateUpdateLiveKitParticipantPermissionsRespMsg: (
    meetingId, userId, grant, outcome,
  ) => {
    return (new UpdateLiveKitParticipantPermissionsRespMsg(
      meetingId, userId, grant, outcome,
    )).toJson();
  },

  generateEjectUserFromVoiceConfRespMsg: (
    meetingId, voiceConf, voiceUserId, outcome,
  ) => {
    return (new EjectUserFromVoiceConfRespMsg(
      meetingId, voiceConf, voiceUserId, outcome,
    )).toJson();
  },

  generateGenerateBreakoutRoomLiveKitTokenRespMsg: (
    parentMeetingId, userId, breakoutRoomId, token, grant,
  ) => {
    return (new GenerateBreakoutRoomLiveKitTokenRespMsg(
      parentMeetingId, userId, breakoutRoomId, token, grant,
    )).toJson();
  },

  generateMuteUserInVoiceConfSysMsg: (
    meetingId, voiceConf, voiceUserId, mute, intId,
  ) => {
    return (new MuteUserInVoiceConfSysMsg(
      meetingId, voiceConf, voiceUserId, mute, intId,
    )).toJson();
  },

  generateUserJoinedVoiceConfEvtMsg: (
    voiceConf,
    voiceUserId,
    intId,
    callerIdName,
    callerIdNum,
    muted,
    talking,
    callingWith,
    hold,
    uuid,
  ) => {
    return (new UserJoinedVoiceConfEvtMsg(
      voiceConf,
      voiceUserId,
      intId,
      callerIdName,
      callerIdNum,
      muted,
      talking,
      callingWith,
      hold,
      uuid,
    )).toJson();
  },

  generateUserLeftVoiceConfEvtMsg: (
    voiceConf,
    voiceUserId,
  ) => {
    return (new UserLeftVoiceConfEvtMsg(
      voiceConf,
      voiceUserId,
    )).toJson();
  },

  generateUserMutedInVoiceConfEvtMsg: (
    voiceConf,
    voiceUserId,
    muted,
  ) => {
    return (new UserMutedInVoiceConfEvtMsg(
      voiceConf,
      voiceUserId,
      muted,
    )).toJson();
  },

  generateUserBroadcastCamStartMsg: (
    meetingId,
    userId,
    streamId,
    contentType,
    hasAudio,
  ) => {
    return (new UserBroadcastCamStartMsg(
      meetingId,
      userId,
      streamId,
      contentType,
      hasAudio,
    )).toJson();
  },

  generateUserBroadcastCamStopMsg: (
    meetingId,
    userId,
    streamId,
  ) => {
    return (new UserBroadcastCamStopMsg(
      meetingId,
      userId,
      streamId,
    )).toJson();
  },

  generateLiveKitParticipantLeftEvtMsg: (
    meetingId,
    userId,
  ) => {
    return (new LiveKitParticipantLeftEvtMsg(
      meetingId,
      userId,
    )).toJson();
  },

  generateSetUserTalkingReqMsg: (meetingId, userId, talking) => {
    return (new SetUserTalkingReqMsg(meetingId, userId, talking)).toJson();
  }
}
