...
module.exports = class BigBlueButtonGW extends EventEmitter {
  ...
  addSubscribeChannel (channel) {
    ...
    subscribers[channel] = {};
    subscribers[channel] = wrobj;
    try {
      wrobj.startSubscriber();
      wrobj.on(C.REDIS_MESSAGE, this.incomingMessage.bind(this));
      Logger.info("[BigBlueButtonGW] Added redis client to subscribers[" + channel + "]");
    ...
    if (header){
      switch (header.name) {
        ...
        case C.USER_CAM_BROADCAST_STARTED_2x:
          this.emit(C.USER_CAM_BROADCAST_STARTED_2x, payload[C.STREAM_URL_2x]);
          break;
        // SCREENSHARE
        default:
  ...
  publish (message, channel) {
    if (!publisher) {
      publisher = new RedisWrapper();
      publisher.startPublisher();
    }

    if (typeof publisher.publishToChannel === 'function') {
      publisher.publishToChannel(message, channel);
    }
  }
  ...
  writeMeetingKey(meetingId, message, callback) {
    ...
    publisher.setKeyWithIncrement(recKey, message, (err, msgId) => {

      publisher.pushToList('meeting:' + meetingId + ':recordings', msgId);

      // Not implemented yet
      publisher.expireKey(recKey + ':' + msgId, EXPIRE_TIME, (err) => {
