...
module.exports = class BigBlueButtonGW extends EventEmitter {
  ...
  incomingMessage (message) {
    ...
    if (msg?.envelope?.timestamp) header.timestamp = msg.envelope.timestamp;
    ...
    if (header) {
      switch (header.name) {
        case C.TOGGLE_LISTEN_ONLY_MODE_SYS_MSG:
          ...
          break;
        case C.VOICE_CONF_CALLSTATE_EVT_MSG:
          this.emit(C.VOICE_CONF_CALLSTATE_EVT_MSG, payload);
          this.emit(C.VOICE_CONF_CALLSTATE_EVT_MSG + payload.userId, payload);
          break;
        ...
        case C.GET_GLOBAL_AUDIO_PERM_RESP_MSG:
          ...
          break;
        case C.GET_MIC_PERM_RESP_MSG:
          this.emit(C.GET_MIC_PERM_RESP_MSG+payload.sfuSessionId, payload);
          break;
        ...
        case C.CAM_BROADCAST_STOP_SYS_MSG: {
          ...
          this.emit(eventName, payload);
          this.emit(C.CAM_BROADCAST_STOP_SYS_MSG, payload);
          break;
        }
        ...
        case C.RECORDING_STATUS_CHANGED_EVT_MSG: {
          meetingId = header[C.MEETING_ID_2x];
          payload.meetingId = meetingId;
          this.emit(C.RECORDING_STATUS_CHANGED_EVT_MSG+meetingId, payload);
          this.emit(C.RECORDING_STATUS_CHANGED_EVT_MSG, payload);
          break;
        }
        case C.PLUGIN_PERSIST_EVENT_EVT_MSG: {
          meetingId = header[C.MEETING_ID_2x];
          payload.meetingId = meetingId;
          payload.userId = header[C.USER_ID_2x];
          this.emit(C.PLUGIN_PERSIST_EVENT_EVT_MSG+meetingId, payload);
          this.emit(C.PLUGIN_PERSIST_EVENT_EVT_MSG, payload);
          break;
        }
        case C.USER_LEFT_VOICE_CONF_TO_CLIENT_EVT_MSG: {
          payload.meetingId = header.meetingId;
          payload.userId = header.userId;
          this.emit(C.USER_LEFT_VOICE_CONF_TO_CLIENT_EVT_MSG+meetingId, payload);
          this.emit(C.USER_LEFT_VOICE_CONF_TO_CLIENT_EVT_MSG, payload);
          break;
        }
        case C.GENERATE_LIVEKIT_TOKEN_REQ_MSG: {
          const { meetingId } = header;
          payload.meetingId = meetingId;
          const suffix = `${meetingId}/${payload.userId}`;
          const enrichedEventId = `${C.GENERATE_LIVEKIT_TOKEN_REQ_MSG}/${suffix}`;
          this.emit(enrichedEventId, payload);
          this.emit(C.GENERATE_LIVEKIT_TOKEN_REQ_MSG, payload);
          break;
        }

        case C.GENERATE_BREAKOUT_ROOM_LIVEKIT_TOKEN_REQ_MSG: {
          const { meetingId } = header;
          const { breakoutRoomId } = payload;
          payload.meetingId = meetingId;
          payload.breakoutRoomId = breakoutRoomId;
          this.emit(C.GENERATE_BREAKOUT_ROOM_LIVEKIT_TOKEN_REQ_MSG, payload);
          break;
        }

        case C.MEETING_CREATED_EVT_MSG:
        case C.MEETING_ENDED_EVT_MSG: {
          const meetingId = payload?.props?.meetingProp?.intId || payload?.meetingId;
          payload.meetingId = meetingId;
          this.emit(header.name + meetingId, payload);
          this.emit(header.name, payload);
          break;
        }

        case C.EJECT_USER_FROM_VOICE_CONF_SYS_MSG:
        case C.UPDATE_LIVEKIT_PARTICIPANT_PERMISSIONS_SYS_MSG:
        case C.MUTE_USER_IN_VOICE_CONF_SYS_MSG: {
