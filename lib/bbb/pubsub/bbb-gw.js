  ...
  incomingMessage (message) {
    ...
    if (header) {
      switch (header.name) {
        ...
        case C.GENERATE_WEBRTC_TOKEN_REQ_MESSAGE: {
          const { meetingId } = header;
          payload.meetingId = meetingId;
          const suffix = `${meetingId}/${payload.userId}`;
          const enrichedEventId = `${C.GENERATE_WEBRTC_TOKEN_REQ_MESSAGE}/${suffix}`;
          this.emit(enrichedEventId, payload);
          this.emit(C.GENERATE_WEBRTC_TOKEN_REQ_MESSAGE, payload);
          break;
        }
