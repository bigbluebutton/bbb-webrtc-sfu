...
module.exports = class RedisWrapper extends EventEmitter {
  ...
  startSubscriber () {
    ...
    this.redisCli.on("error", (error) => {
      Logger.error("Redis client error event", {
        errorMessage: error.message,
      });
    });

    this.redisCli.on("reconnecting", (event) => {
      Logger.warn("Redis client reconnecting", { event });
      //TODO
    });

    this.redisCli.on("psubscribe", (channel) => {
      Logger.debug(`Redis client subscribed to ${channel}`);
    });
    ...
    Logger.debug(`Redis client started: ${options.host}:${options.port}`);
  }
