...
module.exports = class BaseManager {
    ...
    start() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!this.mcsStarted) {
                    yield this.mcs.start(MCS_ADDRESS, MCS_PORT);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (process.env.SFU_IPC_MODE) {
                case 'native':
                    ...
                    break;
                case 'redis':
                    this._mainInboundGateway = yield this._bbbGW.addSubscribeChannel(this._connectionChannel);
        ...
        switch (process.env.SFU_IPC_MODE) {
            ...
            case 'none':
                if (global.CM_ROUTER && typeof global.CM_ROUTER.emit === 'function') {
                    ...
                    Logger.error(this._logPrefix, "Can't send outbound request, router not found", { request: message, ipc: process.env.SFU_IPC_MODE, target });
                }
                break;
            default:
                Logger.error(this._logPrefix, "Can't send outbound request, invalid IPC mode", { request: message, ipc: process.env.SFU_IPC_MODE, target });
                return;
        }
    }
    ...
    _handleError(logPrefix, connectionId, streamId, role, error) {
        ...
        if (stack && !error.stackWasLogged) {
            Logger.error(logPrefix, `Stack trace for error ${error.code} | ${error.message} ->`, { errorStack: error.stack.toString() });
    ...
    _validateErrorMessage(error) {
        const { connectionId = null, type = null, id = null, role = null, streamId = null, code = null, reason = null, } = error;
