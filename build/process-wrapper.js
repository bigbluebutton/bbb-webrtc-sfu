...
module.exports = class ProcessWrapper {
    ...
    _onMessage(message) {
        Logger.info(LOG_PREFIX, "Received message from forked process", { pid: this.process.pid, message });
    }
    onError(error) {
        Logger.error(LOG_PREFIX, "Forked process error event", { pid: this.process.pid, errorMessage: error.message, errorName: error.name, errorCode: error.code });
    }
    start() {
        ...
        const childEnv = Object.assign(Object.assign({}, process.env), { SFU_MODULE_NAME: this.name, SFU_IPC_MODE: this.ipcMode, SFU_IPC_OPTS: this.options, SFU_MODULE_PATH: this.path });
