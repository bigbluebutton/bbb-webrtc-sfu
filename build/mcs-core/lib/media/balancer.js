...
class Balancer extends EventEmitter {
    ...
    upstartHosts() {
        return __awaiter(this, void 0, void 0, function* () {
            const processHosts = () => __awaiter(this, void 0, void 0, function* () {
                const tryToConnect = (host) => __awaiter(this, void 0, void 0, function* () {
                    const { url, ip, mediaType, retries, ipClassMappings = { public: host.ip }, options = KMS_DEFAULT_OPTIONS, } = host;
                    if (retries < NOF_STARTUP_CONNECTION_RETRIES) {
                        if (!this._hostStarted(url, ip)) {
                            try {
                                const newHost = yield Balancer.connectToHost(url, ip, options, ipClassMappings, mediaType);
                                this._monitorConnectionState(newHost);
                                this.addHost(newHost);
                            }
                            catch (e) {
                                host.retries++;
                                Logger.error(LOG_PREFIX, `Failed to connect to media server`, { url, ip, mediaType, retries });
                        ...
                        Logger.error(LOG_PREFIX, `Maximum number of retries expired for media server`, { url, ip, mediaType, retries });
                    }
                });
                const tentativeHosts = KMS_ARRAY.map(th => { return Object.assign(Object.assign({}, th), { retries: 0 }); });
    ...
    getHost(mediaType = C.MEDIA_PROFILE.ALL) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    addHost(host) {
        if (host) {
            ...
            Logger.info(LOG_PREFIX, `New media server host added`, { hostId: id, url: host.url, ip: host.ip, mediaType: host.mediaType });
            return;
        }
    ...
    decrementHostStreams(hostId, mediaType) {
        ...
        if (host) {
            ...
            Logger.info(LOG_PREFIX, `Media server ${mediaType} streams decremented`, { hostId: host.id, url: host.url, mediaType, [mediaType]: host.medias[mediaType] });
        }
    }
    incrementHostStreams(hostId, mediaType) {
        ...
        if (host) {
            host.medias[mediaType]++;
            Logger.info(LOG_PREFIX, `Media server ${mediaType} streams incremented`, { hostId: host.id, url: host.url, mediaType, [mediaType]: host.medias[mediaType] });
        }
    }
    ...
    _roundRobinHost() {
        let host = this.hosts.find(host => (host.medias[C.MEDIA_PROFILE.MAIN] < VIDEO_TRANSPOSING_CEILING) &&
    ...
    _monitorConnectionState(host) {
        ...
        try {
            ...
            Logger.error(LOG_PREFIX, `Error when trying to monitor media server`, { hostId: id, url, ip, mediaType: host.mediaType });
        }
    }
    _onDisconnection(host) {
        try {
            const { client, id } = host;
            Logger.error(LOG_PREFIX, 'Media server disconnected', { hostId: id, url: host.url, mediaType: host.mediaType });
            this.removeHost(id);
            ...
            Logger.error(LOG_PREFIX, `Error trying to handle media server disconnection`, { hostId: host.id, url: host.url, mediaType: host.mediaType, error });
        }
    }
    _onReconnection(sameSession, host) {
        if (!sameSession) {
            Logger.warn(LOG_PREFIX, `Media server reconnected, not same session`, { hostId: host.id, url: host.url, mediaType: host.mediaType });
            this._onDisconnection(host);
        }
    }
    _reconnectToServer(host) {
        const { client, id, url, options } = host;
        Logger.info(LOG_PREFIX, `Reconnecting to media server`, { hostId: id, url, mediaType: host.mediaType });
        if (this._reconnectionRoutine[id] == null) {
            this._reconnectionRoutine[id] = setInterval(() => __awaiter(this, void 0, void 0, function* () {
                try {
                    const connect = new Promise((resolve, reject) => {
                        mediaServerClient(url, options, (error, client) => {
                            if (error) {
                    ...
                    Promise.race([connect, failOver]).then(h => {
                        ...
                        this.addHost(h);
                        Logger.warn(LOG_PREFIX, `Reconnection to media server succeeded`, { hostId: id, url, mediaType: host.mediaType });
                    }).catch(error => {
                        Logger.error(LOG_PREFIX, `Failed to reconnect to media server`, { hostId: id, url, mediaType: host.mediaType, error });
                    });
                }
                catch (error) {
                    Logger.error(LOG_PREFIX, `Failed to reconnect to media server`, { hostId: id, url, mediaType: host.mediaType, error });
