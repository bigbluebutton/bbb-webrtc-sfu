...
module.exports = class MediaController {
    ...
    static isValidMediaType(type) {
        return Object.keys(C.MEDIA_TYPE).some(validTypeKey => C.MEDIA_TYPE[validTypeKey] === type);
    }
    _handleNewVideoFloor(event) {
        ...
        if (!mediaId) {
            try {
                ...
                Logger.error(LOG_PREFIX, `Error releasing conference floor at ${roomId}`, { errorMessage: error.message, errorCode: error.code });
            }
        }
        else {
            try {
                ...
                Logger.error(LOG_PREFIX, `Error setting conference floor at ${roomId}`, { errorMessage: error.message, errorCode: error.code });
            }
        }
    }
    start() {
    ...
    _ejectUser(userInfo) {
        ...
        try {
            ...
            Logger.error(LOG_PREFIX, `Auto eject for user ${userId} at ${roomId} failed due to ${error.message}`, { roomId, userId, externalUserId, error });
        }
    }
    ...
    _isAboveThreshold() {
        if (GLOBAL_MEDIA_THRESHOLD > 0 && this.medias.size >= GLOBAL_MEDIA_THRESHOLD) {
            Logger.error(LOG_PREFIX, `Server has exceeded the media threshold`, { threshold: GLOBAL_MEDIA_THRESHOLD, current: this.medias.size });
            return true;
        }
        return false;
    }
    ...
    publishAndSubscribe(roomId, userId, sourceId, type, params = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!params.ignoreThresholds && this.isAboveMediaThresholds(room, user)) {
                throw (this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_SERVER_NO_RESOURCES), { details: `Threshold exceeded. Threshold: ${GLOBAL_MEDIA_THRESHOLD}` })));
            }
            try {
                ({ session, answer } = yield user.publish(params.descriptor, type, params));
            }
            catch (error) {
            ...
            if (source) {
                try {
                    yield user.connect(source.id, session.id);
                }
                catch (error) {
    ...
    subscribe(userId, sourceId, type, params = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ({ session, answer } = yield user.subscribe(params.descriptor, type, source, params));
            }
            catch (error) {
                throw (this._handleError(error));
            }
    ...
    unpublish(userId, mediaId) {
        ...
        try {
            ...
            Logger.warn(LOG_PREFIX, `Unpublish from user ${userId} for media ${mediaId} failed due to ${error.message}`, { userId, mediaId, error });
            throw (this._handleError(error));
        }
        try {
            this.removeMediaSession(mediaId);
            ...
            Logger.warn(LOG_PREFIX, `Unpublish from user ${userId} for media ${mediaId} failed due to ${error.message}`, { roomId: room.id, userId, mediaId, error });
            throw (this._handleError(error));
        }
    }
    unsubscribe(userId, mediaId) {
        ...
        try {
            ...
            Logger.warn(LOG_PREFIX, `Unsubscribe from user ${userId} for media ${mediaId} failed due to ${error.message}`, { userId, mediaId, error });
            throw (this._handleError(error));
        }
        try {
            this.removeMediaSession(mediaId);
            ...
            Logger.warn(LOG_PREFIX, `Unsubscribe from user ${userId} for media ${mediaId} failed due to ${error.message}`, { roomId: room.id, userId, mediaId, error });
            throw (this._handleError(error));
        }
    }
    startRecording(userId, sourceId, recordingPath, params = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                Logger.warn(LOG_PREFIX, `startRecording from user ${userId} of media ${sourceId} failed due to ${error.message}`, { userId, mediaId: sourceId, error });
                throw (this._handleError(error));
            }
            if (!params.ignoreThresholds && this.isAboveMediaThresholds(room, user)) {
                throw (this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_SERVER_NO_RESOURCES), { details: `Threshold exceeded. Threshold: ${GLOBAL_MEDIA_THRESHOLD}` })));
            ...
            try {
                ({ recordingSession, answer } = yield user.startRecording(recordingPath, C.MEDIA_TYPE.RECORDING, sourceSession, params));
            }
            catch (error) {
                Logger.warn(LOG_PREFIX, `startRecording from user ${userId} of media ${sourceId} failed due to ${error.message}`, { userId, mediaId: sourceId, error });
                throw (this._handleError(error));
    ...
    stopRecording(userId, recId) {
        ...
        try {
            ...
            Logger.warn(LOG_PREFIX, `stopRecording for user ${userId} of recording ${recId} failed due to ${error.message}`, { userId, mediaId: recId, error });
            throw (this._handleError(error));
        }
        try {
            ...
            Logger.error(LOG_PREFIX, `stopRecording from user ${userId} of recording ${recId} failed due to ${error.message}`, { roomId: room.id, userId, mediaId: recId, error });
            throw (this._handleError(error));
        }
    }
    connect(sourceId, sinkId, type = 'ALL') {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield sourceSession.connect(sinkSession, type);
                return resolve();
            }
            catch (err) {
    ...
    disconnect(sourceId, sinkId, type = 'ALL') {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                const sourceSession = this.getMediaSession(sourceId);
                const sinkSession = this.getMediaSession(sinkId);
                yield sourceSession.disconnect(sinkSession, type);
                return resolve();
            }
            catch (err) {
                return reject(this._handleError(err));
    ...
    addIceCandidate(mediaId, candidate) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield session.addIceCandidate(candidate);
                return resolve();
            }
            catch (err) {
                return reject(this._handleError(err));
    ...
    removeRoom(roomId) {
        ...
        try {
            ...
            Logger.error(LOG_PREFIX, "CRITICAL: Room deletion failed", { roomId, errorMessage: error.message, errorCode: error.code });
            // Try to roll back, but this is a critical
            if (!removed)
    ...
    createUser(room, type, params) {
        ...
        if (externalUserId) {
            try {
                ...
                if (!ALLOW_DUPLICATE_EXT_USER_ID) {
                    throw this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_INVALID_OPERATION), { details: `externalUserId specified on join and duplicate externalUserId is not allowed` }));
                }
                return user;
            }
            catch (e) {
        ...
        if (user.externalUserId !== user.id)
            this.users.set(user.externalUserId, user);
        room.addUser(user);
    ...
    getUsers(roomId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `getUsers failed for room ${roomId} due to ${error.message}`, { roomId, error });
            throw (this._handleError(error));
        }
    }
    getUserMedias(userId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `getUserMedias failed for user ${userId} due to ${error.message}`, { userId, error });
            throw (this._handleError(error));
        }
    }
    getRoomMedias(roomId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `getRoomMedias failed for room ${roomId} due to ${error.message}`, { roomID, error });
            throw (this._handleError(error));
        }
    }
    ...
    getMediaSession(mediaId) {
        ...
        if (media == null) {
            throw this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_NOT_FOUND), { details: `mediaId: ${mediaId}` }));
        }
        return media;
    }
    ...
    setContentFloor(roomId, mediaId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `setContentFloor for room ${roomId} as media ${mediaId} failed due to ${error.message}`, { roomId, mediaId, error });
            throw (this._handleError(error));
        }
    }
    setConferenceFloor(roomId, mediaId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `setConferenceFloor for room ${roomId} as media ${mediaId} failed due to ${error.message}`, { roomId, mediaId, error });
            throw (this._handleError(error));
        }
    }
    releaseContentFloor(roomId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `releaseContentFloor for room ${roomId} failed due to ${error.message}`, { roomId, error });
            throw (this._handleError(error));
        }
    }
    releaseConferenceFloor(roomId, preserve = true) {
        try {
            ...
            Logger.error(LOG_PREFIX, `releaseConferenceFloor for room ${roomId} failed due to ${error.message}`, { roomId, error });
            throw (this._handleError(error));
        }
    }
    getContentFloor(roomId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `getContentFloor for room ${roomId} failed due to ${error.message}`, { roomId, error });
            throw (this._handleError(error));
        }
    }
    getConferenceFloor(roomId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `getConferenceFloor for room ${roomId} failed due to ${error.message}`, { roomId, error });
            throw (this._handleError(error));
        }
    }
    setVolume(mediaId, volume) {
        try {
            ...
            Logger.error(LOG_PREFIX, `setVolume for media$ ${mediaId} failed due to ${error.message}`, { mediaId, volume, error });
            throw (this._handleError(error));
        }
    }
    mute(mediaId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `mute for media$ ${mediaId} failed due to ${error.message}`, { mediaId, error });
            throw (this._handleError(error));
        }
    }
    unmute(mediaId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `unmute for media$ ${mediaId} failed due to ${error.message}`, { mediaId, error });
            throw (this._handleError(error));
        }
    }
    /*
    ...
    setStrategy(identifier, strategy, params = {}) {
        throw this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_INVALID_OPERATION), { details: 'setStrategy is not implemented' }));
    }
    /*
     * Given a generic identifier that can represent a room, user, media session
    ...
    getStrategy(identifier) {
        throw this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_INVALID_OPERATION), { details: 'getStrategy is not implemented' }));
    }
    /*
     * Given a generic identifier that can represent a room, user, media session
    ...
    _getMediasByMemberIdentifier(memberType, identifier) {
        switch (memberType) {
            ...
            default:
                throw (this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_INVALID_TYPE), { details: `getMedias memberType is invalid: ${memberType}` })));
        }
    }
    // FIXME
    ...
    _handleExternalAudioMediaConnected(event) {
        try {
            ...
            const room = this.createRoom(roomId);
            const user = this.createUser(room, C.USERS.SFU, { externalUserId: userId, name: userName, autoLeave: true });
