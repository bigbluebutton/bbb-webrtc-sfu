...
const { METRICS_PREFIX, METRIC_NAMES, buildMetrics } = require('./core-metrics.js');
const { enabled: METRICS_ENABLED, host: METRICS_HOST, port: METRICS_PORT, path: METRICS_PATH, collectDefaultMetrics: COLLECT_DEFAULT_METRICS, } = config.has('prometheus')
...
const registerMediaUnitTypeMetrics = (operation, media) => {
    if (media.mediaTypes.video) {
        ...
        MCSPrometheusAgent[operation](METRIC_NAMES.MEDIA_UNITS, { media_type: C.MEDIA_PROFILE.MAIN, unit_type: media.type, direction });
    }
    if (media.mediaTypes.audio) {
        const direction = media.mediaTypes.audio === true ? 'sendrecv' : media.mediaTypes.audio;
        MCSPrometheusAgent[operation](METRIC_NAMES.MEDIA_UNITS, { media_type: C.MEDIA_PROFILE.AUDIO, unit_type: media.type, direction });
    }
    if (media.mediaTypes.content) {
        const direction = media.mediaTypes.content === true ? 'sendrecv' : media.mediaTypes.content;
        MCSPrometheusAgent[operation](METRIC_NAMES.MEDIA_UNITS, { media_type: C.MEDIA_PROFILE.CONTENT, unit_type: media.type, direction });
    }
};
