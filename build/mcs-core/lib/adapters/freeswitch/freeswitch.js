...
const { ip: FREESWITCH_CONNECTION_IP, sip_ip: FREESWITCH_SIP_IP, port: FREESWITCH_PORT, handleExternalConnections: FS_HANDLE_EXTERNAL_CONNECTIONS, } = config.get('freeswitch');
...
module.exports = class Freeswitch extends EventEmitter {
    ...
    _handleExternalMediaConnected(channelId, callerIdNumber, roomId) {
        ...
        if (match = callerNameWithSessIdPattern.exec(callerIdNumber)) {
            ...
            throw (this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_INVALID_OPERATION), { details: `handleExternalMediaConnected failure on regex for: ${callerIdNumber}` })));
        }
        const channelInfo = this._channelIdInfos[channelId];
        ...
        return this.createMediaElement(roomId, C.MEDIA_TYPE.WEBRTC, {})
            ...
            Logger.info(LOG_PREFIX, `External audio media joined at ${roomId}: ${callerIdNumber}`, { roomId, callerIdNumber, channelId });
    ...
    _handleConferenceMember(channelId, memberId, callerIdNumber, roomId) {
        Logger.debug(LOG_PREFIX, "New conference member, associating UUID to mID", { roomId, channelId, memberId, callerIdNumber });
        const channelInfo = this._channelIdInfos[channelId];
        ...
        if (elementId == null && FS_HANDLE_EXTERNAL_CONNECTIONS) {
            this._handleExternalMediaConnected(channelId, callerIdNumber, roomId)
                ...
                .catch(error => {
                Logger.error(LOG_PREFIX, `External audio media handling failed due to ${error.message}`, { channelid, callerIdNumber, roomId, error });
            });
        }
        else {
    ...
    _handleVolumeChanged(channelId, volume) {
        const elementId = this._channelIdInfos[channelId].ua;
        const convertedVolume = convertRange({ floor: -4, ceiling: 4 }, { floor: 0, ceiling: 100 }, volume);
    ...
    _handleFloorChanged(roomId, newFloorMemberId) {
        ...
        Logger.debug(LOG_PREFIX, 'Video floor changed', { adapterElementId: newFloorElementId, roomId });
    ...
    _negotiateSDPEndpoint(roomId, userId, mediaSessionId, descriptor, type, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ({ mediaElement, host } = yield this.createMediaElement(roomId, type, options));
                const answer = yield this.processOffer(mediaElement, descriptor, options);
    ...
    createMediaElement(roomId, type, params) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const userAgentId = rid();
                let userAgent = yield this._createUserAgent(type, params.name, roomId);
                ...
                return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                    ...
                    switch (type) {
                        ...
                        case C.CONNECTION_TYPE.VIDEO:
                            try {
                                yield this._Kurento.connect(rtpConverter.elementId, sinkId, type);
                                return resolve();
                            }
                            catch (e) {
                                return reject(e);
    ...
    stop(roomId, type = {}, elementId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield this._stopUserAgent(elementId);
            }
            catch (error) {
                Logger.error(LOG_PREFIX, `Error when stopping userAgent for ${elementId} at room ${roomId}`, { error: this._handleError(error) });
            }
            try {
                yield this._stopRtpConverter(roomId, elementId);
            }
            catch (error) {
                Logger.error(LOG_PREFIX, `Error when stopping RTP converter for ${elementId} at room ${roomId}`, { error: this._handleError(error) });
            }
            try {
                yield this._stopRtpProxy(roomId, elementId);
            }
            catch (error) {
                Logger.error(LOG_PREFIX, `Error when stopping RTP proxy for ${elementId} at room ${roomId}`, { error: this._handleError(error) });
            }
    ...
    _stopRtpProxy(voiceBridge, elementId) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    _processProxyElement(voiceBridge, elementId, offer) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                let mediaElement, host;
                ({ mediaElement, host } = yield this._Kurento.createMediaElement(voiceBridge, C.MEDIA_TYPE.RTP, { mediaProfile: C.MEDIA_PROFILE.AUDIO }));
                this._rtpProxies[elementId] = { elementId: mediaElement, host };
                let answer;
                if (offer) {
                    answer = yield this._Kurento.processOffer(mediaElement, offer, { replaceIp: true });
                }
                else {
                    answer = yield this._Kurento.generateOffer(mediaElement);
                }
                ...
                try {
                    if (userAgent) {
                        if (hackProxyViaKurento) {
                            ...
                            proxyAnswer = yield this._processProxyElement(voiceBridge, elementId, sdpOffer);
                        ...
                        const session = this.sipCall(userAgent, name, userAgent.voiceBridge, FREESWITCH_SIP_IP, FREESWITCH_PORT, this._rtpConverters[elementId] ? this._rtpConverters[elementId].elementId : null, sdpOffer);
                        session.sessionId = userAgent.userAgentId;
                        session.isOfferer = !sdpOffer;
                        userAgent.session = session;
                        const handleOffer = (offer) => __awaiter(this, void 0, void 0, function* () {
                            if (!hackProxyViaKurento) {
                                return resolve(offer);
                            }
                            const procOffer = SdpWrapper.getAudioSDP(offer);
                            const procAns = yield this._Kurento.processOffer(this._rtpConverters[elementId].elementId, procOffer, { replaceIp: true });
    ...
    setVolume(mediaElementId, volume) {
        ...
        if (callId == null) {
            throw (this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_NOT_FOUND), { details: `setVolume. adapterElementId: ${mediaElementId}` })));
        }
        try {
            const channelId = userAgent.channelId;
            const memberId = this._channelIdInfos[channelId].memberId;
            Logger.debug(LOG_PREFIX, "Set volume", { adapterElementId: mediaElementId, channelId, memberId, volume });
            if (volume != 0) {
                // Normalizes volume from a 0/100 range to FS's -4/4 range
                const convertedVolume = convertRange({ floor: 0, ceiling: 100 }, { floor: -4, ceiling: 4 }, volume);
                if (userAgent.muted) {
            ...
            Logger.error(LOG_PREFIX, `setVolume failed due to ${error.message}`, { adapterElementId: mediaElementId, volume, error });
    ...
    mute(mediaElementId) {
        ...
        const { voiceBridge, callId } = userAgent;
        if (callId == null) {
            throw (this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_NOT_FOUND), { details: `mute. adapterElementId: ${mediaElementId}` })));
        }
        try {
            const channelId = userAgent.channelId;
            const memberId = this._channelIdInfos[channelId].memberId;
            return this._eslWrapper.mute(voiceBridge, memberId).then(() => {
                Logger.debug(LOG_PREFIX, "Audio muted", { adapterElementId: mediaElementId, channelId, memberId });
                userAgent.muted = true;
            });
        }
        catch (error) {
            Logger.error(LOG_PREFIX, `Mute failed due to ${error.message}`, { adapterElementId: mediaElementId, error });
            throw (this._handleError(error));
        }
    }
    unmute(mediaElementId) {
        ...
        const { voiceBridge, callId } = userAgent;
        if (callId == null) {
            throw (this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_NOT_FOUND), { details: `unmute. adapterElementId: ${mediaElementId}` })));
        }
        try {
            const channelId = userAgent.channelId;
            const memberId = this._channelIdInfos[channelId].memberId;
            return this._eslWrapper.unmute(voiceBridge, memberId).then(() => {
                userAgent.muted = false;
                Logger.debug(LOG_PREFIX, "Audio unmuted", { adapterElementId: mediaElementId, channelId, memberId });
                if (userAgent.volume) {
            ...
            Logger.error(LOG_PREFIX, `unmute failed due to ${error.message}`, { adapterElementId: mediaElementId, error });
            throw (this._handleError(error));
        }
    }
    ...
    dtmf(elementId, tone) {
        ...
        if (callId == null) {
            throw (this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_NOT_FOUND), { details: `dtmf. adapterElementId: ${elementId}` })));
        }
        try {
            const channelId = userAgent.channelId;
            ...
            Logger.error(LOG_PREFIX, `dtmf failed due to ${error.message}`, { adapterElementId: elementId, error });
            throw (this._handleError(error));
        }
    }
    requestKeyframe(elementId) {
        throw this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_INVALID_OPERATION), { details: "FREESWITCH_REQUEST_KEYFRAME_NOT_IMPLEMENTED" }));
    }
    _handleError(error) {
