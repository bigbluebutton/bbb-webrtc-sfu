...
class EslWrapper extends EventEmitter {
    ...
    _connect() {
        this._client = new Connection(this._clientOptions.host, this._clientOptions.port, this._clientOptions.auth, this._onConnected.bind(this));
    ...
    _monitorESLClientConnectionErrors() {
        this._client.on('error', (error) => {
            if (error) {
                Logger.error(LOG_PREFIX, `FreeSWITCH ESL connection received error ${error.code}`, { error });
    ...
    start() {
        try {
            ...
            Logger.error(LOG_PREFIX, `Error when starting ESL interface`, { error });
        ...
        if (this._reconnectionRoutine == null) {
            ...
            this._reconnectionRoutine = setInterval(() => __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    Logger.warn(LOG_PREFIX, `Failed to reconnect to ESL, try again in ${RECONNECTION_TIMER}`, { error });
                    this.stop();
                }
            }), RECONNECTION_TIMER);
        }
    ...
    _executeCommand(command) {
        return new Promise((resolve, reject) => {
            if (!this.connected) {
                Logger.error(LOG_PREFIX, `ESL wrapper is disconnected, unable to execute ${command}`);
                return reject(this.error);
            }
            Logger.debug(LOG_PREFIX, `Sending command: ${command}`);
    ...
    setVolume(conferenceId, memberId, volume) {
        try {
            const conferenceCommand = `${ESL_MESSAGE.CONFERENCE}${ESL_MESSAGE_SEPARATOR}` +
            ...
            Logger.error(LOG_PREFIX, `Error when executing setVolume command ${error.message}`, { conferenceId, memberId, volume, error });
            throw (this._normalizeError(error));
        }
    }
    /**
    ...
    mute(conferenceId, memberId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `Error when executing mute command ${error.message}`, { conferenceId, memberId, error });
            throw (this._normalizeError(error));
        }
    }
    /**
    ...
    unmute(conferenceId, memberId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `Error when executing unmute command ${error.message}`, { conferenceId, memberId, error });
            throw (this._normalizeError(error));
        }
    }
    dtmf(channelId, tone) {
        try {
            const conferenceCommand = `${ESL_MESSAGE.UUID_SEND_DTMF}${ESL_MESSAGE_SEPARATOR}` +
                `${channelId}${ESL_MESSAGE_SEPARATOR}${tone}`;
            return this._executeCommand(conferenceCommand);
        }
        catch (error) {
            Logger.error(LOG_PREFIX, `Error when executing dtmf command ${error.message}`, { channelId, tone, error });
            throw (this._normalizeError(error));
        }
    }
