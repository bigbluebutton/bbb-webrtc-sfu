...
module.exports = class Kurento extends EventEmitter {
    ...
    setMediaElement(mediaElement) {
        ...
        if (typeof this._mediaElements[enrichedId] === 'object') {
            return this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_ID_COLLISION), { details: "KURENTO_ID_COLLISION" }));
        }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (this._mediaPipelines[roomId] && this._mediaPipelines[roomId][host.id]) {
                    ...
                    Logger.debug(LOG_PREFIX, `Created pipeline at room ${roomId}`, { hostId: host.id, pipeline: pipeline.id, roomId });
                    return pipeline;
                }
            }
            catch (err) {
    ...
    _releaseAllRoomPipelines(room) {
        try {
            if (this._mediaPipelines[room]) {
                Object.keys(this._mediaPipelines[room]).forEach((pk) => __awaiter(this, void 0, void 0, function* () {
                    yield this._releasePipeline(room, pk);
                }));
            }
        }
        catch (e) {
    ...
    _releasePipeline(room, hostId) {
        return new Promise((resolve, reject) => {
            try {
                Logger.debug(LOG_PREFIX, `Releasing pipeline of room ${room}`, { hostId, roomId: room });
    ...
    _createElement(pipeline, type, options = {}) {
        return new Promise((resolve, reject) => {
            try {
                ...
                pipeline.create(type, { stopOnEndOfStream, uri, mediaProfile: recordingProfile }, (error, mediaElement) => {
                    if (error) {
                        return reject(this._handleError(error));
                    }
            ...
            const negotiationProcedures = partialDescriptors.map((d, i) => {
                return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                    try {
                        ...
                        ({ mediaElement, host } = yield this.createMediaElement(roomId, type, Object.assign(Object.assign({}, options), { mediaType })));
                        media.adapterElementId = mediaElement;
                        media.host = host;
                        media.trackMedia();
                        if (d) {
                            answer = yield this.processOffer(mediaElement, d, options);
                        }
                        else {
                            ...
                            answer = yield this.generateOffer(mediaElement, options);
                        }
    ...
    _negotiateWebRTCEndpoint(roomId, userId, mediaSessionId, descriptor, type, options) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const isTrickled = typeof options.trickle === 'undefined' || options.trickle;
                options.trickle = isTrickled;
                const medias = yield this._negotiateSDPEndpoint(roomId, userId, mediaSessionId, descriptor, type, options);
                if (isTrickled) {
                    medias.forEach(m => {
                        if (m.type === C.MEDIA_TYPE.WEBRTC) {
                            this.gatherCandidates(m.adapterElementId).catch(error => {
                                Logger.error(LOG_PREFIX, `Candidate gathering for media ${m.id} failed due to ${error.message}`, { mediaId: m.id, adapterElementId: m.adapterElementId, errorMessage: error.message, errorCode: error.code });
                            });
                        }
                    });
                    return medias;
    ...
    _negotiateRecordingEndpoint(roomId, userId, mediaSessionId, descriptor, type, options) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const mediaType = this._parseMediaType(media);
                ({ mediaElement, host } = yield this.createMediaElement(roomId, type, Object.assign(Object.assign({}, options), { mediaType })));
                const answer = yield this.startRecording(mediaElement);
                // Just do a late-set of the properties that were nullified in the early
                // media instantiation
                media.adapterElementId = mediaElement;
                media.host = host;
    ...
    createMediaElement(roomId, type, options = {}) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const host = yield this.balancer.getHost(mediaType);
                yield this._getMediaPipeline(host.id, roomId);
                const pipeline = this._mediaPipelines[roomId][host.id];
                const mediaElement = yield this._createElement(pipeline, type, options);
    ...
    _transposeAndConnect(sourceId, sinkId) {
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                try {
                    const source = this.getMediaElement(sourceId);
                    ...
                    if (sourceTransposer == null) {
                        ...
                        Logger.debug(LOG_PREFIX, "Source transposer not found", { sourceId, sinkHostId: sink.host.id });
                        sourceTransposer = yield this._createElement(source.pipeline, C.MEDIA_TYPE.RTP);
                        source.transposers[sink.host.id] = sourceTransposer;
                        sourceOffer = yield this.generateOffer(this.getMediaElementId(sourceTransposer));
                        ...
                        sink.pipeline.transposers[source.host.id + sourceId] = sinkTransposer = yield this._createElement(sink.pipeline, C.MEDIA_TYPE.RTP);
                        sinkAnswer = yield this.processOffer(this.getMediaElementId(sinkTransposer), SdpWrapper.stReplaceServerIpv4(sourceOffer, source.host.ip));
                        yield this.processAnswer(this.getMediaElementId(sourceTransposer), SdpWrapper.stReplaceServerIpv4(sinkAnswer, sink.host.ip));
    ...
    connect(sourceId, sinkId, type) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (shouldTranspose) {
                    yield this._transposeAndConnect(sourceId, sinkId);
                    return resolve();
                }
                else {
                    yield this._connect(source, sink, type);
                    resolve();
                }
            }
            catch (error) {
        ...
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (isTransposed) {
                    ...
                    yield this._disconnect(transposedSink, sink, type);
                    resolve();
                }
                else {
                    yield this._disconnect(source, sink, type);
                    resolve();
                }
            }
            catch (error) {
        ...
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (type === 'RecorderEndpoint') {
                    yield this._stopRecording(elementId);
                }
                if (mediaElement) {
                    ...
                    if (mediaElement.transposers) {
                        Object.keys(mediaElement.transposers).forEach(t => {
                            setTimeout(() => {
                                ...
                                Logger.debug(LOG_PREFIX, `Releasing transposer`, { transposerId: t, elementId });
                    ...
                    if (typeof mediaElement.release === 'function') {
                        mediaElement.release((error) => __awaiter(this, void 0, void 0, function* () {
                            if (error) {
                                return reject(this._handleError(error));
                            }
                            if (pipeline) {
                                pipeline.activeElements--;
                                Logger.info(LOG_PREFIX, `Pipeline elements decreased for room ${room}`, { activeElements: pipeline.activeElements, roomId: room, hostId });
                                if (pipeline.activeElements <= 0) {
                                    yield this._releasePipeline(room, hostId);
                                }
                            }
                            return resolve();
                        }));
    ...
    _vanillaGatherCandidates(elementId) {
        return __awaiter(this, void 0, void 0, function* () {
            const mediaElement = this.getMediaElement(elementId);
            ...
            try {
                yield this.gatherCandidates(elementId);
                return Promise.race([handleGatheringDone, failOver()]);
            }
            catch (error) {
                Logger.error(LOG_PREFIX, `Vanilla candidate gathering failed`, { error, mediaElementId: elementId });
                ...
                if (mediaElement) {
                    ...
                    mediaElement.processOffer(sdpOffer, (error, answer) => {
                        ...
                        this._vanillaGatherCandidates(elementId)
                            ...
                            Logger.info(LOG_PREFIX, `Vanilla candidate gathering succeeded`, { mediaElementId: elementId });
                            return resolve(localDescriptor);
                        })
                            .catch(error => {
                            return reject(this._handleError(error));
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (typeof mediaElement.requestKeyframe !== 'function') {
                    throw this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_INVALID_OPERATION), { details: "KURENTO_REQUEST_KEYFRAME_NOT_IMPLEMENTED" }));
                }
    ...
    dtmf(elementId, tone) {
        throw this._handleError(Object.assign(Object.assign({}, C.ERROR.MEDIA_INVALID_OPERATION), { details: "KURENTO_DTMF_NOT_IMPLEMENTED" }));
    }
                    ...
                    switch (eventTag) {
                        case C.EVENT.MEDIA_STATE.ICE:
                            ...
                            event = {
                                ...
                                timestampUTC,
                                timestampHR,
                                rawEvent: Object.assign({}, rawEvent),
                            };
    ...
    _destroyElementsFromHost(hostId) {
        try {
            ...
            Logger.error(LOG_PREFIX, `Error destroying elements from host ${hostId}`, { error, hostId });
        }
    }
    _handleError(err) {
        ...
        if (stack && !err.stackWasLogged) {
            Logger.error(LOG_PREFIX, `Stack trace for error ${err.code} | ${err.message} ->`, { errorStack: err.stack.toString() });
