...
module.exports = class Media extends EventEmitter {
    constructor(roomId, userId, mediaSessionId, type, adapter, adapterElementId, host, options = {}) {
        super();
        this.id = rid();
        this.roomId = roomId;
        this.userId = userId;
        ...
        this.mediaSpecs = options.mediaSpecs ? options.mediaSpecs : Object.assign({}, MEDIA_SPECS);
    ...
    _connect(sink, connectionType) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            connSinks.forEach((sim) => __awaiter(this, void 0, void 0, function* () {
                const logObject = {
                    sourceId: this.id,
                ...
                try {
                    yield this.adapter.connect(this.adapterElementId, sim.adapterElementId, connectionType);
                    // Update the sink's source data
                    sim.subscribedTo = this.id;
                }
                catch (error) {
                    Logger.error(LOG_PREFIX, `Failed to run underlying _connect procedure due to ${error.message}`, Object.assign(Object.assign({}, logObject), { error }));
                }
            }));
        });
    }
    disconnect(sink, type = 'ALL') {
        return __awaiter(this, void 0, void 0, function* () {
            try {
    ...
    _disconnect(sink, connectionType) {
        return __awaiter(this, void 0, void 0, function* () {
            const sinks = sink.medias ? sink.medias : [sink];
            const sinkMediasToDisconnect = sinks.filter(sim => sim.subscribedTo === this.id);
            sinkMediasToDisconnect.forEach((sim) => __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    yield this.adapter.disconnect(this.adapterElementId, sim.adapterElementId, connectionType);
    ...
    _dispatchMediaStateEvent(event) {
        ...
        const normEvent = Object.assign({ mediaSessionId: this.mediaSessionId, mediaId: this.mediaSessionId }, event);
        this.addToEventBuffer(C.EVENT.MEDIA_STATE.MEDIA_EVENT, normEvent);
        Logger.trace(LOG_PREFIX, `Dispatched ${C.EVENT.MEDIA_STATE.MEDIA_EVENT}`, { mediaSessionId: this.mediaSessionId, mediaId: this.id, event: normEvent });
    ...
    _dispatchIceCandidate(event) {
        ...
        Logger.trace(LOG_PREFIX, `Dispatched ${C.EVENT.MEDIA_STATE.ICE}`, { mediaSessionId: this.mediaSessionId, mediaId: this.id, event: normEvent });
