...
module.exports = class SDPSession extends MediaSession {
    constructor(remoteDescriptor = null, room, user, type = 'WebRtcEndpoint', options) {
    ...
    set shouldProcessRemoteDescriptorAsAnswerer(shouldProcess) {
        if (shouldProcess === true && !this.shouldProcessRemoteDescriptorAsAnswerer) {
            GLOBAL_EVENT_EMITTER.emit(`${C.EVENT.MEDIA_NEGOTIATED}:${this.id}`, this.getMediaInfo());
        }
    ...
    _negotiateAudioMedias(descriptor = '') {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const audioOptions = isAnswerer ?
                this._options : Object.assign(Object.assign({}, this._options), { mediaProfile: C.MEDIA_PROFILE.AUDIO });
            const shouldNegotiate = !!descriptor
                || (this.profiles.audio && !isAnswerer);
            if (shouldNegotiate) {
                try {
                    const audioMedias = yield audioAdapter.negotiate(this.roomId, this.userId, this.id, descriptor, this.type, audioOptions);
    ...
    _negotiateVideoMedias(descriptor = '') {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const videoOptions = isAnswerer ?
                this._options : Object.assign(Object.assign({}, this._options), { mediaProfile: C.MEDIA_PROFILE.MAIN });
            const shouldNegotiate = !!descriptor
                || (this.profiles.video && !isAnswerer);
            if (shouldNegotiate) {
                try {
                    const videoMedias = yield videoAdapter.negotiate(this.roomId, this.userId, this.id, descriptor, this.type, videoOptions);
    ...
    _negotiateContentMedias(descriptor = '') {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const contentOptions = isAnswerer ?
                this._options : Object.assign(Object.assign({}, this._options), { mediaProfile: C.MEDIA_PROFILE.CONTENT });
            const shouldNegotiate = !!descriptor
                || (this.profiles.content && !isAnswerer);
            if (shouldNegotiate) {
                try {
                    const contentMedias = yield contentAdapter.negotiate(this.roomId, this.userId, this.id, descriptor, this.type, contentOptions);
    ...
    _negotiateApplicationMedias(descriptor) {
        return __awaiter(this, void 0, void 0, function* () {
            return this._rejectMedias(descriptor);
        });
    }
    ;
    _rejectMedias(descriptor = '') {
        return __awaiter(this, void 0, void 0, function* () {
            if (!descriptor) {
                return [];
            }
            const unsupportedMediaContainer = new InternalUnsupportedMedia(this.roomId, this.userId, this.id, descriptor, C.MEDIA_TYPE.INTERNAL_UNSUPPORTED);
            return [unsupportedMediaContainer];
        });
    }
    ;
    _composedAdapterProcess() {
        return __awaiter(this, void 0, void 0, function* () {
            const remoteDescriptor = this.remoteDescriptor
                ? this.remoteDescriptor._plainSdp
                : '';
            let { numberOfMediaLines, annotatedDescriptors: annotatedPartialDescriptors, } = SdpWrapper.getAnnotatedPartialDescriptors(remoteDescriptor);
            ...
            return Object.keys(annotatedPartialDescriptors).reduce((promise, type) => {
                return promise.then(() => {
                    ...
                    return processMethods[type](descriptor)
                        ...
                        Logger.error(LOG_PREFIX, `Failed to negotiate ${type} medias, none will be generated due to ${error.message}`, { error: this._handleError(error) });
                    });
                });
                ...
                Logger.error(LOG_PREFIX, `Composed adapter process error on promise chain ${error.message}`, { error: normalizedError });
            });
            ;
        });
    }
    ...
    renegotiateStreams() {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                const { videoAdapter, audioAdapter, contentAdapter } = this._adapters;
                ...
                if (this.remoteDescriptor.mainVideoSdp && this.shouldProcessRemoteDescriptorAsAnswerer) {
                    try {
                        ...
                        yield videoAdapter.processAnswer(videoMedia.adapterElementId, this.remoteDescriptor.mainVideoSdp);
                ...
                if (this.remoteDescriptor.audioSdp && this.shouldProcessRemoteDescriptorAsAnswerer) {
                    try {
                        ...
                        yield audioAdapter.processAnswer(audioMedia.adapterElementId, mainWithInvalidVideo);
                ...
                if (this.remoteDescriptor.invalidSdps
                    ...
                    const rejectedMedias = yield this._rejectMedias(this.remoteDescriptor.invalidSdps);
                ...
                if (this.remoteDescriptor.contentVideoSdp) {
                    ...
                    if (this.shouldProcessRemoteDescriptorAsAnswerer || (this.localDescriptor && this.localDescriptor.contentVideoSdp)) {
                        ...
                        yield contentAdapter.processAnswer(contentMedia.adapterElementId, this.remoteDescriptor.contentVideoSdp);
                        ...
                        try {
                            const contentMedias = yield this._negotiateContentMedias(this.remoteDescriptor.contentVideoSdp);
                            let currentMediaLength = this.medias.length + this.invalidMedias.length;
                    ...
                    try {
                        try {
                            const contentMedias = yield this._negotiateContentMedias();
                            let currentMediaLength = this.medias.length + this.invalidMedias.length;
                            contentMedias.forEach(cm => {
                                cm.sdpPosition = currentMediaLength;
                                currentMediaLength += 1;
    ...
    process() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.shouldRenegotiate || this.shouldProcessRemoteDescriptorAsAnswerer) {
                try {
                    localDescriptorAnswer = yield this.renegotiateStreams();
            ...
            try {
                ...
                Logger.error(LOG_PREFIX, `Failed to process SDP session ${this.id} due to ${error.message}`, { roomId: this.roomId, userId: this.userId, mediaSessionId: this.id, error });
    ...
    addIceCandidate(candidate) {
        ...
        if (sdpMLineIndex && this.medias[sdpMLineIndex]) {
            ...
            throw this._handleError(Object.assign(Object.assign({}, C.ERROR.ICE_CANDIDATE_FAILED), { details: `Invalid or not found media index` }));
        }
    }
    getAnswer() {
        let header = '', body = '';
