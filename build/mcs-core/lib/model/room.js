...
module.exports = class Room {
    ...
    isAboveThreshold() {
        if (ROOM_MEDIA_THRESHOLD > 0 && this.medias.length >= ROOM_MEDIA_THRESHOLD) {
            Logger.error(LOG_PREFIX, `Room has exceeded the media threshold`, { roomId: this.id, threshold: ROOM_MEDIA_THRESHOLD, current: this.medias.length });
            return true;
        }
        return false;
    }
    ...
    getSourceMediaSessionsOfType(mediaType) {
        return this.mediaSessions.filter(({ medias }) => medias.some(({ mediaTypes }) => mediaTypes[mediaType] && mediaTypes[mediaType] !== 'recvonly'));
    }
    getSinkMediaSessionsOfType(mediaType) {
        return this.mediaSessions.filter(({ medias }) => medias.some(({ mediaTypes }) => mediaTypes[mediaType] && mediaTypes[mediaType] !== 'sendonly'));
    }
    ...
    setConferenceFloor(media) {
        ...
        GLOBAL_EVENT_EMITTER.emit(C.EVENT.CONFERENCE_FLOOR_CHANGED, Object.assign({ roomId: this.id }, conferenceFloorInfo));
        return conferenceFloorInfo;
    }
    ...
    setContentFloor(media) {
        ...
        GLOBAL_EVENT_EMITTER.emit(C.EVENT.CONTENT_FLOOR_CHANGED, Object.assign({ roomId: this.id }, contentFloorInfo));
        return contentFloorInfo;
    }
