...
module.exports = class ScreenshareManager extends BaseManager {
    ...
    _trackMediaServerOfflineEvent(session, connectionId, voiceBridge, userId, role) {
        session.once(C.MEDIA_SERVER_OFFLINE, (event) => {
            ...
            this.sendToClient(Object.assign({}, errorMessage), C.FROM_SCREENSHARE);
    ...
    _trackPresenterChangeEvent() {
        this._bbbGW.on(C.PRESENTER_ASSIGNED_2x, (payload) => {
            ...
            if (session && session.userId !== presenterId) {
                Logger.info(this._logPrefix, "Presenter changed, closing screensharing session", Object.assign(Object.assign({}, session._getFullPresenterLogMetadata()), { oldPresenterId: session.userId, presenterId }));
    ...
    handleStart(message) {
        const { internalMeetingId, voiceBridge, connectionId, role, sdpOffer, callerName: userId, bitrate, vh = 0, vw = 0, hasAudio, mediaServer = SCREENSHARE_MEDIA_SERVER, } = message;
        let iceQueue, session;
        session = this._fetchSession(voiceBridge);
        iceQueue = this._fetchIceQueue(voiceBridge);
        if (!session) {
            session = new Screenshare(connectionId, this._bbbGW, voiceBridge, userId, vh, vw, internalMeetingId, this.mcs, hasAudio);
        ...
        return session.start(connectionId, userId, role, sdpOffer, options)
            .then(sdpAnswer => {
            Logger.debug(this._logPrefix, "Screensharing session started", ScreenshareManager.getMetadataFromMessage(message));
            ...
            this._trackMediaServerOfflineEvent(session, connectionId, voiceBridge, userId, role);
        })
            .catch(error => {
            ...
            this.sendToClient(Object.assign({}, errorMessage), C.FROM_SCREENSHARE);
            this.sendToClient(Object.assign({}, closeMessage), C.FROM_SCREENSHARE);
        });
    }
    handleIceCandidate(message) {
        const { voiceBridge, candidate, role, callerName: userId, connectionId, } = message;
        let iceQueue, session;
        session = this._fetchSession(voiceBridge);
        iceQueue = this._fetchIceQueue(voiceBridge);
        if (session && session.constructor === Screenshare) {
            session.onIceCandidate(candidate, role, userId, connectionId);
            Logger.debug(this._logPrefix, "Screensharing ICE candidate added", ScreenshareManager.getMetadataFromMessage(message));
        }
        else {
    ...
    handleStop(message) {
        ...
        this._stopSession(voiceBridge).then(() => {
            ...
            Logger.info(this._logPrefix, "Screensharing session destroyed", ScreenshareManager.getMetadataFromMessage(message));
        }).catch(error => {
            this._deleteIceQueue(voiceBridge);
            Logger.error(this._logPrefix, `Screensharing session stop failed`, Object.assign({ errorMessage: error.message, errorCode: error.code }, ScreenshareManager.getMetadataFromMessage(message)));
        });
    }
    handleClose(message) {
        const { voiceBridge, candidate, role, connectionId, } = message;
        let session;
        session = this._fetchSession(voiceBridge);
        Logger.info(this._logPrefix, 'Connection closed', ScreenshareManager.getMetadataFromMessage(message));
        return this.closeSession(session, role, voiceBridge, connectionId);
    }
    _onMessage(message = {}) {
    ...
    _handleRoomCreated(event) {
        ...
        this.mcs.onEvent('roomDestroyed', room, (event) => {
            try {
                ...
                Logger.error(this._logPrefix, `Screensharing session stop failed at room destroyed handler`, { error: this._handleError(this._logPrefix, null, null, null, error), roomId: room });
            }
        });
    ...
    _handleContentFloorChanged(event) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                Logger.error(this._logPrefix, `Screensharing session stop failed at content floor changed handler`, { error: this._handleError(this._logPrefix, null, null, null, error), roomId: room });
            }
        });
    }
    closeSession(session, role, sessionId, connectionId = '') {
        if (session && session.constructor == Screenshare) {
            if (role === C.SEND_ROLE) {
                Logger.info(this._logPrefix, `Stopping screensharing presenter session ${sessionId}`, session._getPartialLogMetadata());
                ...
                return this._stopSession(sessionId).then(() => {
                    ...
                    this._deleteIceQueue(sessionId);
                    delete this._meetings[internalMeetingId];
                    Logger.error(this._logPrefix, "Screensharing session stop failed at closeSession", { error: this._handleError(this._logPrefix, null, null, null, error) });
                });
            }
            if (role === C.RECV_ROLE && session) {
                Logger.info(this._logPrefix, "Stopping screensharing viewer", session._getFullViewerLogMetadata(connectionId));
