...
module.exports = class VideoManager extends BaseManager {
    ...
    static getVideoSpecsFromRequest(message) {
        ...
        const spec = Object.assign({}, DEFAULT_MEDIA_SPECS);
        if (bitrate != null) {
    ...
    _trackMediaServerOfflineEvent(session) {
        session.once(C.MEDIA_SERVER_OFFLINE, (event) => {
            ...
            const errorMessage = this._handleError(this._logPrefix, session.connectionId, session.id, session.role, errors.MEDIA_SERVER_OFFLINE);
            this.sendToClient(Object.assign({}, errorMessage), C.FROM_VIDEO);
        });
    }
    ...
    handleStart(message) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const role = VideoManager.getRole(message);
            const { userId, voiceBridge, meetingId, connectionId, sdpOffer, cameraId, record = true, mediaServer = VIDEO_MEDIA_SERVER, } = message;
            video = this._fetchSession(sessionId);
            iceQueue = this._fetchIceQueue(sessionId);
            if (video) {
                Logger.warn(this._logPrefix, `Aborted trailing start request for video session`, video._getLogMetadata());
                return;
            }
            else {
                video = new Video(this._bbbGW, meetingId, cameraId, role, connectionId, this.mcs, voiceBridge, userId, sessionId, record, mediaServer);
            ...
            return video.start(sdpOffer, mediaSpecs)
                .then(sdpAnswer => {
                Logger.info(this._logPrefix, "Video session started", VideoManager.getMetadataFromMessage(message));
                ...
                return this.sendToClient(Object.assign({}, errorMessage), C.FROM_VIDEO);
            });
        });
    }
    handleStop(message) {
        ...
        return this._stopSession(sessionId).then(() => {
            ...
            Logger.error(this._logPrefix, "Video session stop failed", Object.assign({ errorMessage: error.message, errorCode: error.code }, VideoManager.getMetadataFromMessage(message)));
            this._deleteIceQueue(sessionId);
        });
    }
    handleIceCandidate(message) {
        ...
        if (VideoManager.isVideoInstanceReady(video)) {
            ...
            Logger.debug(this._logPrefix, 'Video ICE candidate added', VideoManager.getMetadataFromMessage(message));
        }
        else {
    ...
    handleSubscriberAnswer(message) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const video = this._fetchSession(sessionId);
            try {
                yield video.processAnswer(answer);
            }
            catch (error) {
    ...
    handleClose(message) {
        ...
        Logger.info(this._logPrefix, 'Connection closed', VideoManager.getMetadataFromMessage(message));
