...
module.exports = class Video extends BaseProvider {
    constructor(bbbGW, meetingId, cameraId, role, connectionId, mcs, voiceBridge, bbbUserId, managerSessionId, record, mediaServer) {
        super(bbbGW);
    ...
    static setSource(id, stream) {
        Logger.debug(LOG_PREFIX, 'New source set', { cameraId: id, mediaId: stream });
        sources[id] = stream;
    }
    static getSource(id) {
        return sources[id];
    ...
    static removeSource(id) {
        Logger.debug(LOG_PREFIX, 'Media source removed', { cameraId: id });
        delete sources[id];
    }
    /* ======= ICE HANDLERS ======= */
    processAnswer(answer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                yield this.mcs.subscribe(this.userId, stream, C.WEBRTC, Object.assign(Object.assign({}, this.options), { descriptor: answer, mediaId: this.mediaId }));
            }
            catch (error) {
                throw error;
            }
    ...
    onIceCandidate(_candidate) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.mediaId) {
                try {
                    yield this.mcs.addIceCandidate(this.mediaId, _candidate);
                }
                catch (error) {
                    this._handleError(LOG_PREFIX, error, this.role, this.id);
                    Logger.error(LOG_PREFIX, `ICE candidate failed to be added for ${this.streamName}`, Object.assign(Object.assign({}, this._getLogMetadata()), { error }));
                }
            }
            else {
                try {
                    ...
                    Logger.error(LOG_PREFIX, `Error on queuing ICE candidate for ${this.streamName}`, Object.assign(Object.assign({}, this._getLogMetadata()), { error }));
                }
            }
        });
    }
    ...
    _handleIceComponentStateChange(state) {
        ...
        Logger.debug(LOG_PREFIX, `Video ICE component state changed`, Object.assign(Object.assign({}, this._getLogMetadata()), { elementId,
            iceComponentId,
            iceComponentState }));
    }
    _handleCandidatePairSelected(state) {
        ...
        Logger.info(LOG_PREFIX, `Video new candidate pair selected`, Object.assign(Object.assign({}, this._getLogMetadata()), { elementId,
            iceComponentId,
            localCandidate,
            remoteCandidate }));
    }
    _handleIceGatheringDone(state) {
        ...
        Logger.debug(LOG_PREFIX, `Video ICE gathering done`, Object.assign(Object.assign({}, this._getLogMetadata()), { elementId }));
    }
    _handleMediaStateChanged(state) {
        const { rawEvent, details } = state;
        const { source: elementId } = rawEvent;
        Logger.info(LOG_PREFIX, `Video media state changed`, Object.assign(Object.assign({}, this._getLogMetadata()), { elementId, mediaState: details }));
    }
    _mediaStateWebRTC(event, endpoint) {
        ...
        switch (name) {
                ...
                if (details === 'NOT_FLOWING' && this.status !== C.MEDIA_PAUSED) {
                    if (!this.notFlowingTimeout) {
                        ...
                        this.notFlowingTimeout = setTimeout(() => {
                            if (this.shared) {
                                Logger.warn(LOG_PREFIX, "Media NOT_FLOWING timeout hit, stopping media", this._getLogMetadata());
                                this.sendPlayStop();
                    ...
                    if (this.notFlowingTimeout) {
                        Logger.debug(LOG_PREFIX, "Media FLOWING received while timeout was set, clearing it", this._getLogMetadata());
                        clearTimeout(this.notFlowingTimeout);
                        delete this.notFlowingTimeout;
                    }
            ...
            case C.MEDIA_SERVER_OFFLINE:
                Logger.error(LOG_PREFIX, "Video provider received MEDIA_SERVER_OFFLINE event", Object.assign(Object.assign({}, this._getLogMetadata()), { event }));
    ...
    _mediaStateRecording(event, endpoint) {
        ...
        switch (name) {
            ...
            case "MediaFlowInStateChange":
                if (details === 'NOT_FLOWING' && this.status !== C.MEDIA_PAUSED) {
                    Logger.warn(LOG_PREFIX, `Recording media STOPPED FLOWING on endpoint ${endpoint}`, this._getLogMetadata());
                }
                else if (details === 'FLOWING') {
    ...
    sendBroadcastCamStop() {
        ...
        if (typeof this.meetingId === 'string'
            ...
            const userCamEvent = Messaging.generateUserCamBroadcastStoppedEventMessage2x(this.meetingId, this.bbbUserId, this.id);
    ...
    sendStopShareEvent() {
        ...
        const stopShareEvent = Messaging.generateWebRTCShareEvent('StopWebRTCShareEvent', this.meetingId, this.recording.filename, timestampHR, timestampUTC);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    const recordingPath = this.getRecordingPath(this.meetingId, this._recordingSubPath, recordingName, format);
                    const recordingId = yield this.mcs.startRecording(this.userId, this.mediaId, recordingPath, { recordingProfile, ignoreThresholds: true });
                    ...
                    Logger.error(LOG_PREFIX, "Error on recording start", Object.assign(Object.assign({}, this._getLogMetadata()), { error }));
    ...
    getBroadcastPermission(meetingId, userId, sfuSessionId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const msg = Messaging.generateGetCamBroadcastPermissionReqMsg(meetingId, userId, sfuSessionId);
    ...
    getSubscribePermission(meetingId, userId, streamId, sfuSessionId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const msg = Messaging.generateGetCamSubscribePermissionReqMsg(meetingId, userId, streamId, sfuSessionId);
        ...
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            if (this.status === C.MEDIA_STOPPED) {
                try {
                    ...
                    if (this.shared) {
                        yield this.getBroadcastPermission(this.meetingId, this.bbbUserId, this.streamName);
                    }
                    else {
                        yield this.getSubscribePermission(this.meetingId, this.bbbUserId, this.id, this.streamName);
                    }
                    ...
                    if (SHOULD_RECORD && this.shared) {
                        this.isRecorded = yield this.probeForRecordingStatus(this.meetingId, this.id);
                    }
                    const userId = yield this.mcs.join(this.voiceBridge, 'SFU', { externalUserId: this.bbbUserId, autoLeave: true });
                    this.userId = userId;
                    const sdpAnswer = yield this._addMCSMedia(C.WEBRTC, sdpOffer, mediaSpecs);
                    ...
                    Logger.error(LOG_PREFIX, `Video start procedure failed due to ${error.message}`, Object.assign(Object.assign({}, this._getLogMetadata()), { error }));
                    this.status = C.MEDIA_NEGOTIATION_FAILED;
                    reject(this._handleError(LOG_PREFIX, error, this.role, this.id));
                }
            }
            else {
                Logger.warn(LOG_PREFIX, `Video rejected due to invalid status`, this._getLogMetadata());
        ...
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                if (this.shared) {
                    ...
                    const { mediaId, answer } = yield this.mcs.publish(this.userId, this.voiceBridge, type, options);
                    ...
                    if (stream) {
                        ...
                        Logger.warn(LOG_PREFIX, `Publisher stream from ${this.id} isn't set yet. Rejecting with MEDIA_NOT_FOUND`, this._getLogMetadata());
                        throw error;
                    }
                }
            }
    ...
    _subscribeToMedia(descriptor, mediaSpecs) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Get the REMB spec to be used. Video uses the default mixed in with
                // the custom bitrate sent when the profile is chosen. Fetching bitrate
                ...
                const stream = Video.getSource(this.id);
                const { mediaId, answer } = yield this.mcs.subscribe(this.userId, stream, C.WEBRTC, options);
                this.mediaId = mediaId;
                return answer;
            }
            catch (err) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (state && (this.status !== C.MEDIA_STARTING || this.status !== C.MEDIA_PAUSED)) {
                    yield this.mcs.disconnect(stream, sinkId, 'VIDEO');
                    this.status = C.MEDIA_PAUSED;
                }
                else if (!state && this.status === C.MEDIA_PAUSED) { //un-pause
                    yield this.mcs.connect(stream, sinkId, 'VIDEO');
                    this.status = C.MEDIA_STARTED;
                }
            }
            catch (err) {
    ...
    _finishVideoSession(resolver) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                Logger.warn(LOG_PREFIX, `Error on stopping recording for user ${this.userId} with stream ${this.streamName}`, Object.assign(Object.assign({}, this._getLogMetadata()), { error }));
            }
            if (this.mediaId) {
                if (this.shared) {
                    try {
                        yield this.mcs.unpublish(this.userId, this.mediaId);
                    }
                    catch (error) {
                        Logger.error(LOG_PREFIX, `Unpublish failed for user ${this.userId} with stream ${this.streamName}`, Object.assign(Object.assign({}, this._getLogMetadata()), { error }));
                    }
                    Video.removeSource(this.id);
                }
                else {
                    try {
                        yield this.mcs.unsubscribe(this.userId, this.mediaId);
                    }
                    catch (error) {
                        Logger.error(LOG_PREFIX, `Unsubscribe failed for user ${this.userId} with stream ${this.streamName}`, Object.assign(Object.assign({}, this._getLogMetadata()), { error }));
                    }
                }
            }
            if (this.shared) {
            ...
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                ...
                switch (this.status) {
                    case C.MEDIA_STOPPED:
                        Logger.warn(LOG_PREFIX, `Video session ${this.streamName} already stopped`, this._getLogMetadata());
                        return resolve();
                        break;
                    case C.MEDIA_STOPPING:
                        Logger.warn(LOG_PREFIX, `Video session ${this.streamName} already stopping`, this._getLogMetadata());
                        this.once(C.MEDIA_STOPPED, () => {
                            Logger.info(LOG_PREFIX, `Calling delayed stop resolution for queued stop call for ${this.streamName}`, this._getLogMetadata());
                            return resolve();
                        });
                        break;
                    case C.MEDIA_STARTING:
                        Logger.warn(LOG_PREFIX, `Video session ${this.streamName} is still starting. Let it finish then stop it`, this._getLogMetadata());
                        if (!this._stopActionQueued) {
                            this._stopActionQueued = true;
                            this.once(C.MEDIA_NEGOTIATED, () => {
                                Logger.info(LOG_PREFIX, `Calling delayed MEDIA_STARTING stop action for queued stop call for ${this.streamName}`, this._getLogMetadata());
    ...
    disconnectUser() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                Logger.info(LOG_PREFIX, 'Disconnect a video session on UserLeft*/DisconnectAll', this._getLogMetadata());
                yield this.stop();
            }
            catch (error) {
                Logger.warn(LOG_PREFIX, 'Failed to disconnect video session on UserLeft*/DisconnectAll', Object.assign(Object.assign({}, this._getLogMetadata()), { error }));
            }
            finally {
