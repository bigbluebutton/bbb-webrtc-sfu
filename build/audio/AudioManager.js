...
module.exports = class AudioManager extends BaseManager {
    ...
    _handleUserJoinedVoiceConf(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (FS_HANDLE_EXTERNAL_CONNECTIONS && !listenOnly && userId.startsWith("w_")) {
                    yield this.mcs.join(voiceConf, 'SFU', { userId, externalUserId: userId, name: callerName, autoLeave: true });
                }
            }
            catch (error) {
                Logger.warn(this._logPrefix, `Failed to pre-start audio user`, { userId, voiceConf, errorMessage: error.message, errorCode: error.code });
            }
        });
    }
    // FIXME enqueue stop
    _disconnectAllUsers(meetingId) {
        ...
        if (typeof sessionId !== 'undefined') {
            ...
            if (session) {
                Logger.info(this._logPrefix, 'Disconnecting all listen only sessions', { roomId: sessionId, internalMeetingId: meetingId });
    ...
    _handleSessionWideError(error, sessionId, rawMessage) {
        Logger.error(this._logPrefix, `Listen only session wide fatal failure`, Object.assign({ errorMessage: error.message, errorCode: error.code }, AudioManager.getMetadataFromMessage(rawMessage)));
        error.id = 'webRTCAudioError';
        this._stopSession(sessionId);
        this.sendToClient(Object.assign({ type: 'audio' }, error), C.FROM_AUDIO);
    }
    _handleListenerStartError(session, userConnectionId, error, rawMessage) {
        Logger.error(this._logPrefix, `Listen only listener failure`, Object.assign({ errorMessage: error.message, errorCode: error.code }, AudioManager.getMetadataFromMessage(rawMessage)));
        error.id = 'webRTCAudioError';
        if (session)
            session.stopListener(userConnectionId);
        this.sendToClient((Object.assign({ type: 'audio' }, error)), C.FROM_AUDIO);
    }
    handleStart(message) {
        return __awaiter(this, void 0, void 0, function* () {
            const { connectionId, voiceBridge: sessionId, internalMeetingId, sdpOffer, userId, userName, mediaServer = AUDIO_MEDIA_SERVER, } = message;
            ...
            return session.start(sessionId, connectionId, sdpOffer, userId, userName, mediaServer)
                ...
                session.once(C.MEDIA_SERVER_OFFLINE, (event) => __awaiter(this, void 0, void 0, function* () {
                ...
                Logger.info(this._logPrefix, `Started listen only session for user ${userId}`, AudioManager.getMetadataFromMessage(message));
            })
                .catch(error => {
    ...
    _closeListener(sessionId, connectionId, metadata = {}) {
        ...
        if (session) {
            return session.stopListener(connectionId)
                ...
                Logger.info(this._logPrefix, 'Listen only listener destroyed', metadata);
            })
                .catch((error) => {
                this._deleteIceQueue(this._getReqIdentifier(sessionId, connectionId));
                Logger.error(this._logPrefix, 'Listen only listener stop failed', Object.assign({ errorMessage: error.message, errorCode: error.code }, metadata));
            });
        }
        return Promise.resolve();
    }
    handleStop(message) {
        const { voiceBridge: sessionId, connectionId, } = message;
    ...
    handleIceCandidate(message) {
        const { voiceBridge: sessionId, connectionId, candidate, } = message;
        const session = this._fetchSession(sessionId);
        const iceQueue = this._fetchIceQueue(this._getReqIdentifier(sessionId, connectionId));
        if (session) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (message.id) {
                ...
                default:
                    ...
                    this.sendToClient(Object.assign({ type: 'audio' }, errorMessage), C.FROM_AUDIO);
                    break;
            }
        });
    }
