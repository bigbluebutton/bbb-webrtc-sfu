...
module.exports = class Audio extends BaseProvider {
    ...
    onIceCandidate(_candidate, connectionId) {
        ...
        if (endpoint && endpoint.mediaId) {
            try {
                ...
                Logger.error(LOG_PREFIX, "ICE candidate could not be added to media controller.", Object.assign(Object.assign({}, this._getFullLogMetadata(connectionId)), { error }));
            }
        }
        else {
    ...
    ;
    _flushCandidatesQueue(connectionId) {
    ...
    addUser(connectionId, user) {
        if (this.connectedUsers.hasOwnProperty(connectionId)) {
            Logger.debug(LOG_PREFIX, `Updating user for connectionId ${connectionId}`, this._getFullLogMetadata(connectionId));
        }
        this.connectedUsers[connectionId] = user;
        Logger.debug(LOG_PREFIX, `Added user with connectionId ${connectionId}`, this._getFullLogMetadata(connectionId));
    }
    ;
    ...
    removeUser(connectionId) {
        if (this.connectedUsers.hasOwnProperty(connectionId)) {
            Logger.info(LOG_PREFIX, `Removing user with connectionId ${connectionId}`, this._getFullLogMetadata(connectionId));
            delete this.connectedUsers[connectionId];
        }
        else {
            Logger.debug(LOG_PREFIX, `User not found on remove for connectionId ${connectionId}`, this._getPartialLogMetadata());
        }
    }
    ;
    /**
    ...
    getUser(connectionId, suppressLog = false) {
        if (this.connectedUsers.hasOwnProperty(connectionId)) {
            ...
            if (!suppressLog) {
                Logger.error(LOG_PREFIX, `User not found on getUser for connectionId ${connectionId}`, this._getPartialLogMetadata());
            }
            return {};
        }
    }
    ...
    */
    getConnectionId(userId) {
        ...
        Logger.error(LOG_PREFIX, `User not found on getConnectionId for userId ${userId}`, this._getPartialLogMetadata());
    }
    ;
    ...
    _onSubscriberMediaFlowing(connectionId) {
        Logger.debug(LOG_PREFIX, `Listen only WebRTC media is FLOWING`, this._getFullLogMetadata(connectionId));
    ...
    _onSubscriberMediaNotFlowing(connectionId) {
        Logger.debug(LOG_PREFIX, `Listen only WebRTC media is NOT_FLOWING`, this._getFullLogMetadata(connectionId));
        this.setMediaFlowingTimeout(connectionId);
    }
    _onSubscriberMediaNotFlowingTimeout(connectionId) {
        Logger.error(LOG_PREFIX, `Listen only WebRTC media NOT_FLOWING timeout reached`, this._getFullLogMetadata(connectionId));
        this.sendToClient({
            type: 'audio',
            connectionId: connectionId,
            id: "webRTCAudioError",
    ...
    setMediaFlowingTimeout(connectionId) {
        if (!this._mediaFlowingTimeouts[connectionId]) {
            Logger.debug(LOG_PREFIX, `Listen only NOT_FLOWING timeout set`, Object.assign(Object.assign({}, this._getFullLogMetadata(connectionId)), { MEDIA_FLOW_TIMEOUT_DURATION }));
    ...
    clearMediaFlowingTimeout(connectionId) {
        if (this._mediaFlowingTimeouts[connectionId]) {
            Logger.debug(LOG_PREFIX, `clearMediaFlowingTimeout for listener ${connectionId}`, this._getFullLogMetadata(connectionId));
    ...
    _onSubscriberMediaConnected(connectionId) {
        Logger.info(LOG_PREFIX, `Listen only WebRTC media is CONNECTED`, this._getFullLogMetadata(connectionId));
        this.clearMediaStateTimeout(connectionId);
    }
    ;
    _onSubscriberMediaDisconnected(connectionId) {
        Logger.warn(LOG_PREFIX, `Listen only WebRTC media is DISCONNECTED`, this._getFullLogMetadata(connectionId));
        this.setMediaStateTimeout(connectionId);
    }
    _onSubscriberMediaDisconnectedTimeout(connectionId) {
        Logger.error(LOG_PREFIX, `Listen only WebRTC media DISCONNECTED timeout reached`, this._getFullLogMetadata(connectionId));
        this.sendToClient({
            type: 'audio',
            connectionId: connectionId,
            id: "webRTCAudioError",
    ...
    setMediaStateTimeout(connectionId) {
        if (!this._mediaStateTimeouts[connectionId]) {
            Logger.warn(LOG_PREFIX, `Listen only DISCONNECTED media state timeout set`, Object.assign(Object.assign({}, this._getFullLogMetadata(connectionId)), { MEDIA_STATE_TIMEOUT_DURATION }));
    ...
    clearMediaStateTimeout(connectionId) {
        if (this._mediaStateTimeouts[connectionId]) {
            Logger.debug(LOG_PREFIX, `clearMediaStateTimeout for listener ${connectionId}`, this._getFullLogMetadata(connectionId));
    ...
    _mediaStateRTP(event, endpoint) {
        ...
        switch (name) {
            ...
            case "MediaFlowOutStateChange":
                Logger.info(LOG_PREFIX, `RTP source relay session ${mediaId} received MediaFlowOut`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { userId: this.userId, mediaId, state }));
                break;
            case "MediaFlowInStateChange":
                Logger.info(LOG_PREFIX, `RTP source relay session ${mediaId} received MediaFlowIn`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { userId: this.userId, mediaId, state }));
                break;
    ...
    _onMCSIceCandidate(event, endpoint, connectionId) {
        ...
        Logger.debug(LOG_PREFIX, `Received ICE candidate from mcs-core`, Object.assign(Object.assign({}, this._getFullLogMetadata(connectionId)), { candidate }));
        this.sendToClient({
            type: 'audio',
            connectionId,
            id: 'iceCandidate',
    ...
    _handleIceComponentStateChange(state, logMetadata) {
        ...
        Logger.debug(LOG_PREFIX, `Audio ICE component state changed`, Object.assign(Object.assign({}, logMetadata), { elementId,
            iceComponentId,
            iceComponentState }));
    }
    _handleCandidatePairSelected(state, logMetadata) {
        ...
        Logger.info(LOG_PREFIX, `Audio new candidate pair selected`, Object.assign(Object.assign({}, logMetadata), { elementId,
            iceComponentId,
            localCandidate,
            remoteCandidate }));
    }
    _handleIceGatheringDone(state, logMetadata) {
        ...
        Logger.debug(LOG_PREFIX, "Audio ICE gathering done", Object.assign(Object.assign({}, logMetadata), { elementId }));
    }
    _handleMediaStateChanged(state, connectionId, logMetadata) {
        const { rawEvent, details } = state;
        const { source: elementId } = rawEvent;
        Logger.debug(LOG_PREFIX, "Audio media state changed", Object.assign(Object.assign({}, logMetadata), { elementId, mediaState: details }));
    ...
    _mediaStateWebRTC(event, endpoint, connectionId) {
        ...
        switch (name) {
            ...
            case "MediaFlowOutStateChange":
            case "MediaFlowInStateChange":
                Logger.debug(LOG_PREFIX, `Listen only WebRTC media received MediaFlow state`, Object.assign(Object.assign({}, logMetadata), { state }));
                if (details === 'FLOWING') {
            ...
            case C.MEDIA_SERVER_OFFLINE:
                Logger.error(LOG_PREFIX, `WebRTC listen only session ${mediaId} received MEDIA_SERVER_OFFLINE event`, Object.assign(Object.assign({}, logMetadata), { event }));
    ...
    startGlobalAudioBridge(mediaServer) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            Logger.info(LOG_PREFIX, `Starting RTP source audio/GLOBAL_AUDIO for ${this.voiceBridge}`, this._getPartialLogMetadata());
            try {
                if (!this.sourceAudioStarted && this.sourceAudioStatus === C.MEDIA_STOPPED) {
                    ...
                    const userId = yield this.mcs.join(this.voiceBridge, 'SFU', { name: this.globalAudioBridge });
                    this.userId = userId;
                    ...
                    const { mediaId, answer } = yield this.mcs.publish(this.userId, this.voiceBridge, C.RTP, globalAudioOptions);
                    ...
                    const { mediaId: proxyId, answer: proxyAnswer } = yield this.mcs.subscribe(this.userId, mediaId, C.RTP, proxyOptions);
                    ...
                    // relay RTP
                    yield this.mcs.publish(this.userId, this.voiceBridge, C.RTP, Object.assign(Object.assign({}, globalAudioOptions), { mediaId, descriptor: proxyAnswer }));
                    this.sourceAudio = proxyId;
                    this.sourceAudioStarted = true;
                    this.sourceAudioStatus = C.MEDIA_STARTED;
                    this.emit(C.MEDIA_STARTED);
                    Logger.info(LOG_PREFIX, `Listen only source RTP relay successfully created`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { mediaId: this.sourceAudio, userId: this.userId }));
                    return resolve(true);
                }
            }
            catch (error) {
                Logger.error(LOG_PREFIX, `Error on starting listen only source RTP relay`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { error }));
    ...
    start(sessionId, connectionId, sdpOffer, userId, userName, mediaServer) {
        return __awaiter(this, void 0, void 0, function* () {
            let mcsUserId;
            const isConnected = yield this.mcs.waitForConnection();
            if (!isConnected) {
            ...
            try {
                yield this.getGlobalAudioPermission(this.meetingId, this.voiceBridge, userId, connectionId);
            }
            catch (error) {
                const normalizedError = this._handleError(LOG_PREFIX, error, "recv", userId);
                Logger.error(LOG_PREFIX, 'New listen only session failed: unauthorized', Object.assign(Object.assign({}, this._getPartialLogMetadata()), { error: normalizedError }));
                throw normalizedError;
            }
            try {
                yield this._waitForGlobalAudio(mediaServer);
            }
            catch (error) {
                const normalizedError = this._handleError(LOG_PREFIX, error, "recv", userId);
                Logger.error(LOG_PREFIX, `New listen only session failed: GLOBAL_AUDIO unavailable`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { error: normalizedError }));
                throw errors.SFU_GLOBAL_AUDIO_FAILED;
            }
            try {
                mcsUserId = yield this.mcs.join(this.voiceBridge, 'SFU', { externalUserId: userId, autoLeave: true });
            }
            catch (error) {
                const normalizedError = this._handleError(LOG_PREFIX, error, "recv", userId);
                Logger.error(LOG_PREFIX, `mcs-core join failure for new listen only session`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { connectionId,
                    userId, error: normalizedError }));
                throw normalizedError;
            }
            ...
            Logger.info(LOG_PREFIX, `Starting new listen only session`, this._getFullLogMetadata(connectionId));
            try {
                const sdpAnswer = yield this._subscribeToGlobalAudio(sdpOffer, connectionId, mediaServer);
                return sdpAnswer;
            }
            catch (error) {
                const normalizedError = this._handleError(LOG_PREFIX, error, "recv", userId);
                Logger.error(LOG_PREFIX, `GLOBAL_AUDIO subscribe failed`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { connectionId,
                    userId, error: normalizedError }));
                // Rollback
    ...
    _subscribeToGlobalAudio(sdpOffer, connectionId, mediaServer) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let mediaId, answer;
            try {
                ({ mediaId, answer } = yield this.mcs.subscribe(mcsUserId, this.sourceAudio, C.WEBRTC, options));
            }
            catch (subscribeError) {
                Logger.error(LOG_PREFIX, `New listen only session failed to subscribe to GLOBAL_AUDIO`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { subscribeError }));
            ...
            this._flushCandidatesQueue(connectionId);
            Logger.info(LOG_PREFIX, 'Listen only session subscribed to global audio', this._getFullLogMetadata(connectionId));
            return answer;
        });
    }
    /* ======= STOP METHODS ======= */
    stopListener(connectionId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (listener && listener.mediaId && listener.mcsUserId) {
                try {
                    yield this.mcs.unsubscribe(listener.mcsUserId, listener.mediaId);
                    Logger.info(LOG_PREFIX, 'Listen only session stopped', this._getFullLogMetadata(connectionId));
                }
                catch (error) {
                    Logger.warn(LOG_PREFIX, `Error on unsubscribing listener media ${listener.mediaId}`, Object.assign(Object.assign({}, this._getFullLogMetadata(connectionId)), { error }));
                }
            }
            this.sendUserDisconnectedFromGlobalAudioMessage(connectionId);
    ...
    stop() {
        return __awaiter(this, void 0, void 0, function* () {
            Logger.info(LOG_PREFIX, `Listen only session-wide stop for room ${this.voiceBridge}, releasing everything`, this._getPartialLogMetadata());
            ...
            try {
                ...
                for (var connectionId in this.connectedUsers) {
                    try {
                        yield this.stopListener(connectionId);
                    }
                    catch (error) {
                        Logger.error(LOG_PREFIX, `Listen only session stop failed`, Object.assign(Object.assign({}, this._getFullLogMetadata(connectionId)), { error }));
                    }
                }
                return Promise.resolve();
            }
            catch (error) {
                Logger.error(LOG_PREFIX, `Error on session-wide stop for room ${this.voiceBridge}}`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { error }));
    ...
    _stopSourceAudio() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.userId) {
                try {
                    yield this.mcs.leave(this.voiceBridge, this.userId);
                    this.userId = null;
                }
                catch (error) {
                    Logger.warn(LOG_PREFIX, `Error on stopping source audio ${this.voiceBridge} with MCS user ${this.userId}`, Object.assign(Object.assign({}, this._getPartialLogMetadata()), { error }));
                }
            }
            this.sourceAudioStarted = false;
            this.sourceAudioStatus = C.MEDIA_STOPPED;
